...
mod ebpf_integration_tests {
    ...
    fn test_bpf_helper_get_prandom_u32() {
        ...
        let result1 = helper
            .execute(&mut vm)
            .expect("First random generation failed");

        let mut vm2 = BpfVm::new();
        let result2 = helper
            .execute(&mut vm2)
            .expect("Second random generation failed");
    ...
    fn test_bpf_program_complex_flow() {
        ...
        let program = vec![
            ...
            BpfInstruction::SubImm { dst_reg: 1, imm: 1 },
            BpfInstruction::Jne {
                dst_reg: 1,
    ...
    fn test_bpf_program_verification_before_load() {
        ...
        let result =
            registry.load_program(BpfProgType::Tracing, invalid_program, "invalid".to_string());
    ...
    fn test_bpf_program_list() {
        ...
        for i in 0..3 {
            ...
            registry
                .load_program(BpfProgType::Tracing, program, format!("prog{}", i))
                .expect("Program loading failed");
        }
    ...
    fn test_ebpf_verification_report_accuracy() {
        ...
        assert_eq!(
            report.instructions_verified, 4,
            "All instructions should be verified"
        );
        assert_eq!(report.errors.len(), 0, "No errors expected");
    }
