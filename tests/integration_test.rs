...
mod tests {
    ...
    #[test]
    fn test_kernel_lists_and_primitives() {
        // Singly linked list verification
        let mut s_list = SinglyLinkedList::new(10);
        s_list.push_next(20);
        s_list.push_next(30);
        assert_eq!(s_list.value, 10);
        assert_eq!(s_list.next.as_ref().unwrap().value, 30); // pushed next to head

        // Sequenced Singly linked list verification
        let seq_list = SequencedSinglyLinkedList::new(42, 100);
        assert_eq!(seq_list.value, 42);
        assert_eq!(seq_list.sequence_number, 100);

        // Circular doubly linked list verification
        let c_list = CircularDoublyLinkedList::new(99);
        assert_eq!(c_list.value, Some(99));

        // SystemThread register context verification
        let mut thread = SystemThread::new(3, 10, CpuArchitectureClass::X64);
        assert_eq!(thread.thread_id, 3);
        assert_eq!(thread.priority, 10);
        assert_eq!(thread.arch, CpuArchitectureClass::X64);
        thread.register_context[0] = 0xDEADBEEF; // rax
        assert_eq!(thread.register_context[0], 0xDEADBEEF);

        // WorkItem atomics verification
        let work = WorkItem::new(1001, 0x11223344);
        assert_eq!(work.work_id, 1001);
        assert_eq!(work.payload_hash, 0x11223344);
        assert!(!work.is_processed.load(Ordering::SeqCst));
        work.is_processed.store(true, Ordering::SeqCst);
        assert!(work.is_processed.load(Ordering::SeqCst));

        // APC queues delivery verification
        let mut apc_queue = ApcQueue::new();
        apc_queue.queue_apc(Apc {
            apc_id: 101,
        ...
        assert_eq!(apc_queue.deliver_apcs(ApcMode::KernelMode), 1);
        assert_eq!(apc_queue.apcs.len(), 1); // UserMode Apc retained
        assert_eq!(apc_queue.apcs[0].apc_id, 102);
    }

    #[test]
    fn test_edf_realtime_scheduler() {
        let manager = AdvancedAlgorithmsManager::new(42);

        let t1 = EdfTask {
            tid: 101,
            absolute_deadline: 50,
        ...
        let t3 = EdfTask {
            ...
            remaining_execution: 15,
            is_active: true,
        };

        assert!(manager.add_edf_task(t1).is_ok());
        assert!(manager.add_edf_task(t2).is_ok());
        assert!(manager.add_edf_task(t3).is_ok());

        // Select task with the closest absolute deadline (t2 has 20, which is the closest)
        let selected = manager.schedule_edf().unwrap();
        assert_eq!(selected.tid, 102);
    }

    #[test]
    fn test_lottery_scheduler_draws() {
        ...
        let t2 = LotteryTask {
            ...
            tickets: 90,
            is_active: true,
        }; // t2 holds 90% of tickets

        assert!(manager.add_lottery_task(t1).is_ok());
        assert!(manager.add_lottery_task(t2).is_ok());

        // Draw multiple lottery scheduler rounds
        let mut t1_wins = 0;
        let mut t2_wins = 0;
        for _ in 0..100 {
            if let Some(task) = manager.schedule_lottery() {
                if task.tid == 1 {
                    t1_wins += 1;
                } else if task.tid == 2 {
                    t2_wins += 1;
        ...
        // t2 should statistically win a significant majority of rounds
        assert!(t2_wins > t1_wins);
        println!(
            "Lottery Scheduler Wins: Task #1 (10 tix) = {}, Task #2 (90 tix) = {}",
            t1_wins, t2_wins
        );
    }

    #[test]
    fn test_consensus_audit_ledger() {
        let manager = AdvancedAlgorithmsManager::new(999);

        // Append events
        assert!(manager
            .audit_process_event(1716000000, 1, 100, 0x1111)
            .is_ok());
        assert!(manager
            .audit_process_event(1716000001, 3, 100, 0x2222)
            .is_ok());

        // Validate cryptographic chain integrity
        assert!(manager.verify_ledger_integrity());

        // Simulate tamper: edit block 1's timestamp in the ledger directly
        {
            let mut ledger = manager.audit_ledger.borrow_mut();
            if let Some(ref mut block) = ledger[0] {
                block.timestamp = 9999999999;
            }
        }

        // Verify ledger integrity should now trigger cryptographic hash mismatch failure!
        assert!(!manager.verify_ledger_integrity());
    }
}
