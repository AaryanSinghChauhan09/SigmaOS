...
use sigmaos::kernel::{
    Priority, Process, ProcessState, ArchitectureEngine, CpuRegisters, HardwareException,
    InstructionCyclePhase as ArchInstructionCyclePhase, Irql, LookasideList, MemoryDescriptorList, Pcb, PoolType,
...
mod tests {
    ...
    fn test_cachyos_uksm_samepage_merger() {
        ...
        let mut frames = [
            PhysicalPageFrame { address: 0x1000, content_hash: 0xAAAA_BBBB },
            PhysicalPageFrame { address: 0x2000, content_hash: 0xCCCC_DDDD },
            PhysicalPageFrame { address: 0x3000, content_hash: 0xAAAA_BBBB }, // Duplicate identical page!
        ];

        let merged = merger.deduplicate_pages(&mut frames);
        assert_eq!(merged, 1);
        assert_eq!(merger.scanned_pages_count.load(core::sync::atomic::Ordering::SeqCst), 3);
        assert_eq!(merger.saved_pages_count.load(core::sync::atomic::Ordering::SeqCst), 1);
    }
    ...
    #[test]
    fn test_aur_pkgbuild_parsing_and_compilation_sandbox() {
        let mut orchestrator = AurSandboxOrchestrator::new();

        // 1. Verify line lexing and extraction of PKGBUILD variables
        let meta_opt = orchestrator.parse_pkgbuild_line("pkgname=custom-kernel-git");
        assert!(meta_opt.is_some());
        let meta = meta_opt.unwrap();

        // Check computed target name hash parity
        let expected_hash = AurSandboxOrchestrator::calculate_name_hash("custom-kernel-git");
        assert_eq!(meta.name_hash, expected_hash);
        assert_eq!(meta.version_major, 1);
        assert_eq!(meta.version_minor, 0);

        // 2. Verify parsing dependencies array
        orchestrator.parse_pkgbuild_line("depends=('rust' 'clang' 'git')");
        assert_eq!(orchestrator.dep_count.get(), 3);

        {
            let deps = orchestrator.dependencies.borrow();
            assert_eq!(deps[0], Some(AurSandboxOrchestrator::calculate_name_hash("rust")));
            assert_eq!(deps[1], Some(AurSandboxOrchestrator::calculate_name_hash("clang")));
            assert_eq!(deps[2], Some(AurSandboxOrchestrator::calculate_name_hash("git")));
        }

        // 3. Verify hermetic sandboxed setup
        let sandbox = orchestrator.prepare_compilation_sandbox(&meta);
        assert!(!sandbox.allow_internet); // hermetic
        assert_eq!(sandbox.restricted_source_path_hash, meta.name_hash);

        // 4. Verify compiler output registration and CAS hashing
        let package_hash = orchestrator.run_compilation(meta, &sandbox).unwrap();
        assert_eq!(package_hash, meta.name_hash ^ 0xAAAAAAAA);
    }
}
