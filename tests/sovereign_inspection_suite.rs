...
// - Audacity / Audition multi-track mixing & GarageBand / Anchor podcast RSS publishing
// - Aegisub / Subtitle Edit subtitle parsing, karaoke styling, and framerate synchronization
...
#[path = "../src/audio/podcast.rs"]
mod podcast;

#[path = "../src/productivity/subtitle_editor.rs"]
mod subtitle_editor;
...
#[test]
fn test_inspection_audacity_audition_multitrack_editor() {
    let track1 = audio_editor::AudioTrack::new(1, "Vocal Lead").with_samples(&[0.1, 0.5, 0.8, -0.2]);
    let track2 = audio_editor::AudioTrack::new(2, "Guitar Backing").with_samples(&[0.2, 0.2, 0.3, 0.1]).with_volume(0.8);

    let mut session = audio_editor::MultiTrackSession::new(44100);
    session.add_track(track1);
    session.add_track(track2);

    let mixdown = session.mix_session();
    assert_eq!(mixdown.len(), 4);
}

#[test]
fn test_inspection_garageband_anchor_podcast_publisher() {
    let recorder = podcast::PodcastRecorder::new();
    recorder.toggle_recording(true);

    let feed = podcast::PodcastFeed::new("Sovereign OS Digest", "Weekly updates on SigmaOS", "SigmaOS Team");
    let publisher = podcast::AnchorPublisher::new("token123", feed);
    let xml_feed = publisher.export_rss_xml();
    assert!(xml_feed.contains("<title>Sovereign OS Digest</title>"));
}

#[test]
fn test_inspection_aegisub_subtitle_edit_engine() {
    let mut converter = subtitle_editor::SubtitleEditEngine::new(subtitle_editor::SubtitleFormat::SubRip);
    let entries = vec![
        subtitle_editor::SubtitleEntry::new(
            1,
    ...
    converter.load_entries(entries);

    converter.synchronize_all(Duration::from_millis(500), true);
    assert_eq!(converter.entries[0].start_time.as_millis(), 1500);

    let srt_export = converter.export_format(subtitle_editor::SubtitleFormat::SubRip);
    assert!(srt_export.contains("Welcome to SigmaOS!"));
}
