...
#define SIGMA_ASSERT(cond, msg) \
    if (!(cond)) { sigma_kprintf("Σ [FAIL]: %s\n", msg); sigma_exit(1); }

// =========================================================================
// Σ MEMORY SUBSYSTEM TESTS
// =========================================================================
void test_slab_allocation() {
    sigma_kprintf("Σ [TEST]: Running Slab Allocation Test...\n");
    ...
    sigma_free(ptr);
    sigma_kprintf("Σ [PASS]: Slab Allocation & Free life-cycle verified.\n");
}
...
void test_process_scheduling_logic() {
    sigma_kprintf("Σ [TEST]: Running Process Scheduling Logic Test...\n");
    ...
    sigma_kprintf("Σ [PASS]: Process scheduling logic verified.\n");
}
...
int main(int argc, char** argv) {
    sigma_kprintf("--- Σ SIGMAOS ZENITH MASTER TEST SUITE: SILICON-DIRECT --- \n");
    
    test_slab_allocation();
    test_process_scheduling_logic();
    
    sigma_kprintf("--- Σ ALL SOVEREIGN TESTS PASSED ACCORDING TO PLAN. --- \n");
    return 0;
}
