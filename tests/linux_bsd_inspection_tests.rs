...
fn test_gentoo_portage_mask_engine_inspection() {
    ...
    portage.set_use_flag("+pipewire");

    assert!(portage.is_flag_enabled("wayland"));
    assert!(portage.is_flag_enabled("pipewire"));
    assert!(!portage.is_flag_enabled("x11"));

    assert!(portage.resolve_conflicts(("wayland", "x11")).is_ok());
    portage.set_use_flag("+x11");
    assert!(portage.resolve_conflicts(("wayland", "x11")).is_err());
}
