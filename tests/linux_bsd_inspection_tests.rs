...
#[test]
fn test_sovereign_cron_spool_and_inotify_inspection() {
    use cron::SovereignCronDaemon;

    let mut cron_daemon = SovereignCronDaemon::new();

    // 1. Spool Dir File Scanning
    cron_daemon.spool_directory.load_crontab("alice", "0 2 * * * /usr/bin/backup_home.sh");
    assert_eq!(cron_daemon.spool_directory.spool_files.len(), 1);
    assert_eq!(
        cron_daemon.spool_directory.spool_files["alice"].crontab_path,
        "/var/spool/cron/crontabs/alice"
    );

    // 2. Inotify Hot-Reloading Watcher
    cron_daemon.inotify_watcher.watch_path("/var/spool/cron/crontabs/alice");
    cron_daemon.inotify_watcher.notify_modification("/var/spool/cron/crontabs/alice");
    let reloads = cron_daemon.inotify_watcher.process_reloads();
    assert_eq!(reloads, vec!["/var/spool/cron/crontabs/alice".to_string()]);

    // 3. FreeBSD-style cron.allow/cron.deny ACL
    cron_daemon.allow_user("alice");
    cron_daemon.deny_user("mallory");
    assert!(cron_daemon.is_user_permitted("alice"));
    assert!(!cron_daemon.is_user_permitted("mallory"));
}
