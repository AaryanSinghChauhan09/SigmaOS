...
fn test_namespace_context_cloning() {
    ...
    assert_eq!(context1.metadata_summary(), context2.metadata_summary());
}
...
fn test_process_descriptor_child_creation() {
    let parent =
        ProcessDescriptor::new_root(1, 1, 0, "init".to_string()).expect("Failed to create parent");
...
fn test_multi_level_process_hierarchy() {
    ...
    let root =
        ProcessDescriptor::new_root(1, 1, 0, "init".to_string()).expect("Failed to create root");
    ...
    let grandchild1 = child1
        .create_child(4, 2, "grandchild1".to_string())
...
fn test_cross_namespace_access_prevention() {
    ...
    let isolated1 = parent
        .create_isolated_child(2, 1, "iso1".to_string())
        .expect("Failed to create isolated1");

    let isolated2 = parent
        .create_isolated_child(3, 1, "iso2".to_string())
...
fn test_large_process_hierarchy() {
    ...
    for i in 0..20 {
        ...
        let child = parent
            .create_child(kernel_pid, parent.kernel_pid, format!("proc_{}", i))
            .expect("Failed to create child");
...
fn test_concurrent_descriptor_creation() {
    ...
    let desc = Arc::new(
        ProcessDescriptor::new_root(1, 1, 0, "init".to_string())
            .expect("Failed to create descriptor"),
    );
...
fn test_process_tree_relationships() {
    ...
    let parent = root
        .create_child(2, 1, "parent".to_string())
        .expect("Failed to create parent");

    let child = parent
        .create_child(3, 2, "child".to_string())
        .expect("Failed to create child");
