...
fn test_universal_adapter_all_formats() {
    ...
    let yay_action = dispatcher.dispatch_command("yay -Syu --noconfirm neovim").unwrap();
    ...
    let microdnf_action = dispatcher.dispatch_command("microdnf remove httpd").unwrap();
