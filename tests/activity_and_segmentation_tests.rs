...
fn test_activity_manager_complete_suite() {
    ...
    am.update_activity_metrics(300, 95.0, 512 * 1024 * 1024, 1048576, 524288, 5000).unwrap();
    ...
    am.bind_address_space(200, "/usr/bin/firefox", 0x0000_7FFF_0000_0000, 0x500000, 0x100000, true).unwrap();
    ...
    am.update_activity_metrics(400, 0.0, 1024 * 1024, 0, 0, 1000).unwrap();
...
fn test_segmentation_paging_and_protection_suite() {
    ...
    let linear0 = engine.translate_logical_to_linear(ring0_cs, 0x4000, CpuRing::Ring0Kernel).unwrap();
    assert_eq!(linear0, 0x4000);

    let ring3_cs = SegmentSelector::new(3, false, CpuRing::Ring3User);
    let linear3 = engine.translate_logical_to_linear(ring3_cs, 0x8000, CpuRing::Ring3User).unwrap();
    ...
    let smep_err = engine.translate_virtual_to_physical(0x0000_0000_0001_0000, false, true, CpuRing::Ring0Kernel);
    assert!(smep_err.is_err());

    // SMAP: Ring 0 Kernel accessing User-space page data -> Fault
    let smap_err = engine.translate_virtual_to_physical(0x0000_0000_0001_0000, false, false, CpuRing::Ring0Kernel);
    assert!(smap_err.is_err());

    // W^X: Simultaneous Write and Execute -> Security Violation
    let wx_err = engine.translate_virtual_to_physical(0x0000_7FFF_0000_0000, true, true, CpuRing::Ring3User);
    assert!(wx_err.is_err());

    // 3. ASLR Layout Entropy
    let config = AslrEntropyConfig::linux_default();
    let aslr_layout = RandomizedAddressSpace::compute_aslr_layout(0x0000_0000_0040_0000, config, 0xABCD1234);
