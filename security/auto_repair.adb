...
package Sigma.auto_repair is
   ...
   type SovereignAutoRepair_T is tagged record
      Initialized : SigmaBool := False;
   end record;

   procedure IgniteRepair (Self : in out SovereignAutoRepair_T)
     with Post => Self.Initialized = True;

   procedure SelfHeal (Self : in out SovereignAutoRepair_T)
     with Post => Self.Initialized = True;

   procedure IgniteRepair
     with Export, Convention => C, External_Name => "IgniteRepair";

   procedure SelfHeal
     with Export, Convention => C, External_Name => "SelfHeal";


end Sigma.auto_repair;
