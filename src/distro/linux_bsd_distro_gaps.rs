...
impl SovereignDnsTlsResolverEngine {
    ...
    pub fn resolve_domain(&self, domain: &str) -> Option<[u8; 4]> {
        self.local_cache.iter().find(|r| r.domain_name == domain).map(|r| r.ip_address)
    }
...
impl SovereignDynamicDevfsEngine {
    ...
    pub fn create_node(
        ...
        self.devices.push(node.clone());
        self.nodes.push(node);
    }

    pub fn register_device_node(&mut self, name: &'static str, node_type: DeviceNodeType, major: u32, minor: u32) {
