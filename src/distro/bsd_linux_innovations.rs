...
// ============================================================================
// 10. OpenBSM BSD Security Event Audit Framework (FreeBSD/OpenBSD/macOS auditd)
// ============================================================================

/// OpenBSM Security Audit Event Class
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum OpenBsmAuditEventClass {
    Exec,               // ex
    FileCreate,         // fc
    FileDelete,         // fd
    FileWrite,          // fw
    FileRead,           // fr
    PrivilegeEscalation,// p1
    NetworkConnect,     // nt
}

/// OpenBSM Audit Record Entry
#[derive(Debug, Clone)]
pub struct OpenBsmAuditRecord {
    pub record_seq: u64,
    pub class: OpenBsmAuditEventClass,
    pub pid: u32,
    pub uid: u32,
    pub euid: u32,
    pub path: &'static str,
...
/// OpenBSM BSD Security Event Audit System (`auditd`, `praudit`, `/etc/security/audit_control`, `/dev/auditpipe`)
#[derive(Debug)]
pub struct OpenBsmBsdAuditEventEngine {
    pub enabled_classes: Vec<OpenBsmAuditEventClass>,
    pub audit_records: Vec<OpenBsmAuditRecord>,
    pub next_seq: u64,
    pub auditpipe_subscribers: usize,
}

impl OpenBsmBsdAuditEventEngine {
    /// Parse audit flags from `/etc/security/audit_control` (e.g., "ex,fc,fd,p1")
    pub fn new(audit_control_flags: &str) -> Self {
        let mut classes = Vec::new();
        for flag in audit_control_flags.split(',') {
            match flag.trim() {
                "ex" | "exec" => classes.push(OpenBsmAuditEventClass::Exec),
                "fc" | "create" => classes.push(OpenBsmAuditEventClass::FileCreate),
                "fd" | "delete" => classes.push(OpenBsmAuditEventClass::FileDelete),
                "fw" | "write" => classes.push(OpenBsmAuditEventClass::FileWrite),
                "fr" | "read" => classes.push(OpenBsmAuditEventClass::FileRead),
                "p1" | "priv" => classes.push(OpenBsmAuditEventClass::PrivilegeEscalation),
                "nt" | "net" => classes.push(OpenBsmAuditEventClass::NetworkConnect),
                _ => {}
            }
        }
        Self {
            ...
            next_seq: 1,
            auditpipe_subscribers: 0,
        }
    }

    /// Log security event if event class is configured in `/etc/security/audit_control`
    pub fn log_event(
        &mut self,
        class: OpenBsmAuditEventClass,
        pid: u32,
        uid: u32,
        euid: u32,
        path: &'static str,
        ...
        if !self.enabled_classes.contains(&class) {
            return None; // Class not audited
        }

        let seq = self.next_seq;
        self.next_seq += 1;

        self.audit_records.push(OpenBsmAuditRecord {
            record_seq: seq,
            class,
            pid,
            uid,
    ...
    /// Format record into human-readable OpenBSM `praudit` string output
    pub fn format_praudit_output(&self, seq: u64) -> Option<&'static str> {
        self.audit_records.iter().find(|r| r.record_seq == seq)?;
        Some("header,116,1,execve(2),0,Mon Sep 20 00:00:00 2026, + 0 msec\nsubject,root,root,root,root,root,1001,0,0,0.0.0.0\nreturn,success,0\ntrailer,116")
    }

    /// Connect subscriber to `/dev/auditpipe` real-time event stream
    pub fn subscribe_auditpipe(&mut self) -> usize {
        self.auditpipe_subscribers += 1;
        self.auditpipe_subscribers
    }

    pub fn get_audited_record_count(&self) -> usize {
        self.audit_records.len()
    }
}

impl Default for OpenBsmBsdAuditEventEngine {
    fn default() -> Self {
        Self::new("ex,fc,fd,p1")
    }
}
...
mod tests {
    ...
    #[test]
    fn test_openbsm_bsd_audit_event_engine() {
        let mut audit_engine = OpenBsmBsdAuditEventEngine::new("ex,fc,fd,p1");
        assert_eq!(audit_engine.get_audited_record_count(), 0);

        // Exec event is enabled
        let seq1 = audit_engine.log_event(
            OpenBsmAuditEventClass::Exec,
            1001,
            0,
            0,
            "/usr/bin/sudo",
        ...
        assert_eq!(audit_engine.get_audited_record_count(), 1);

        // NetworkConnect is not in "ex,fc,fd,p1" control flags
        let seq2 = audit_engine.log_event(
            OpenBsmAuditEventClass::NetworkConnect,
            1001,
            0,
            0,
            "10.0.0.1:443",
        ...
        // praudit output formatting
        let praudit_out = audit_engine.format_praudit_output(1);
        assert!(praudit_out.is_some());
        assert!(praudit_out.unwrap().contains("execve"));

        // /dev/auditpipe subscriber
        assert_eq!(audit_engine.subscribe_auditpipe(), 1);
    }
}
