...
mod tests {
    ...
    fn test_arch_devtools_pkgctl_archweb_archinstall_wiki() {
        ...
        let cmd = devtools.build_in_clean_chroot("curl").unwrap();
        assert!(cmd.contains("curl-1-x86_64.pkg.tar.zst"));
        ...
        let installer = ArchArchinstallEngine::default();
        let ok = installer.execute_installation_profile("btrfs");
        assert!(ok);

        let wiki = ArchWikiOfflineEngine::new();
        let article = wiki.search_offline_wiki("pacman");
        assert!(article.contains("ArchWiki"));
    }
