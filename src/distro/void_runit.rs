...
impl RunitSupervisor {
    ...
    /// Check if service can stop (dependent services already stopped)
    fn can_stop_service(&self, name: &str, stopped: &[String]) -> bool {
        for (s_name, service) in &self.services {
            if !stopped.contains(s_name) && service.dependencies.contains(&name.to_string()) {
                return false;
            }
        }
        true
