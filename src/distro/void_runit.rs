...
impl RunitSupervisor {
    ...
    /// Check if service can stop (no active services depending on it)
    fn can_stop_service(&self, name: &str, stopped: &[String]) -> bool {
        for service in self.services.values() {
            if service.status == RunitServiceStatus::Running
                && service.dependencies.contains(&name.to_string())
                && !stopped.contains(&service.name)
            {
                return false;
            }
        }
