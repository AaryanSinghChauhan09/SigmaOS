...
/// Runit Service Supervision Stages
...
impl RunitSupervisor {
    ...
    /// Check if service can stop (dependent services stopped)
    pub fn can_stop_service(&self, name: &str, stopped: &[String]) -> bool {
        for (other_name, service) in &self.services {
            if service.dependencies.contains(&name.to_string()) && !stopped.contains(other_name) {
                return false;
            }
        }
        true
