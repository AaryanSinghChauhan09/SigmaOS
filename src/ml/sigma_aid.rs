...
impl SigmaAid {
    ...
    pub fn load_gguf_model(&mut self, path: &str) -> Result<(), &'static str> {
        self.status.store(ModelStatus::Loading as usize, Ordering::SeqCst);
        ...
        self.status.store(ModelStatus::Ready as usize, Ordering::SeqCst);
        Ok(())
    }
    ...
    pub fn execute_prompt(&self, prompt: &str) -> String {
        ...
        self.status.store(ModelStatus::Inferencing as usize, Ordering::SeqCst);
...
mod tests {
    ...
    fn test_gguf_loading() {
        ...
        assert_eq!(aid.status.load(Ordering::SeqCst), ModelStatus::Unloaded as usize);
        
        assert!(aid.load_gguf_model("/models/llama3-8b.gguf").is_ok());
        assert_eq!(aid.status.load(Ordering::SeqCst), ModelStatus::Ready as usize);
