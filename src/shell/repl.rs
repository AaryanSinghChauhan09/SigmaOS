...
impl ShellRepl {
    ...
    pub fn parse_command(&self, input: &str) -> ShellCommand {
        ...
        match parts[0] {
            ...
            "sestatus" => ShellCommand::SeStatus,
            "setenforce" => {
                if parts.len() >= 2 {
                    ShellCommand::SetEnforce {
    ...
    pub fn execute_command(&mut self, command: ShellCommand) -> Result<String, String> {
        match command {
            ShellCommand::Help => Ok("Available commands:\n\
                   ...
                   sestatus  - Query security status and AVC caching of SELinux\n\
                   setenforce- Toggle SELinux Mode (enforcing, permissive, disabled)\n\
                   exit      - Exit the shell"
                .to_string()),
            ...
            ShellCommand::SeStatus => {
                use crate::security::SelinuxEngine;
                let mut engine = SelinuxEngine::new();

                // Perform a mock policy lookup to populate hit/miss
                let _ = engine.has_permission(
                ...
                let out = format!(
                    "SELinux status:\n  \
                     ...
                     Active policy rules loaded:     {}\n  \
                     AVC hit ratio metrics:\n    \
                       * Hits: {}\n    \
                       * Misses: {}\n    \
                       * Logged Denials: {}",
                    engine.mode.as_str(),
                    ...
                    engine.avc.hits,
                    engine.avc.misses,
                    engine.audit_logs.len()
                );
                Ok(out)
            }
            ShellCommand::SetEnforce { mode } => {
                use crate::security::{SelinuxEngine, SeLinuxMode};
                let mut engine = SelinuxEngine::new();

                match mode.trim().to_lowercase().as_str() {
                    "enforcing" | "1" => {
                        engine.set_mode(SeLinuxMode::Enforcing);
                        Ok("SELinux mode updated to: enforcing".to_string())
                    }
                    "permissive" | "0" => {
                        engine.set_mode(SeLinuxMode::Permissive);
                        Ok("SELinux mode updated to: permissive".to_string())
                    }
                    "disabled" => {
                        engine.set_mode(SeLinuxMode::Disabled);
                        Ok("SELinux mode updated to: disabled".to_string())
                    }
                    _ => Err("Invalid SELinux mode! Use 'enforcing' (1), 'permissive' (0), or 'disabled'".to_string()),
                }
            }
...
mod tests {
    ...
    #[test]
    fn test_selinux_repl_commands() {
        let mut repl = ShellRepl::new();

        // Check status
        let cmd_status = repl.parse_command("sestatus");
        let status_res = repl.execute_command(cmd_status).unwrap();
        assert!(status_res.contains("SELinux status:                 enabled"));
        assert!(status_res.contains("Current mode:                   enforcing"));

        // Toggle to permissive
        let cmd_permissive = repl.parse_command("setenforce permissive");
        let permissive_res = repl.execute_command(cmd_permissive).unwrap();
        assert_eq!(permissive_res, "SELinux mode updated to: permissive");

        // Toggle to enforcing (using numeric 1)
        let cmd_enforce = repl.parse_command("setenforce 1");
        let enforce_res = repl.execute_command(cmd_enforce).unwrap();
        assert_eq!(enforce_res, "SELinux mode updated to: enforcing");
    }
}
