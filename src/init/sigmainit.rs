...
impl Supervisor {
    ...
    pub fn start_service(&mut self, name: &str) -> Result<(), ServiceError> {
        ...
        let deps_vec = self.dependency_graph.get_dependencies(name).cloned();
        if let Some(deps) = deps_vec {
            for dep in &deps {
    ...
    pub fn stop_service(&mut self, name: &str) -> Result<(), ServiceError> {
        ...
        let dependents_vec = self.dependency_graph.get_dependents(name).cloned();
        if let Some(dependents) = dependents_vec {
