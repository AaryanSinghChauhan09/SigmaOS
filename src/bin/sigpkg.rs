...
fn cmd_install(args: &[String]) {
    ...
    let name = &args[0];
    let mut store = ContentAddressedStore::new(sigmaos::klib::path::PathBuf::from("/var/lib/sigpkg/store"));
    let pkg = Package::new(
        name.clone(),
...
fn cmd_search(args: &[String]) {
    ...
    let name = &args[0];
    let store = ContentAddressedStore::new(sigmaos::klib::path::PathBuf::from("/var/lib/sigpkg/store"));
