...
// SigmaPkg — Sovereign Universal Package Manager CLI (`sigma-pkg`)
// Implements the `sigma-pkg` commands documented in docs/PACKAGE_MANAGEMENT.md,
...
fn usage() -> ! {
    eprintln!(
        "sigma-pkg — SigmaOS universal package manager\n\
         \n\
         USAGE:\n\
         \x20 sigma-pkg install [--fmt] <pkg|file>... Add package(s) or foreign (.deb/.rpm/PKGBUILD/.apk/.xbps/e.t.c.) to store\n\
         \x20 sigma-pkg convert <file>                Dry-run convert foreign package manifest & print metadata\n\
         \x20 sigma-pkg dispatch \"<foreign cmd>\"       Dispatch raw foreign PM command (apt, pacman, dnf, apk, pkg, emerge, nix, etc.)\n\
         \x20 sigma-pkg apt|dnf|pacman|apk|pkg|zypper|xbps|emerge|eopkg|nix|guix|pkgin|slackpkg <cmd> Foreign PM command alias\n\
         \x20 sigma-pkg debian|fedora|arch|alpine|freebsd|openbsd|netbsd|void|gentoo|opensuse <cmd> Distro PM command alias\n\
         \x20 sigma-pkg info|query <package>          Show detailed package metadata, sandboxing, and capabilities\n\
         \x20 sigma-pkg remove <package>              Remove a package from the store\n\
         \x20 sigma-pkg search <package>              Show a stored package's metadata\n\
         \x20 sigma-pkg status                        List stored packages and counts\n\
         \x20 sigma-pkg verify|audit <package>        Verify package checksum, signature, and security policy\n\
         \x20 sigma-pkg deps|tree <package>           Display dependency tree for a stored package\n\
         \x20 sigma-pkg triggers|hooks                Execute and report universal system triggers\n\
         \x20 sigma-pkg clean|paccache                Garbage-collect orphaned packages and clean store cache\n\
         \x20 sigma-pkg repo add <name> <url>         Register an apt-style repository\n\
         \x20 sigma-pkg repo list                     List registered repositories\n\
         \x20 sigma-pkg mirror best <repo>            Choose the best mirror for a repo\n\
         \x20 sigma-pkg snapshot <desc>               Create a pre-transaction snapshot\n\
         \x20 sigma-pkg rollback <generation>         Roll the store back to a snapshot\n\
         \x20 sigma-pkg update                        Check the repository for package updates\n\
...
fn cmd_dispatch(args: &[String]) {
    if args.is_empty() {
        eprintln!("sigma-pkg: dispatch requires a foreign command string");
        exit(2);
    }
    ...
    match dispatcher.dispatch_command(&full_cmd) {
        ...
        Err(err) => {
            eprintln!("sigma-pkg: dispatch error: {}", err);
            exit(1);
        }
    }
}
...
fn cmd_foreign_pm(pm_name: &str, args: &[String]) {
    ...
    match dispatcher.dispatch_command(&full_cmd) {
        Ok(action) => execute_dispatched_action(action),
        Err(err) => {
            eprintln!("sigma-pkg: foreign command error: {}", err);
            exit(1);
        }
    }
}
...
fn cmd_install(args: &[String]) {
    if args.is_empty() {
        eprintln!("sigma-pkg: install requires package name(s) or file path(s)");
        exit(2);
    }
    ...
    if targets.is_empty() {
        eprintln!("sigma-pkg: no package targets specified to install");
        exit(2);
    }
    ...
    for target in &targets {
        ...
        let (pkg, raw_bytes) = if path.exists() {
            let data = match fs::read(path) {
                Ok(d) => d,
                Err(e) => {
                    eprintln!("sigma-pkg: failed to read file '{}': {}", target, e);
                    exit(1);
                }
            };
        ...
        match store.add(pkg, &raw_bytes) {
            ...
            Err(err) => {
                eprintln!("sigma-pkg: failed to install {}: {:?}", name, err);
                exit(1);
            }
        }
    }
...
fn cmd_convert(args: &[String]) {
    if args.is_empty() {
        eprintln!("sigma-pkg: convert requires a package file path or manifest content");
        exit(2);
    }
    let target = &args[0];
    let path = Path::new(target);
    ...
    match simulator.simulate_install(fmt, &content) {
        ...
        Err(err) => {
            eprintln!("sigma-pkg: conversion dry-run failed: {}", err);
            exit(1);
        }
    }
}
...
fn cmd_remove(args: &[String]) {
    if args.is_empty() {
        eprintln!("sigma-pkg: remove requires a package name");
        exit(2);
    }
    let name = &args[0];
    let mut store = ContentAddressedStore::new("/var/lib/sigpkg/store".to_string());
    ...
    match store.remove(name) {
        ...
        Err(err) => {
            eprintln!("sigma-pkg: failed to remove {}: {:?}", name, err);
            exit(1);
        }
    }
}
...
fn cmd_search(args: &[String]) {
    if args.is_empty() {
        eprintln!("sigma-pkg: search requires a package name");
        exit(2);
    }
    let name = &args[0];
    ...
    match store.get(name) {
        ...
        None => {
            eprintln!("sigma-pkg: package '{}' not found in store", name);
            exit(1);
        }
    }
}
...
fn cmd_info(args: &[String]) {
    if args.is_empty() {
        eprintln!("sigma-pkg: info requires a package name");
        exit(2);
    }
    let name = &args[0];
    let store = ContentAddressedStore::new("/var/lib/sigpkg/store".to_string());
...
fn cmd_deps(args: &[String]) {
    if args.is_empty() {
        eprintln!("sigma-pkg: deps requires a package name");
        exit(2);
    }
    let name = &args[0];
    let store = ContentAddressedStore::new("/var/lib/sigpkg/store".to_string());
...
fn cmd_status(args: &[String]) {
    if !args.is_empty() {
        eprintln!("sigma-pkg: status takes no arguments");
        exit(2);
    }
    let store = ContentAddressedStore::new("/var/lib/sigpkg/store".to_string());
...
fn cmd_verify(args: &[String]) {
    if args.is_empty() {
        eprintln!("sigma-pkg: verify requires a package name");
        exit(2);
    }
    let name = &args[0];
    let store = ContentAddressedStore::new("/var/lib/sigpkg/store".to_string());
    let pkg: Package = match store.get(name) {
        Some(p) => (*p).clone(),
        None => {
            eprintln!("sigma-pkg: package '{}' not found", name);
            exit(1);
        }
    };
...
fn cmd_repo(args: &[String]) {
    ...
    match args.first().map(|s| s.as_str()) {
        Some("add") => {
            if args.len() < 3 {
                eprintln!("sigma-pkg: repo add requires <name> <url>");
                exit(2);
            }
        ...
        _ => {
            eprintln!("sigma-pkg: repo requires 'add' or 'list'");
            exit(2);
        }
    }
}
...
fn cmd_mirror(args: &[String]) {
    if args.len() != 2 || args[0] != "best" {
        eprintln!("sigma-pkg: mirror usage: sigma-pkg mirror best <repo>");
        exit(2);
    }
    let mut manager = RepositoryManager::new();
    ...
    match manager.select_best_mirror(&args[1]) {
        ...
        Err(err) => {
            eprintln!("sigma-pkg: no mirror available for {}: {}", args[1], err);
            exit(1);
        }
    }
}
...
fn cmd_rollback(args: &[String]) {
    if args.is_empty() {
        eprintln!("sigma-pkg: rollback requires a generation number");
        exit(2);
    }
    let generation: u32 = match args[0].parse() {
        Ok(g) => g,
        Err(_) => {
            eprintln!("sigma-pkg: invalid generation '{}'", args[0]);
            exit(2);
        }
    };
    let mut engine = SovereignPackageSnapshotRollbackEngine::new();
    match engine.rollback_to_snapshot(generation) {
        ...
        Err(err) => {
            eprintln!("sigma-pkg: rollback failed: {}", err);
            exit(1);
        }
    }
}
...
fn cmd_update(args: &[String]) {
    if !args.is_empty() {
        eprintln!("sigma-pkg: update takes no arguments");
        exit(2);
    }
    ...
    match daemon.sync_repository(payload, &sig) {
        ...
        sigmaos::sigpkg::SyncStatus::Failed { reason } => {
            eprintln!("sigma-pkg: update failed: {}", reason);
            exit(1);
        }
    }
}
...
fn cmd_daemon(args: &[String]) {
    if args.is_empty() {
        eprintln!("sigma-pkg: daemon requires 'sync', 'gc', or 'status'");
        exit(2);
    }
    let mut daemon = SigpkgDaemon::default();
    daemon.add_trusted_key("root-key");
