...
mod tests {
    ...
    #[test]
    fn test_mint4win_installer_engine() {
        let config = Mint4WinInstallationConfig::default_windows_c('C', "alice");
        let mut engine = Mint4WinInstallerEngine::new(config);

        assert!(engine.configure_windows_bcd_boot_entry().is_err()); // VHD not allocated yet

        let alloc_res = engine.allocate_loopback_disks().unwrap();
        assert!(alloc_res.contains("32768 MB"));

        let bcd_res = engine.configure_windows_bcd_boot_entry().unwrap();
        assert!(bcd_res.contains("Added Windows BCD boot entry"));
        assert!(engine.installed);

        let script = engine.generate_unattended_install_script();
        assert!(script.contains("alice"));
        assert!(script.contains("BcdBootmgr"));

        let uninstall_res = engine.uninstall_mint4win().unwrap();
        assert!(uninstall_res.contains("Successfully removed mint4win"));
        assert!(!engine.installed);
    }
}
