...
mod tests {
    ...
    fn test_fedora_dnf_resolver() {
        ...
        assert!(resolver.resolve_dependencies("kernel").is_ok());
    }

    #[test]
    fn test_fedora_koji_build_server() {
        let mut koji = KojiBuildServer::new();
        let task_id = koji.submit_build_task("coreutils", "9.3-1.fc39", "x86_64");
        assert_eq!(task_id, 1);
    ...
    fn test_fedora_bodhi_update_triage() {
        ...
        bodhi.submit_update("systemd-254.1-1.fc39");
        assert_eq!(bodhi.get_update_status("systemd-254.1-1.fc39"), Some(BodhiUpdateStatus::Pending));
    }
    ...
    fn test_fedora_dracut_initramfs() {
        ...
        dracut.add_dracut_module("base", 10);
        dracut.add_dracut_module("kernel-modules", 20);
        dracut.add_dracut_module("systemd", 30);

        let img = dracut.build_initramfs_image();
        assert!(img.len() > 0);
    }
    ...
    fn test_fedora_abrt_crash_daemon() {
        ...
        assert_eq!(report.process_name, "gnome-shell");
