...
mod tests {
    ...
    fn test_fedora_dnf_resolver() {
        ...
        resolver.register_rpm("kernel", vec!["glibc"]);
    ...
    fn test_fedora_koji_build_server() {
        ...
        let task_id = koji.submit_task("coreutils", "x86_64").unwrap();
        assert_eq!(task_id, 1);
    ...
    fn test_fedora_dracut_initramfs() {
        ...
        dracut.include_module("base", "cmdline", &["base_drv"]);
        dracut.include_module("kernel-modules", "pre-pivot", &["ext4"]);
        dracut.include_module("systemd", "init", &["systemd_drv"]);

        assert_eq!(dracut.modules.len(), 5);
    }
