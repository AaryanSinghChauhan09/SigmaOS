...
impl OssieInterpreter {
    ...
    pub fn evaluate_metric(
        ...
        for row in dataset {
            let dim_val = row.get_value(&dimension.source_field).unwrap_or("Unknown").to_string();
            let metric_val_str = row.get_value(&metric.source_field).ok_or("Metric source field not found in row")?;
            let metric_val: f64 = metric_val_str.parse().map_err(|_| "Failed to parse metric value as float")?;
...
mod tests {
    ...
    fn test_ossie_metric_and_dimension_creation() {
        let metric = OssieMetric::new("total_revenue", "Total Revenue", MetricAggregation::Sum, "revenue")
    ...
    fn test_ossie_metric_aggregation_interpreter() {
        let metric_rev = OssieMetric::new("total_revenue", "Total Revenue", MetricAggregation::Sum, "revenue");
        let metric_count = OssieMetric::new("active_users", "Active Users", MetricAggregation::CountDistinct, "user_id");
        let dimension = OssieDimension::new("country", "signup_country", "text");

        // Mock dataset representing business facts
        let dataset = vec![
            SemanticRow::new().with_field("signup_country", "US").with_field("revenue", "100.0").with_field("user_id", "1"),
            SemanticRow::new().with_field("signup_country", "US").with_field("revenue", "150.0").with_field("user_id", "2"),
            SemanticRow::new().with_field("signup_country", "US").with_field("revenue", "50.0").with_field("user_id", "1"), // Duplicate user_id 1
            SemanticRow::new().with_field("signup_country", "CA").with_field("revenue", "80.0").with_field("user_id", "3"),
        ];

        let interpreter = OssieInterpreter::new();

        // 1. Evaluate Sum(revenue) GroupBy Country
        let revenue_results = interpreter.evaluate_metric(&metric_rev, &dimension, &dataset).unwrap();
        ...
        let user_results = interpreter.evaluate_metric(&metric_count, &dimension, &dataset).unwrap();
