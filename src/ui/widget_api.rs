...
// =========================================================================
// LINUX & BSD INSPIRED DESKLET & DESKTOP APPLET ENGINE
// =========================================================================

#[derive(Debug, Clone, PartialEq)]
pub enum DeskletKind {
    SystemMonitor { cpu_pct: u8, mem_pct: u8 },
    Weather { location: String, temp_c: i8, condition: String },
    ClockCalendar { format: String, timezone: String },
    StickyNote { text: String, color_hex: String },
    NetworkTraffic { rx_kbps: u32, tx_kbps: u32 },
}

#[derive(Debug, Clone)]
pub struct DeskletPositionSpec {
    pub x: u32,
    pub y: u32,
    pub z_index: i32,
    pub opacity: f32,
...
impl Default for DeskletPositionSpec {
    fn default() -> Self {
        Self {
            x: 20,
            y: 20,
...
pub struct SovereignDesklet {
    pub id: u64,
    pub name: String,
    pub kind: DeskletKind,
    pub pos: DeskletPositionSpec,
    pub is_active: bool,
}

impl SovereignDesklet {
    pub fn new(id: u64, name: &str, kind: DeskletKind, pos: DeskletPositionSpec) -> Self {
        Self {
            id,
    ...
    pub fn render_content(&self) -> String {
        match &self.kind {
            DeskletKind::SystemMonitor { cpu_pct, mem_pct } => {
                alloc::format!("CPU: {}% | MEM: {}%", cpu_pct, mem_pct)
            }
            DeskletKind::Weather { location, temp_c, condition } => {
                alloc::format!("{}: {}°C ({})", location, temp_c, condition)
            }
            DeskletKind::ClockCalendar { format, timezone } => {
                alloc::format!("Time [{}] ({})", format, timezone)
            }
            DeskletKind::StickyNote { text, .. } => {
                alloc::format!("Note: {}", text)
            }
            DeskletKind::NetworkTraffic { rx_kbps, tx_kbps } => {
                alloc::format!("NET: ↓{} KB/s ↑{} KB/s", rx_kbps, tx_kbps)
            }
        }
    }
}

pub struct SovereignDeskletEngine {
    pub desklets: Vec<SovereignDesklet>,
    pub next_id: u64,
}

impl SovereignDeskletEngine {
    pub fn new() -> Self {
        Self {
            desklets: Vec::new(),
            next_id: 1,
    ...
    pub fn add_desklet(&mut self, name: &str, kind: DeskletKind, pos: DeskletPositionSpec) -> u64 {
        let id = self.next_id;
        self.next_id += 1;
        self.desklets.push(SovereignDesklet::new(id, name, kind, pos));
        id
    }

    pub fn remove_desklet(&mut self, id: u64) -> bool {
        let orig_len = self.desklets.len();
        self.desklets.retain(|d| d.id != id);
        self.desklets.len() < orig_len
    }

    pub fn reposition_desklet(&mut self, id: u64, x: u32, y: u32) -> bool {
        if let Some(desklet) = self.desklets.iter_mut().find(|d| d.id == id) {
            desklet.pos.x = x;
            desklet.pos.y = y;
            true
        } else {
    ...
    pub fn render_all_active_desklets(&self) -> Vec<(u64, String)> {
        self.desklets
            .iter()
            .filter(|d| d.is_active)
            .map(|d| (d.id, d.render_content()))
            .collect()
    }
}

impl Default for SovereignDeskletEngine {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    #[test]
    fn test_desklet_creation_and_positioning() {
        let pos = DeskletPositionSpec {
            x: 50,
            y: 100,
            z_index: 2,
            opacity: 0.95,
        ...
        let desklet = SovereignDesklet::new(
            1,
            "SysMon",
            DeskletKind::SystemMonitor { cpu_pct: 15, mem_pct: 40 },
            pos,
        );

        assert_eq!(desklet.id, 1);
        assert_eq!(desklet.name, "SysMon");
        assert_eq!(desklet.render_content(), "CPU: 15% | MEM: 40%");
    }

    #[test]
    fn test_desklet_engine_lifecycle_and_render() {
        let mut engine = SovereignDeskletEngine::new();

        let sys_id = engine.add_desklet(
            "SysMon",
            DeskletKind::SystemMonitor { cpu_pct: 20, mem_pct: 35 },
            DeskletPositionSpec::default(),
        );

        let weather_id = engine.add_desklet(
            "Weather",
            DeskletKind::Weather {
                location: String::from("New Delhi"),
                temp_c: 28,
        ...
        assert_eq!(sys_id, 1);
        assert_eq!(weather_id, 2);

        let rendered = engine.render_all_active_desklets();
        assert_eq!(rendered.len(), 2);
        assert!(rendered[1].1.contains("New Delhi: 28°C"));

        assert!(engine.reposition_desklet(sys_id, 300, 400));
        assert_eq!(engine.desklets[0].pos.x, 300);

        assert!(engine.remove_desklet(weather_id));
        assert_eq!(engine.render_all_active_desklets().len(), 1);
    }

    #[test]
    fn test_desklet_pin_and_z_index() {
        let pos = DeskletPositionSpec {
            x: 10,
            y: 20,
            z_index: 10,
        ...
        let desklet = SovereignDesklet::new(
            1,
            "NetTraffic",
            DeskletKind::NetworkTraffic { rx_kbps: 1024, tx_kbps: 512 },
            pos,
        );

        assert!(desklet.pos.pinned_to_desktop);
        assert_eq!(desklet.pos.z_index, 10);
        assert!(desklet.render_content().contains("NET: ↓1024 KB/s ↑512 KB/s"));
    }
}
