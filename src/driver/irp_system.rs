...
// ================= Linux-style I/O Elevator Scheduler =================

#[derive(Clone)]
pub struct IoElevatorRequest {
...
/// Linux-style disk head seek and starvation deadline optimizer
pub struct IoElevatorScheduler {
    pub queue: Vec<IoElevatorRequest>,
}

impl IoElevatorScheduler {
    pub fn new() -> Self {
        Self { queue: Vec::new() }
    }

    pub fn insert_request(&mut self, req: IoElevatorRequest) {
        self.queue.push(req);
        self.queue.sort_by(|a, b| {
            let res = a.sector_offset.cmp(&b.sector_offset);
            if res == std::cmp::Ordering::Equal {
                a.deadline_ms.cmp(&b.deadline_ms)
            } else {
                res
            }
        });
    ...
    pub fn pop_next_request(&mut self, current_time_ms: u64) -> Option<IoElevatorRequest> {
        if self.queue.is_empty() {
            return None;
        }
        let mut starved_idx = None;
        for (i, req) in self.queue.iter().enumerate() {
            if current_time_ms >= req.deadline_ms {
                starved_idx = Some(i);
                break;
            }
        }
...
// ================= iOS/macOS-style I/O QoS Throttler =================
...
#[derive(Clone)]
pub struct QosIrpWrapper {
    pub irp_id: u32,
    pub qos_class: IoQosClass,
}

/// iOS/macOS-style I/O Quality of Service (QoS) and interactive throttling
pub struct IoQosThrottler {
    pub is_high_load_active: bool,
    pub pending_requests: Vec<QosIrpWrapper>,
}

impl IoQosThrottler {
    pub fn new() -> Self {
        Self {
            is_high_load_active: false,
            pending_requests: Vec::new(),
        }
    }

    pub fn submit_irp(&mut self, irp_id: u32, qos: IoQosClass) {
        self.pending_requests.push(QosIrpWrapper { irp_id, qos_class: qos });
        self.pending_requests.sort_by(|a, b| b.qos_class.cmp(&a.qos_class));
    }

    pub fn process_next_batch(&mut self) -> Vec<u32> {
        let mut processed = Vec::new();
        if self.pending_requests.is_empty() {
            return processed;
        }

        if self.is_high_load_active {
            let mut remaining = Vec::new();
            for req in self.pending_requests.drain(..) {
                if req.qos_class >= IoQosClass::UserInitiated {
                    processed.push(req.irp_id);
                } else {
                    remaining.push(req);
                }
            }
...
// ================= FreeBSD GEOM Storage Filter Topology =================

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum GeomFilterType {
    CryptoXor { key: u8 },
    MirrorJournal,
}

/// FreeBSD-style GEOM layered block transaction mutation and filtering
pub struct GeomStorageLayer {
    pub filter: GeomFilterType,
}

impl GeomStorageLayer {
    pub fn new(filter: GeomFilterType) -> Self {
        Self { filter }
    }

    pub fn transform_buffer(&self, buffer: &mut [u8]) {
        match self.filter {
            GeomFilterType::CryptoXor { key } => {
                for byte in buffer.iter_mut() {
                    *byte ^= key;
                }
            }
            GeomFilterType::MirrorJournal => {
                if !buffer.is_empty() {
                    buffer[0] = 0xAA;
                }
            }
...
mod tests {
    ...
    #[test]
    fn test_io_elevator_sorting_and_deadline() {
        let mut scheduler = IoElevatorScheduler::new();
        scheduler.insert_request(IoElevatorRequest {
            sector_offset: 200,
            payload_bytes: 512,
            deadline_ms: 1000,
            is_write: false,
        ...
        scheduler.insert_request(IoElevatorRequest {
            sector_offset: 150,
            payload_bytes: 512,
            deadline_ms: 500, // Starving soon
            is_write: false,
        });

        // Normally, the next processed request is the one closest to 0 (lowest sector offset: 50)
        // If current time is 0 (no starvation yet)
        let r1 = scheduler.pop_next_request(0).unwrap();
        assert_eq!(r1.sector_offset, 50);

        // If current time is 600, request at sector 150 has expired its deadline (500), so it gets elevated!
        let r2 = scheduler.pop_next_request(600).unwrap();
        assert_eq!(r2.sector_offset, 150);

        let r3 = scheduler.pop_next_request(600).unwrap();
        assert_eq!(r3.sector_offset, 200);
    }

    #[test]
    fn test_io_qos_priority_throttling() {
        let mut throttler = IoQosThrottler::new();
        throttler.submit_irp(10, IoQosClass::Background);
        throttler.submit_irp(20, IoQosClass::UserInteractive);
        throttler.submit_irp(30, IoQosClass::UserInitiated);

        // Under normal load, everything is processed
        throttler.is_high_load_active = false;
        let mut normal_throttler = IoQosThrottler::new();
        normal_throttler.submit_irp(10, IoQosClass::Background);
        normal_throttler.submit_irp(20, IoQosClass::UserInteractive);
        normal_throttler.submit_irp(30, IoQosClass::UserInitiated);
        let processed_normal = normal_throttler.process_next_batch();
        assert_eq!(processed_normal.len(), 3);
        assert_eq!(processed_normal[0], 20); // UserInteractive first

        // Under high load, Background is throttled and delayed, only high priority passes
        throttler.is_high_load_active = true;
        let processed_high_load = throttler.process_next_batch();
        assert_eq!(processed_high_load.len(), 2);
        assert_eq!(processed_high_load[0], 20); // UserInteractive
        assert_eq!(processed_high_load[1], 30); // UserInitiated

        assert_eq!(throttler.pending_requests.len(), 1); // Background still pending
        assert_eq!(throttler.pending_requests[0].irp_id, 10);
    }

    #[test]
    fn test_geom_layered_filter_transformations() {
        let geom_xor = GeomStorageLayer::new(GeomFilterType::CryptoXor { key: 0x55 });
        let mut buffer = [1, 2, 3, 4];
        geom_xor.transform_buffer(&mut buffer);
        assert_eq!(buffer, [1 ^ 0x55, 2 ^ 0x55, 3 ^ 0x55, 4 ^ 0x55]);

        let geom_journal = GeomStorageLayer::new(GeomFilterType::MirrorJournal);
        let mut log_block = [0; 4];
        geom_journal.transform_buffer(&mut log_block);
        assert_eq!(log_block[0], 0xAA);
    }
}
