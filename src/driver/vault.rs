...
mod tests {
    ...
    fn test_driver_archive_vault() {
        let mut vault = DriverArchiveVault::new(0x42);
        vault.store_driver("ne2000_isa_nic", b"driver_binary_payload");
        let retrieved = vault.retrieve_driver("ne2000_isa_nic").unwrap();
        assert_eq!(retrieved.as_slice(), b"driver_binary_payload");
    }
}
