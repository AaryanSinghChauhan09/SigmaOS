...
impl UsbHidDriver {
    ...
    pub fn send_report(&mut self, report_type: HidReportType, _data: &[u8]) -> Result<(), HidError> {
        if !self.connected {
...
impl HidKeyboard {
    ...
    pub fn scancode_to_ascii(scancode: u8, shift: bool) -> Option<u8> {
        ...
        if idx >= HID_SCANCODE_TO_ASCII.len() { return None; }
        let ch = HID_SCANCODE_TO_ASCII[idx];
        if ch == 0 { return None; }
        if shift && ch.is_ascii_alphabetic() { Some(ch.to_ascii_uppercase()) } else { Some(ch) }
    }
...
impl PeripheralDevice for HidKeyboard {
    ...
    fn initialize(&mut self) -> Result<(), &'static str> {
        self.inner.connect().map_err(|_| "USB HID: Failed to connect")?;
        self.power_state = PowerState::On;
        Ok(())
    }

    fn read(&mut self, buffer: &mut [u8]) -> Result<usize, &'static str> {
        if self.power_state != PowerState::On { return Err("HID Keyboard is not powered on"); }
        let mut count = 0;
    ...
    fn write(&mut self, data: &[u8]) -> Result<usize, &'static str> {
        if let Some(&led_byte) = data.first() {
            self.inner.set_leds(led_byte).map_err(|_| "HID: LED set failed")?;
        }
        Ok(data.len().min(1))
    }
