...
impl<T> Vec<T> {
    ...
    unsafe fn grow(&mut self) {
        ...
        let new_layout = core::std::std::Layout::array::<T>(new_capacity).unwrap();
        let new_data = std::std::alloc(new_layout) as *mut T;
        if !new_data.is_null() {
            for i in 0..self.len { core::ptr::copy_nonoverlapping(self.data.add(i), new_data.add(i), 1); }
            if self.capacity > 0 {
                let old_layout = core::std::std::Layout::array::<T>(self.capacity).unwrap();
                std::std::dealloc(self.data as *mut u8, old_layout);
            }
            self.data = new_data;
