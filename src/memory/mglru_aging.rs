...
impl MglruPageAgingEngine {
    ...
    pub fn run_aging_scan(&mut self) {
        ...
        // Iterate through generations from youngest (3) down to 1 and demote unaccessed pages
        for gen in (1..MAX_MGLRU_GENERATIONS).rev() {
            let mut pfns_to_demote = Vec::new();
            for &pfn in &self.generations[gen] {
                if let Some(desc) = self.page_map.get_mut(&pfn) {
                    if (desc.access_flags & PAGE_ACCESSED_BIT) == 0 {
                        pfns_to_demote.push(pfn);
                    } else {
            ...
            // Perform demotions down to gen - 1
            for pfn in pfns_to_demote {
...
mod tests {
    ...
    fn test_mglru_page_tracking_and_access_promotion() {
        ...
        // Run aging scan twice without access -> Pages demoted from Gen 3 -> Gen 2 -> Gen 1 -> Gen 0
