...
impl TCPConnection for SimpleSocket {
    ...
    fn connect(&mut self, remote_port: Port) -> Result<(), NetworkError> {
        self.remote_port
            .store(remote_port as u32, Ordering::SeqCst);
        self.state
            .store(TCPState::SynSent as u32, Ordering::SeqCst);
        self.state
    ...
    fn accept(&mut self) -> Result<SocketID, NetworkError> {
        if self.state.load(Ordering::SeqCst) != TCPState::Listen as u32 {
...
impl NetworkStack for SimpleNetworkStack {
    ...
    fn get_socket(&self, id: SocketID) -> Option<&dyn Socket> {
        for socket_option in &self.sockets {
            if socket_option.id() == id {
                return Some(socket_option.as_ref());
            }
        }
        None
    }
...
mod tests {
    ...
    fn test_socket_creation() {
        let socket = SimpleSocket::new(1, Protocol::Tcp, 8080);
