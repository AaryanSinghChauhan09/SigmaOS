...
mod tests {
    ...
    #[test]
    fn test_process_tree_hierarchy_walk_and_reparenting() {
        let mut tree = ProcessTreeHierarchyEngine::new();

        // 1. Add subreaper daemon (PID 10) under init (PID 1)
        tree.add_process(10, 1, 10, 10, "subreaper_daemon");
        tree.set_subreaper(10, true).unwrap();

        // 2. Add worker (PID 20) under subreaper, and orphan (PID 30) under worker
        tree.add_process(20, 10, 100, 10, "worker_parent");
        tree.add_process(30, 20, 100, 10, "orphan_child");

        let pg_pids = tree.get_process_group_pids(100);
        assert_eq!(pg_pids.len(), 2);
        assert!(pg_pids.contains(&20));
        assert!(pg_pids.contains(&30));

        // 3. Worker dies -> Orphan (PID 30) should be re-parented to closest subreaper (PID 10), not init (PID 1)
        tree.reparent_orphans(20);

        let orphan_node = tree.nodes.get(&30).unwrap();
        assert_eq!(orphan_node.ppid, 10);

        let subreaper_node = tree.nodes.get(&10).unwrap();
        assert!(subreaper_node.children_pids.contains(&30));
    }
}
...
// --- Linux & BSD Process Hierarchy Tree & Subreaper Engine ---

#[derive(Debug, Clone)]
pub struct ProcessTreeHierarchyNode {
    pub pid: usize,
    pub ppid: usize,
    pub pgid: usize,
    pub sid: usize,
    ...
    pub children_pids: Vec<usize>,
}

pub struct ProcessTreeHierarchyEngine {
    pub nodes: BTreeMap<usize, ProcessTreeHierarchyNode>,
}

impl ProcessTreeHierarchyEngine {
    pub fn new() -> Self {
        let mut engine = Self {
            nodes: BTreeMap::new(),
        };
        // Root init process (PID 1) is default top-level ancestor & reaper
        engine.nodes.insert(
            1,
            ProcessTreeHierarchyNode {
                pid: 1,
                ppid: 0,
                pgid: 1,
                sid: 1,
    ...
    pub fn add_process(&mut self, pid: usize, ppid: usize, pgid: usize, sid: usize, name: &str) {
        let node = ProcessTreeHierarchyNode {
            pid,
            ppid,
            pgid,
            sid,
        ...
        if let Some(parent) = self.nodes.get_mut(&ppid) {
            if !parent.children_pids.contains(&pid) {
                parent.children_pids.push(pid);
            }
        }
    }

    pub fn set_subreaper(&mut self, pid: usize, is_subreaper: bool) -> Result<(), &'static str> {
        let node = self.nodes.get_mut(&pid).ok_or("Process not found")?;
        node.is_subreaper = is_subreaper;
        Ok(())
    }

    pub fn reparent_orphans(&mut self, dying_pid: usize) {
        let orphan_pids = if let Some(dying_node) = self.nodes.get(&dying_pid) {
            dying_node.children_pids.clone()
        } else {
            return;
        };

        // Find closest ancestor that is a subreaper or fallback to init (PID 1)
        let mut new_parent_pid = 1;
        if let Some(dying_node) = self.nodes.get(&dying_pid) {
            let mut curr = dying_node.ppid;
            while curr > 1 {
                if let Some(ancestor) = self.nodes.get(&curr) {
        ...
        for child_pid in orphan_pids {
            if let Some(child_node) = self.nodes.get_mut(&child_pid) {
                child_node.ppid = new_parent_pid;
            }
            if let Some(new_parent) = self.nodes.get_mut(&new_parent_pid) {
                if !new_parent.children_pids.contains(&child_pid) {
                    new_parent.children_pids.push(child_pid);
                }
            }
        }

        self.nodes.remove(&dying_pid);
    }

    pub fn get_process_group_pids(&self, pgid: usize) -> Vec<usize> {
        self.nodes
            .values()
            .filter(|node| node.pgid == pgid)
            .map(|node| node.pid)
            .collect()
    }
}

impl Default for ProcessTreeHierarchyEngine {
    fn default() -> Self {
        Self::new()
    }
}
