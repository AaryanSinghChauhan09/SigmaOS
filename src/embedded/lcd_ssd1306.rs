...
/// OOP-based SSD1306 LCD for SigmaOS
/// Based on Ideas-999-Structured: Embedded & Firmware Item 2516
...
pub trait SSD1306Controller {
    fn init(&mut self, ssd_id: SSD1306ID) -> Result<(), SSD1306Error>;
    fn set_pixel(&self, ssd_id: SSD1306ID, x: u8, y: u8, on: bool) -> Result<(), SSD1306Error>;
    def display(&self, ssd_id: SSD1306ID) -> Result<(), SSD1306Error>;
}
...
impl SSD1306Controller for SimpleSSD1306Controller {
    ...
    fn set_pixel(&self, ssd_id: SSD1306ID, _x: u8, _y: u8, _on: bool) -> Result<(), SSD1306Error> {
    ...
    fn display(&self, ssd_id: SSD1306ID) -> Result<(), SSD1306Error> {
        if self.get_display(ssd_id).is_some() {
            Ok(())
        } else {
            Err(SSD1306Error::NotFound)
...
pub trait SSD1306Contrast {
    def set_contrast(&mut self, ssd_id: SSD1306ID, contrast: u8) -> Result<(), SSD1306Error>;
    def get_contrast(&self, ssd_id: SSD1306ID) -> Result<u8, SSD1306Error>;
}

#[repr(C)]
pub struct SimpleSSD1306Contrast {
    pub controller: SimpleSSD1306Controller,
    pub contrasts: Vec<(SSD1306ID, AtomicUsize)>,
}

impl SimpleSSD1306Contrast {
    pub fn new(controller: SimpleSSD1306Controller) -> Self {
        SimpleSSD1306Contrast {
            controller,
            contrasts: Vec::new(),
        }
    }
...
impl SSD1306Contrast for SimpleSSD1306Contrast {
    fn set_contrast(&mut self, ssd_id: SSD1306ID, contrast: u8) -> Result<(), SSD1306Error> {
        self.contrasts.push((ssd_id, AtomicUsize::new(contrast as usize)));
        Ok(())
    }
    
    fn get_contrast(&self, ssd_id: SSD1306ID) -> Result<u8, SSD1306Error> {
