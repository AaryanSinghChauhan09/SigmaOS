...
/// Based on Ideas-999-Structured: Embedded & Firmware Item 4106
...
pub trait PMController {
    fn init(&mut self, sensor_id: PMID) -> Result<(), PMError>;
    fn read_pm25(&self, sensor_id: PMID) -> Result<u16, PMError>;
    def read_pm10(&self, sensor_id: PMID) -> Result<u16, PMError>;
}
...
impl PMController for SimplePMController {
    ...
    fn read_pm10(&self, sensor_id: PMID) -> Result<u16, PMError> {
        if self.get_sensor(sensor_id).is_some() {
            Ok(0)
        } else {
            Err(PMError::NotFound)
    ...
    fn get_sensor(&self, id: PMID) -> Option<&dyn PMSensor> {
...
pub trait PM1 {
    def read_pm1(&self, sensor_id: PMID) -> Result<u16, PMError>;
}

#[repr(C)]
pub struct SimplePM1 {
    pub controller: SimplePMController,
}

impl SimplePM1 {
    pub fn new(controller: SimplePMController) -> Self {
        SimplePM1 { controller }
    }
}

impl PM1 for SimplePM1 {
    fn read_pm1(&self, sensor_id: PMID) -> Result<u16, PMError> {
