...
/// OOP-based I2S Sound for SigmaOS
/// Based on Ideas-999-Structured: Embedded & Firmware Item 3396
...
impl I2SSound for SimpleI2SSound {
...
pub trait I2SController {
    ...
    fn read(&self, i2s_id: I2SID, buffer: &mut [i16]) -> Result<usize, I2SError>;
    def write(&self, i2s_id: I2SID, data: &[i16]) -> Result<usize, I2SError>;
}

#[repr(C)]
pub struct SimpleI2SController {
    pub devices: Vec<Option<Box<dyn I2SSound>>>,
...
impl I2SController for SimpleI2SController {
    ...
    fn write(&self, i2s_id: I2SID, data: &[i16]) -> Result<usize, I2SError> {
        if self.get_device(i2s_id).is_some() {
            Ok(data.len())
        } else {
            Err(I2SError::NotFound)
    ...
    fn get_device(&self, id: I2SID) -> Option<&dyn I2SSound> {
...
pub trait I2SConfig {
    ...
    def set_bits(&mut self, i2s_id: I2SID, bits: u8) -> Result<(), I2SError>;
}

#[repr(C)]
pub struct SimpleI2SConfig {
    pub controller: SimpleI2SController,
    pub rates: Vec<(I2SID, AtomicUsize)>,
}

impl SimpleI2SConfig {
...
impl I2SConfig for SimpleI2SConfig {
    fn set_sample_rate(&mut self, i2s_id: I2SID, rate: u32) -> Result<(), I2SError> {
        self.rates.push((i2s_id, AtomicUsize::new(rate as usize)));
        Ok(())
    }
    
    fn set_bits(&mut self, _i2s_id: I2SID, _bits: u8) -> Result<(), I2SError> {
        Ok(())
    }
}
