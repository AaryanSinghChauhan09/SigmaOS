...
mod tests_tss {
    ...
    fn test_sovereign_ring3_user_mode_tss_engine() {
        ...
        let updated_rsp0 = engine.tss.rsp0;
        assert_eq!(updated_rsp0, 0xFFFF_8000_0010_8000);
    ...
    fn test_gdt_segment_descriptors() {
        ...
        let kc_access = kernel_code.access_byte;
        assert_eq!(kc_access, 0x9A);

        let user_code = GdtSegmentDescriptor::new_code_segment(PrivilegeRing::Ring3Userland);
        let uc_access = user_code.access_byte;
        assert_eq!(uc_access, 0xFA);

        let kernel_data = GdtSegmentDescriptor::new_data_segment(PrivilegeRing::Ring0Kernel);
        let kd_access = kernel_data.access_byte;
        assert_eq!(kd_access, 0x92);

        let user_data = GdtSegmentDescriptor::new_data_segment(PrivilegeRing::Ring3Userland);
        let ud_access = user_data.access_byte;
        assert_eq!(ud_access, 0xF2);
