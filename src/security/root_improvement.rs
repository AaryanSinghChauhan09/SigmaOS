...
mod tests {
    ...
    fn test_doas_last_match_rules() {
        ...
        let res1 = engine.evaluate("alice", &["wheel"], "root", "shutdown");
        assert!(!res1.permitted);

        let res2 = engine.evaluate("alice", &["wheel"], "root", "reboot");
    ...
    fn test_bsd_securelevel_enforcement() {
        let guard = BsdSecurelevelController::new(BsdSecurelevel::Insecure);
        assert!(guard.check_module_loading_allowed());
        assert!(guard.check_raw_disk_write_allowed());

        assert!(guard.raise_level(BsdSecurelevel::Secure).is_ok());
        assert!(!guard.check_module_loading_allowed());
        assert!(!guard.check_raw_disk_write_allowed());

        assert!(guard.raise_level(BsdSecurelevel::HighlySecure).is_ok());

        assert!(guard.raise_level(BsdSecurelevel::Insecure).is_err());
    }
