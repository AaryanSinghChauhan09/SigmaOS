...
pub enum UniversalPackageFormat {
    ...
    SlackwareTxz,     // .tgz / .txz (slackware pkgtools)
    NixStorePkg,      // .nixpkg / .nix (Nix store expression)
    GuixPkg,          // .scm / .guix (GNU Guix package)
    NetBsdPkgsrc,     // .tgz / .tgz (NetBSD pkgsrc)
    OpenBsdPkg,       // .tgz (OpenBSD pkg_add)
    SolusEopkg,       // .eopkg (Solus eopkg)
    PardusPisi,       // .pisi (Pardus / Solus PiSi)
    SlaxLzm,          // .lzm (Slax Linux module)
    PuppyPet,         // .pet / .pup (Puppy Linux package)
    DeepinSuperdeb,   // .superdeb (Deepin Superdeb)
}
...
impl UniversalPackageImporter {
    pub fn autodetect_format(filename: &str) -> Option<UniversalPackageFormat> {
        ...
        if name.ends_with(".superdeb") {
            Some(UniversalPackageFormat::DeepinSuperdeb)
            ...
            Some(UniversalPackageFormat::PardusPisi)
        } else if name.ends_with(".lzm") {
            Some(UniversalPackageFormat::SlaxLzm)
        } else if name.ends_with(".pet") || name.ends_with(".pup") {
            Some(UniversalPackageFormat::PuppyPet)
        } else if name.ends_with(".nixpkg") || name.ends_with(".nix") {
            Some(UniversalPackageFormat::NixStorePkg)
        } else if name.ends_with(".guix") || name.ends_with(".scm") {
            Some(UniversalPackageFormat::GuixPkg)
        } else if name.ends_with(".pkgsrc.tgz") {
            Some(UniversalPackageFormat::NetBsdPkgsrc)
        } else if name.ends_with(".obsd.tgz") {
            Some(UniversalPackageFormat::OpenBsdPkg)
        } else if name.ends_with(".slack.txz") || name.ends_with(".slack.tgz") {
            Some(UniversalPackageFormat::SlackwareTxz)
        } else if name.ends_with(".txz") || name.ends_with(".pkg") {
...
mod tests {
    ...
    fn test_universal_package_format_importer() {
        ...
        let fmt_slack = UniversalPackageImporter::autodetect_format("bash-5.1.slack.txz");
        assert_eq!(fmt_slack, Some(UniversalPackageFormat::SlackwareTxz));

        let fmt_nix = UniversalPackageImporter::autodetect_format("git.nixpkg");
        assert_eq!(fmt_nix, Some(UniversalPackageFormat::NixStorePkg));

        let fmt_super = UniversalPackageImporter::autodetect_format("vscode.superdeb");
        assert_eq!(fmt_super, Some(UniversalPackageFormat::DeepinSuperdeb));
