...
mod tests {
    ...
    fn test_package_pinning_rules() {
        ...
        let p2 = pin_engine.get_pin_priority("unknown-pkg");
    ...
    fn test_mirror_sync_ranking_and_failover() {
        ...
        if let Some(m) = sync_engine.mirrors.iter_mut().find(|m| m.url == "https://mirror1.sigmaos.org") {
            m.active = false;
        }
    ...
    fn test_package_transaction_rollback() {
        ...
        let tx_id = journal.log_transaction("INSTALL", "htop", "3.2.0", 1700000000);
        let _tx_id2 = journal.log_transaction("UPGRADE", "bash", "5.2", 1700000010);
