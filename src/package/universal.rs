...
pub enum PackageFormat {
    ...
    Gpkg,       // Gentoo Portage v3 binary package (.gpkg)
    Xpak,       // Gentoo Portage legacy binary package (.xpak / .tbz2)
    Flatpakref, // Flatpak single-click bundle ref (.flatpakref / .flatpakrepo)
    Ucl,        // FreeBSD pkg v2 UCL manifest (.ucl)
    ApkV3,      // Alpine Linux APK v3 package (.apkv3)
    Wheel,      // Python Wheel package (.whl)
    Crate,      // Rust Cargo crate (.crate)
    Gem,        // RubyGem package (.gem)
    Nupkg,      // .NET NuGet package (.nupkg)
    Vcpkg,      // C++ Vcpkg manifest (.vcpkg)
    NarInfo,    // Nix/Guix NAR metadata (.narinfo)
    Spack,      // HPC Spack spec (.spack)
    Conan,      // Conan package (.conan)
...
impl PackageFormat {
    pub fn from_filename(filename: &str) -> Option<Self> {
        ...
        } else if name.ends_with(".slackbuild") || name.ends_with(".tlz") || name.ends_with(".tbz")
        {
            ...
            Some(PackageFormat::Gpkg)
        } else if normalized.ends_with(".xpak") {
            Some(PackageFormat::Xpak)
        } else if normalized.ends_with(".flatpakref") || normalized.ends_with(".flatpakrepo") {
            Some(PackageFormat::Flatpakref)
        } else if normalized.ends_with(".ucl") {
            Some(PackageFormat::Ucl)
        } else if normalized.ends_with(".apkv3") {
            Some(PackageFormat::ApkV3)
...
impl_generic_install_strategy!(GpkgInstallStrategy);
impl_generic_install_strategy!(XpakInstallStrategy);
impl_generic_install_strategy!(FlatpakrefInstallStrategy);
impl_generic_install_strategy!(UclInstallStrategy);
impl_generic_install_strategy!(ApkV3InstallStrategy);
...
impl_generic_metadata_adapter!(GpkgMetadataAdapter, Gpkg);
impl_generic_metadata_adapter!(XpakMetadataAdapter, Xpak);
impl_generic_metadata_adapter!(FlatpakrefMetadataAdapter, Flatpakref);
impl_generic_metadata_adapter!(UclMetadataAdapter, Ucl);
impl_generic_metadata_adapter!(ApkV3MetadataAdapter, ApkV3);
...
impl<T: PackageCapability> PackageCapability for HardwareOptimizationDecorator<T> {
    ...
    fn profile_performance(&self) {
        println!(
            "HardwareOptimizationDecorator: Microarch={}, SIMD={:?}",
...
impl<T: PackageCapability> PackageCapability for ResourceLimitDecorator<T> {
    ...
    fn enforce_sandbox(&self) -> Result<(), PackageError> {
        println!(
            "ResourceLimitDecorator: Memory limit {}MB, CPU quota {}%",
...
impl PackageFactory {
    pub fn get_strategy(format: PackageFormat) -> Box<dyn InstallStrategy> {
        match format {
            ...
            PackageFormat::Gpkg => Box::new(GpkgInstallStrategy),
            PackageFormat::Xpak => Box::new(XpakInstallStrategy),
            PackageFormat::Flatpakref => Box::new(FlatpakrefInstallStrategy),
            PackageFormat::Ucl => Box::new(UclInstallStrategy),
            PackageFormat::ApkV3 => Box::new(ApkV3InstallStrategy),
    ...
    pub fn get_adapter(format: PackageFormat) -> Box<dyn PackageMetadataAdapter> {
        match format {
            ...
            PackageFormat::Gpkg => Box::new(GpkgMetadataAdapter),
            PackageFormat::Xpak => Box::new(XpakMetadataAdapter),
            PackageFormat::Flatpakref => Box::new(FlatpakrefMetadataAdapter),
            PackageFormat::Ucl => Box::new(UclMetadataAdapter),
            PackageFormat::ApkV3 => Box::new(ApkV3MetadataAdapter),
