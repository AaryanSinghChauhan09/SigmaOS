...
mod tests {
    ...
    #[test]
    fn test_sovereign_rollback_engine() {
        let mut engine = SovereignRollbackEngine::new(RollbackBackendMechanism::OpenSuseSnapper);
        let pkgs = vec!["curl".to_string(), "nginx".to_string()];
        let snap_id = engine.create_distro_snapshot(pkgs.clone(), "Pre-update checkpoint");
        assert_eq!(snap_id, 1);
