...
pub enum PackageFormat {
    ...
    OpenBsdPkg, // OpenBSD Package
    Ipk,        // OpenWrt IPK
    Opkg,       // Yocto OPKG
    SolarisIps, // Solaris IPS
    GuixNar,    // Nix/Guix NAR
    Spack,      // Spack HPC package
    Conan,      // Conan C/C++ package
    Wheel,      // Python Wheel
    Crate,      // Rust Crate
    Gem,        // Ruby Gem
    Nupkg,      // .NET NuGet package
    Vcpkg,      // Microsoft Vcpkg
    NarInfo,    // Nix/Guix NarInfo
}
...
impl<T: PackageCapability> PackageCapability for HardwareOptimizationDecorator<T> {
    ...
    fn profile_performance(&self) {
        println!(
            "HardwareOptimizationDecorator: Microarch '{}' with SIMD {:?}",
...
impl<T: PackageCapability> PackageCapability for ResourceLimitDecorator<T> {
    ...
    fn enforce_sandbox(&self) -> Result<(), PackageError> {
        println!(
            "ResourceLimitDecorator: Cgroup limits set (max_mem: {}, cpu_quota: {}%)",
...
impl<T: PackageCapability> PackageCapability for PqcSignedDecorator<T> {
    ...
    fn enforce_sandbox(&self) -> Result<(), PackageError> {
        if self.dilithium_signature.starts_with("dilithium-5-valid") {
            println!("PqcSignedDecorator: Dilithium signature valid.");
            self.decorated.enforce_sandbox()
        } else {
            Err(PackageError::InstallationFailed("PQC Signature invalid".to_string()))
        }
    }
    fn restrict_network(&self) -> Result<(), PackageError> {
        self.decorated.restrict_network()
