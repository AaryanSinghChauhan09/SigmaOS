...
impl GentooPortageMaskEngine {
    pub fn new(arch_keyword: &str) -> Self {
        Self {
            arch_keyword: arch_keyword.to_string(),
            ebuilds: Vec::new(),
            hard_masks: Vec::new(),
        }
    }

    pub fn register_ebuild(&mut self, category_package: &str, version: &str, keywords: &[&str], is_masked: bool) {
        self.ebuilds.push(EbuildRecord {
            category_package: category_package.to_string(),
            version: version.to_string(),
    ...
    pub fn add_hard_mask(&mut self, category_package: &str) {
        if !self.hard_masks.contains(&category_package.to_string()) {
            self.hard_masks.push(category_package.to_string());
        }
    }

    pub fn evaluate_installability(&self, category_package: &str, version: &str, allow_testing_keywords: bool) -> Result<bool, &'static str> {
        if self.hard_masks.contains(&category_package.to_string()) {
            return Err("Package is hard masked in package.mask");
        }

        let ebuild = self.ebuilds.iter().find(|e| e.category_package == category_package && e.version == version);
        if let Some(record) = ebuild {
            if record.is_masked {
                return Err("Package ebuild has explicit mask flag set");
            }

            let stable_keyword = self.arch_keyword.clone();
            let testing_keyword = format!("~{}", self.arch_keyword);

            if record.keywords.contains(&stable_keyword) {
                Ok(true)
            } else if record.keywords.contains(&testing_keyword) {
                if allow_testing_keywords {
                    Ok(true)
                } else {
                    Err("Package requires testing keyword (~arch) accept_keywords entry")
                }
            } else {
                Err("Package is missing keyword for host architecture")
            }
        } else {
            Err("No matching ebuild specification found")
        }
    }
}
