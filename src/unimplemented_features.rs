...
mod tests {
    ...
    fn test_polymorphic_baremetal_peripheral_blueprint() {
        let legacy = LegacyController::new(0x3F8);
        let modern = ModernController::new(0xFE00_0000);

        assert_eq!(legacy.base_port, 0x3F8);
        assert_eq!(modern.mmio_base_addr, 0xFE00_0000);

        let mut mgr = BareMetalPeripheralManager::new();
        assert!(mgr.register_device(alloc::boxed::Box::new(legacy)).is_ok());
        assert!(mgr.register_device(alloc::boxed::Box::new(modern)).is_ok());
        assert_eq!(mgr.registry.len(), 2);
    }

    #[test]
    fn test_zero_allocation_udf_bytecode_vm() {
        let mut dev = LegacyController::new(0x3F8);
        dev.initialize().unwrap();
    ...
    fn test_sigmaos_component_inspection_suite() {
        ...
        let mut dev = LegacyController::new(0x200);
        dev.initialize().unwrap();
        let mut vm = UdfVm::new(0, 1024);
        let code = [
