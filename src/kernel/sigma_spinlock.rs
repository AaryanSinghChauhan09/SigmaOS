...
/// Priority Inheritance Mutex to prevent unbounded priority inversion in RT kernel tasks
#[derive(Debug)]
pub struct PriorityInheritanceMutex<T> {
    lock: core::sync::atomic::AtomicBool,
    owner_pid: core::sync::atomic::AtomicU64,
    owner_original_priority: core::sync::atomic::AtomicU32,
    current_boosted_priority: core::sync::atomic::AtomicU32,
...
impl<T> PriorityInheritanceMutex<T> {
    pub fn new(val: T) -> Self {
        Self {
            ...
            owner_pid: core::sync::atomic::AtomicU64::new(0),
            owner_original_priority: core::sync::atomic::AtomicU32::new(0),
            current_boosted_priority: core::sync::atomic::AtomicU32::new(0),
            value: std::cell::UnsafeCell::new(val),
        }
    }

    pub fn lock(&self, calling_pid: u64, calling_priority: u32) -> &mut T {
        ...
        while self.lock.compare_exchange(
            ...
            // Priority Inheritance boost check
            let owner_prio = self.current_boosted_priority.load(Ordering::Relaxed);
            if calling_priority > owner_prio {
                self.current_boosted_priority.store(calling_priority, Ordering::Relaxed);
            }
            core::hint::spin_loop();
        }

        self.owner_pid.store(calling_pid, Ordering::Relaxed);
        self.owner_original_priority.store(calling_priority, Ordering::Relaxed);
        self.current_boosted_priority.store(calling_priority, Ordering::Relaxed);
    ...
    pub fn unlock(&self) {
        ...
        self.current_boosted_priority.store(0, Ordering::Relaxed);
...
mod tests_pi {
    ...
    #[test]
    fn test_pi_mutex_basic() {
        let pi = PriorityInheritanceMutex::new(100u32);
        let val = pi.lock(10, 5);
        *val = 200;
        pi.unlock();

        let val2 = pi.lock(11, 10);
        assert_eq!(*val2, 200);
        pi.unlock();
    }
}
