...
impl TcpSegment {
    pub fn syn(src: u16, dst: u16, seq: u32) -> Self {
        TcpSegment { src_port: src, dst_port: dst, seq, ack: 0, flags: flags::SYN, window: 65535, checksum: 0, urgent_ptr: 0, payload: Vec::new() }
    }
    pub fn syn_ack(src: u16, dst: u16, seq: u32, ack: u32) -> Self {
        TcpSegment { src_port: src, dst_port: dst, seq, ack, flags: flags::SYN | flags::ACK, window: 65535, checksum: 0, urgent_ptr: 0, payload: Vec::new() }
    }
    pub fn ack(src: u16, dst: u16, seq: u32, ack: u32) -> Self {
        TcpSegment { src_port: src, dst_port: dst, seq, ack, flags: flags::ACK, window: 65535, checksum: 0, urgent_ptr: 0, payload: Vec::new() }
    }
    pub fn fin_ack(src: u16, dst: u16, seq: u32, ack: u32) -> Self {
        TcpSegment { src_port: src, dst_port: dst, seq, ack, flags: flags::FIN | flags::ACK, window: 65535, checksum: 0, urgent_ptr: 0, payload: Vec::new() }
    }
    pub fn data(src: u16, dst: u16, seq: u32, ack: u32, data: Vec<u8>) -> Self {
        TcpSegment { src_port: src, dst_port: dst, seq, ack, flags: flags::PSH | flags::ACK, window: 65535, checksum: 0, urgent_ptr: 0, payload: data }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CongestionAlgorithm { Reno, Cubic, Bbr }
...
impl CongestionControl {
    pub fn new(algo: CongestionAlgorithm) -> Self {
        CongestionControl { algorithm: algo, cwnd: 10 * 1460, ssthresh: 65536, rtt_ms: 100, rto_ms: 200, in_slow_start: true }
    }
...
impl TcpConnection {
    ...
    pub fn accept_syn(&mut self, seg: &TcpSegment) -> Option<TcpSegment> {
        if self.state != TcpState::Listen { return None; }
        if !seg.has_flag(flags::SYN) { return None; }
    ...
    pub fn process_syn_ack(&mut self, seg: &TcpSegment) -> Option<TcpSegment> {
        if self.state != TcpState::SynSent { return None; }
        if !seg.has_flag(flags::SYN) || !seg.has_flag(flags::ACK) { return None; }
        self.snd_seq += 1;
        self.rcv_seq = seg.seq + 1;
        self.state = TcpState::Established;
        let ack = TcpSegment::ack(self.local_port, self.remote_port, self.snd_seq, self.rcv_seq);
        self.segments_rx.fetch_add(1, Ordering::Relaxed);
        self.segments_tx.fetch_add(1, Ordering::Relaxed);
        Some(ack)
    }
    ...
    pub fn send(&mut self, data: &[u8]) -> Option<TcpSegment> {
        ...
        let to_send = data.len().min(self.congestion.cwnd as usize).min(self.snd_wnd as usize);
        let seg = TcpSegment::data(self.local_port, self.remote_port, self.snd_seq, self.rcv_seq, data[..to_send].to_vec());
    ...
    pub fn receive(&mut self, seg: &TcpSegment) {
        if self.state == TcpState::Established && !seg.payload.is_empty() {
            self.bytes_rx.fetch_add(seg.payload.len(), Ordering::Relaxed);
            self.segments_rx.fetch_add(1, Ordering::Relaxed);
            for &b in &seg.payload { self.rcv_buf.push_back(b); }
            self.rcv_seq = self.rcv_seq.wrapping_add(seg.payload.len() as u32);
        }
    }

    pub fn read(&mut self, n: usize) -> Vec<u8> { self.rcv_buf.drain(..n.min(self.rcv_buf.len())).collect() }

    /// Initiate close — send FIN
    pub fn close(&mut self) -> Option<TcpSegment> {
        if self.state != TcpState::Established { return None; }
        self.state = TcpState::FinWait1;
        let seg = TcpSegment::fin_ack(self.local_port, self.remote_port, self.snd_seq, self.rcv_seq);
        self.segments_tx.fetch_add(1, Ordering::Relaxed);
        Some(seg)
    }

    pub fn segments_tx(&self) -> usize { self.segments_tx.load(Ordering::Relaxed) }
    pub fn segments_rx(&self) -> usize { self.segments_rx.load(Ordering::Relaxed) }
