...
pub enum NfHookpoint { PreRouting, Input, Forward, Output, PostRouting }

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum NfVerdict { Accept, Drop, Reject, Queue }

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum NfProtocol { Tcp = 6, Udp = 17, Icmp = 1, Any = 0 }
...
impl NetPacket {
    pub fn tcp(src: [u8; 4], dst: [u8; 4], src_port: u16, dst_port: u16) -> Self {
        NetPacket { src_ip: src, dst_ip: dst, src_port, dst_port, protocol: NfProtocol::Tcp, conntrack_state: ConntrackState::New, payload_len: 0 }
    }
}
...
impl NfRule {
    pub fn matches(&self, pkt: &NetPacket) -> bool {
        if let Some(src) = self.src_ip { if pkt.src_ip != src { return false; } }
        if let Some(dst) = self.dst_ip { if pkt.dst_ip != dst { return false; } }
        if let Some(port) = self.dst_port { if pkt.dst_port != port { return false; } }
        if self.protocol != NfProtocol::Any && pkt.protocol != self.protocol { return false; }
        if let Some(ct) = self.conntrack_state { if pkt.conntrack_state != ct { return false; } }
        true
    }
}
...
impl NetfilterTable {
    pub fn new_permissive() -> Self {
        NetfilterTable { rules: Vec::new(), default_policy: NfVerdict::Accept, packets_accepted: AtomicUsize::new(0), packets_dropped: AtomicUsize::new(0) }
    }

    pub fn new_restrictive() -> Self {
        NetfilterTable { rules: Vec::new(), default_policy: NfVerdict::Drop, packets_accepted: AtomicUsize::new(0), packets_dropped: AtomicUsize::new(0) }
    }

    pub fn add_rule(&mut self, rule: NfRule) { self.rules.push(rule); }
    ...
    pub fn process(&self, hook: NfHookpoint, pkt: &NetPacket) -> NfVerdict {
        for rule in self.rules.iter().filter(|r| r.hook == hook) {
            if rule.matches(pkt) {
                match rule.verdict {
                    NfVerdict::Accept => { self.packets_accepted.fetch_add(1, Ordering::Relaxed); }
                    NfVerdict::Drop | NfVerdict::Reject => { self.packets_dropped.fetch_add(1, Ordering::Relaxed); }
                    _ => {}
                }
                return rule.verdict;
            }
        ...
        match self.default_policy {
            NfVerdict::Accept => { self.packets_accepted.fetch_add(1, Ordering::Relaxed); }
            NfVerdict::Drop   => { self.packets_dropped.fetch_add(1, Ordering::Relaxed); }
            _ => {}
        }
        self.default_policy
    }

    pub fn packets_accepted(&self) -> usize { self.packets_accepted.load(Ordering::Relaxed) }
    pub fn packets_dropped(&self) -> usize  { self.packets_dropped.load(Ordering::Relaxed) }
}
...
mod tests {
    ...
    fn test_netfilter_drop_ssh() {
        ...
        assert_eq!(table.process(NfHookpoint::Input, &http_pkt), NfVerdict::Accept);
    }
