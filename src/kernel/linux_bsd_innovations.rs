...
mod tests {
    ...
    fn test_sovereign_cgroup_governor() {
        let mut zones = SovereignZonesManager::new();
        zones
            .create_zone("/sys/fs/cgroup/db", 500, 1024 * 1024)
            .unwrap();
        zones.configure_vnic("/sys/fs/cgroup/db", "1001").unwrap();
        assert_eq!(
            zones.zones.get("/sys/fs/cgroup/db").unwrap().cpu_shares,
            500
        );
    }
