...
impl SigmaCsprng {
    pub fn new(seed: &[u8; 32]) -> Self {
        ...
        // Key
        for i in 0..8 {
            state[4+i] = u32::from_le_bytes([seed[i*4], seed[i*4+1], seed[i*4+2], seed[i*4+3]]);
        }
        SigmaCsprng { state, counter: AtomicU64::new(0) }
    }
    ...
    pub fn generate(&self, n: usize) -> Vec<u8> {
        ...
        let seed = cnt.wrapping_mul(6364136223846793005).wrapping_add(1442695040888963407);
...
pub fn hmac_sha256(key: &[u8], msg: &[u8]) -> [u8; 32] {
    ...
    if k.len() > block_size { k = sha256(&k).to_vec(); }
...
pub fn pbkdf2_hmac_sha256(password: &[u8], salt: &[u8], iterations: u32, output_len: usize) -> Vec<u8> {
    ...
    while output.len() < output_len {
        ...
        for _ in 1..iterations {
            ...
            for (x, y) in xor.iter_mut().zip(u.iter()) { *x ^= y; }
        }
...
pub enum CryptoAlgorithm { Sha256, Blake3, Hmac, Pbkdf2, Aes128 }
...
impl CryptoEngine {
    pub fn new() -> Self { CryptoEngine { hash_ops: AtomicU64::new(0), cipher_ops: AtomicU64::new(0) } }
    ...
    pub fn hash_ops(&self) -> u64  { self.hash_ops.load(Ordering::Relaxed) }
    pub fn cipher_ops(&self) -> u64 { self.cipher_ops.load(Ordering::Relaxed) }
}
