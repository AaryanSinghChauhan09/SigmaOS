...
pub enum PageStatus { Clean, Dirty, Writeback, Evicted }
...
impl CachedPage {
    pub fn new(inode_id: u64, page_idx: u64) -> Self {
        CachedPage { inode_id, page_idx, status: PageStatus::Clean, data: [0u8; PAGE_SIZE], access_count: 0, pin_count: 0 }
    }

    pub fn mark_dirty(&mut self) {
        if self.status == PageStatus::Clean { self.status = PageStatus::Dirty; }
    }
    pub fn mark_clean(&mut self) { self.status = PageStatus::Clean; }
    pub fn pin(&mut self)   { self.pin_count += 1; }
    pub fn unpin(&mut self) { if self.pin_count > 0 { self.pin_count -= 1; } }
    pub fn can_evict(&self) -> bool { self.pin_count == 0 && self.status != PageStatus::Writeback }
}
...
impl PageCache {
    ...
    pub fn write_page(&mut self, inode_id: u64, page_idx: u64, offset: usize, data: &[u8]) {
        let page = if self.pages.contains_key(&(inode_id, page_idx)) {
            ...
            if self.pages.len() >= self.capacity { self.evict_one(); }
            self.pages.insert((inode_id, page_idx), CachedPage::new(inode_id, page_idx));
            self.pages.get_mut(&(inode_id, page_idx)).unwrap()
        };
        ...
        page.mark_dirty();
        if was_clean { self.dirty_count.fetch_add(1, Ordering::Relaxed); }
    }

    /// Read from cached page
    pub fn read_page(&mut self, inode_id: u64, page_idx: u64, offset: usize, buf: &mut [u8]) -> bool {
    ...
    pub fn dirty_pages(&self) -> usize   { self.dirty_count.load(Ordering::Relaxed) }
    pub fn hits(&self) -> usize          { self.hits.load(Ordering::Relaxed) }
    pub fn misses(&self) -> usize        { self.misses.load(Ordering::Relaxed) }
    pub fn hit_rate(&self) -> f64 {
        let h = self.hits() as f64;
        let total = (self.hits() + self.misses()) as f64;
        if total == 0.0 { 0.0 } else { h / total }
    }
    pub fn evictions(&self) -> usize     { self.evictions.load(Ordering::Relaxed) }
    pub fn writeback_ops(&self) -> usize { self.writeback_ops.load(Ordering::Relaxed) }
}

impl Default for PageCache {
    fn default() -> Self { Self::new(1024) } // 4MB default (1024 × 4K pages)
}
