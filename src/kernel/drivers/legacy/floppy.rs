...
impl FloppyType {
    pub fn geometry(&self) -> (u8, u8, u8) { // (heads, tracks, sectors)
        match self {
...
pub struct FloppyGeometry {
    pub heads: u8, pub tracks: u8, pub sectors: u8,
}
...
impl FloppyDrive {
    pub fn new(drive_num: u8, floppy_type: FloppyType) -> Self {
        ...
        FloppyDrive {
            drive_num, floppy_type,
            motor_on: false, disk_present: true,
    ...
    pub fn chs_to_lba(&self, head: u8, track: u8, sector: u8) -> usize {
        ...
        head as usize * (self.data.len() / 2) + track as usize * spt as usize + (sector as usize - 1)
    }

    pub fn read_sector(&self, lba: usize, buf: &mut [u8; 512]) -> Result<(), &'static str> {
        if lba >= self.data.len() { return Err("FDC: sector out of range"); }
    ...
    pub fn motor_on(&mut self)  { self.motor_on = true; }
    pub fn motor_off(&mut self) { self.motor_on = false; }
...
impl FloppyController {
    pub fn new() -> Self {
        FloppyController {
            ...
            base_io: FDC_BASE_IO, irq: FDC_IRQ, dma: FDC_DMA,
            initialized: false,
    ...
    pub fn attach_drive(&mut self, slot: usize, drive: FloppyDrive) {
        if slot < 4 { self.drives[slot] = Some(drive); }
    }
    ...
    pub fn drive(&self, n: usize) -> Option<&FloppyDrive> { self.drives.get(n).and_then(|d| d.as_ref()) }
    pub fn drive_mut(&mut self, n: usize) -> Option<&mut FloppyDrive> { self.drives.get_mut(n).and_then(|d| d.as_mut()) }
    pub fn recalibrate_count(&self) -> usize { self.recalibrate_count.load(Ordering::Relaxed) }
}

impl KernelSubsystem for FloppyController {
    fn name(&self) -> &str { "floppy" }
    ...
    fn initialize(&mut self) -> Result<(), SubsystemError> {
        for drive in self.drives.iter_mut().flatten() { drive.motor_off(); }
...
impl Default for FloppyController { fn default() -> Self { Self::new() } }
