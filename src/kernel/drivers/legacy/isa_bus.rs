...
impl LpcBridge {
    pub fn new(vendor_id: u16, device_id: u16) -> Self {
        LpcBridge { vendor_id, device_id, base_addr: 0, enabled: false }
    }

    /// Intel ICH/PCH common LPC vendor/device IDs
    pub fn intel_lpc() -> Self { Self::new(0x8086, 0x2440) }
...
mod tests {
    ...
    impl IsaDevice for DummyIsaDevice {
        fn resource(&self) -> &IsaResource { &self.res }
        fn probe(&mut self) -> bool { self.probed = true; true }
        fn enable(&mut self) -> Result<(), &'static str> { Ok(()) }
        fn disable(&mut self) -> Result<(), &'static str> { Ok(()) }
        fn device_name(&self) -> &str { "dummy_isa_dev" }
    }
