...
impl VideoMode {
    ...
    pub fn bpp(&self) -> u8 {
        match self {
            ...
            VideoMode::Vga320x200x256 | VideoMode::Vesa800x600x256 | VideoMode::Vesa1024x768 | VideoMode::Vesa1280x1024 => 8,
...
pub mod palette {
    ...
    pub const CYAN:         Color = Color { r: 0,   g: 170, b: 170 };
    pub const RED:          Color = Color { r: 170, g: 0,   b: 0   };
    pub const MAGENTA:      Color = Color { r: 170, g: 0,   b: 170 };
    pub const BROWN:        Color = Color { r: 170, g: 85,  b: 0   };
    pub const LIGHT_GRAY:   Color = Color { r: 170, g: 170, b: 170 };
    pub const DARK_GRAY:    Color = Color { r: 85,  g: 85,  b: 85  };
    pub const LIGHT_BLUE:   Color = Color { r: 85,  g: 85,  b: 255 };
    pub const LIGHT_GREEN:  Color = Color { r: 85,  g: 255, b: 85  };
    pub const LIGHT_CYAN:   Color = Color { r: 85,  g: 255, b: 255 };
    pub const LIGHT_RED:    Color = Color { r: 255, g: 85,  b: 85  };
    pub const LIGHT_MAGENTA:Color = Color { r: 255, g: 85,  b: 255 };
    pub const YELLOW:       Color = Color { r: 255, g: 255, b: 85  };
...
impl TextCell {
    pub fn new(ch: u8) -> Self { TextCell { ch, fg: 7, bg: 0, blink: false } }
...
impl Framebuffer {
    pub fn new(w: u32, h: u32, bpp: u8) -> Self {
        ...
        Framebuffer { width: w, height: h, bpp, data: vec![0u8; size], write_count: AtomicUsize::new(0) }
    }
    ...
    pub fn set_pixel(&mut self, x: u32, y: u32, color: u32) {
        ...
        for i in 0..bytes.min(4) {
            if off + i < self.data.len() { self.data[off + i] = cb[i]; }
        }
    ...
    pub fn clear(&mut self, color: u32) {
        ...
        for y in 0..h { for x in 0..w { self.set_pixel(x, y, color); } }
    }

    pub fn write_count(&self) -> usize { self.write_count.load(Ordering::Relaxed) }
}
...
impl TextConsole {
    pub fn vga80x25() -> Self { Self::new(80, 25) }

    pub fn new(cols: u32, rows: u32) -> Self {
        let cells = vec![TextCell::new(b' '); (cols * rows) as usize];
        TextConsole { cols, rows, cursor_x: 0, cursor_y: 0, cells, scroll_count: AtomicUsize::new(0) }
    }
    ...
    pub fn write_char(&mut self, ch: u8) {
        ...
        if self.cursor_y >= self.rows { self.scroll(); }
    }
    ...
    pub fn get_char(&self, x: u32, y: u32) -> Option<u8> {
        if x >= self.cols || y >= self.rows { return None; }
        Some(self.cells[(y * self.cols + x) as usize].ch)
    }

    pub fn scroll_count(&self) -> usize { self.scroll_count.load(Ordering::Relaxed) }
}
...
impl Default for VgaDriver { fn default() -> Self { Self::new() } }
