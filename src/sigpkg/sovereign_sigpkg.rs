...
impl ReproducibleBuildContext {
    ...
    pub fn audit_reproducibility(&self, bin1: &[u8], bin2: &[u8]) -> (bool, String) {
        if bin1 == bin2 {
            return (
                true,
                "Status: 100% REPRODUCIBLE (Bit-identical match)".to_string(),
            );
        }
        if bin1.len() != bin2.len() {
            return (
                false,
                format!(
                    "Status: NON-REPRODUCIBLE (Size Mismatch: {} vs {} bytes)",
                    bin1.len(),
                    bin2.len()
                ),
            );
        ...
        let mut report = format!(
            "Status: NON-REPRODUCIBLE ({} byte discrepancies found)\n",
            diff_offsets.len()
        );
        for (off, b1, b2) in diff_offsets {
            report.push_str(&format!(
                "  Offset 0x{:X}: 0x{:02X} != 0x{:02X}\n",
                off, b1, b2
            ));
        }
        (false, report)
