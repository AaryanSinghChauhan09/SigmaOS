...
// --- ALPM Keyring Engine & Trust Model ---

#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub enum AlpmTrustLevel {
    Unknown = 0,
    Never = 1,
    Marginal = 2,
    Full = 3,
...
#[derive(Debug, Clone)]
pub struct AlpmMasterKey {
    pub key_id: String,
    pub owner: String,
    pub trust_level: AlpmTrustLevel,
    pub is_revoked: bool,
}

pub struct AlpmKeyringEngine {
    pub keys: HashMap<String, AlpmMasterKey>,
}

impl AlpmKeyringEngine {
    pub fn new() -> Self {
        let mut engine = Self { keys: HashMap::new() };
        // Populate default Arch Linux master signing keys
        engine.import_key("6D42BDD116E0068F", "Arch Linux Master Key", AlpmTrustLevel::Ultimate);
        engine
    }

    pub fn import_key(&mut self, key_id: &str, owner: &str, trust_level: AlpmTrustLevel) {
        self.keys.insert(
            key_id.to_string(),
            AlpmMasterKey {
    ...
    pub fn verify_package_signature(&self, key_id: &str) -> bool {
        if let Some(key) = self.keys.get(key_id) {
            !key.is_revoked && key.trust_level >= AlpmTrustLevel::Marginal
        } else {
            false
        }
    }
...
impl Default for AlpmKeyringEngine {
    fn default() -> Self {
        Self::new()
    }
}

// --- ALPM Mirror Ranking Engine ---
...
pub struct AlpmMirrorRankEngine {
    pub mirrors: Vec<AlpmMirror>,
}

impl AlpmMirrorRankEngine {
    pub fn new() -> Self {
        Self { mirrors: Vec::new() }
    }

    pub fn add_mirror(&mut self, url: &str, country: &str, latency_ms: u64) {
    ...
    pub fn generate_mirrorlist(&self, limit: usize) -> String {
        let mut list = String::from("## SigmaOS ALPM Rankmirrors Generated Mirrorlist\n");
        for mirror in self.mirrors.iter().take(limit) {
            if mirror.is_active {
                list.push_str(&format!("Server = {}/$repo/os/$arch # {}ms ({})\n", mirror.url, mirror.latency_ms, mirror.country));
            }
        }
        list
    }
...
impl Default for AlpmMirrorRankEngine {
    fn default() -> Self {
        Self::new()
    }
}

// --- ALPM Cache Cleaner (paccache) ---

pub struct AlpmPaccacheCleaner {
    pub cache_dir: String,
    pub retain_count: usize,
}

impl AlpmPaccacheCleaner {
    pub fn new(cache_dir: &str, retain_count: usize) -> Self {
        Self {
    ...
    /// Extracts package name stem from filename (e.g. "bash-5.1-1-x86_64.pkg.tar.zst" -> "bash")
    fn extract_pkg_stem(filename: &str) -> String {
        if let Some(dash_idx) = filename.find('-') {
            filename[..dash_idx].to_string()
        } else {
            filename.to_string()
        }
    }

    /// Evaluates list of cached package files, groups them by package name stem, and selects older candidates for deletion
    pub fn list_candidates_for_removal(&self, cached_files: &[String]) -> Vec<String> {
        let mut candidates = Vec::new();
        let mut grouped: HashMap<String, Vec<String>> = HashMap::new();

        for file in cached_files {
            let stem = Self::extract_pkg_stem(file);
            grouped.entry(stem).or_default().push(file.clone());
        }

        for (_stem, mut files) in grouped {
            files.sort();
            if files.len() > self.retain_count {
                for file in &files[..files.len() - self.retain_count] {
...
mod tests {
    ...
    #[test]
    fn test_alpm_keyring_engine() {
        let mut keyring = AlpmKeyringEngine::new();
        assert!(keyring.verify_package_signature("6D42BDD116E0068F"));

        keyring.import_key("4AEE6019E36322D6", "Developer Key", AlpmTrustLevel::Marginal);
        assert!(keyring.verify_package_signature("4AEE6019E36322D6"));
        assert!(!keyring.verify_package_signature("UNKNOWN_KEY"));
    }

    #[test]
    fn test_alpm_mirror_rank_engine() {
        let mut ranker = AlpmMirrorRankEngine::new();
        ranker.add_mirror("https://slow.mirror.org/archlinux", "US", 350);
        ranker.add_mirror("https://fast.mirror.org/archlinux", "DE", 45);

        ranker.rank_mirrors();
        assert_eq!(ranker.mirrors[0].latency_ms, 45);

        let mirrorlist = ranker.generate_mirrorlist(1);
        assert!(mirrorlist.contains("https://fast.mirror.org/archlinux"));
        assert!(!mirrorlist.contains("https://slow.mirror.org/archlinux"));
    }

    #[test]
    fn test_alpm_paccache_cleaner() {
        let cleaner = AlpmPaccacheCleaner::new("/var/cache/pacman/pkg", 2);
        let cached = vec![
            "bash-5.1-1-x86_64.pkg.tar.zst".to_string(),
            "bash-5.1-2-x86_64.pkg.tar.zst".to_string(),
            "bash-5.2-1-x86_64.pkg.tar.zst".to_string(),
        ];

        let removal = cleaner.list_candidates_for_removal(&cached);
        assert_eq!(removal.len(), 1);
        assert_eq!(removal[0], "bash-5.1-1-x86_64.pkg.tar.zst");
    }
}
