...
impl PackageFormat {
    pub fn from_filename(filename: &str) -> Option<Self> {
        let f = filename.trim();
        if f.ends_with(".pkg.tar.xz") || f.ends_with(".pkg.tar.zst") || f.contains("pacman") { Some(PackageFormat::Pacman) }
        ...
        else if f.ends_with(".ports") || f.ends_with(".dports") || f.ends_with("bsd.ports") { Some(PackageFormat::Ports) }
        else if f.ends_with(".aab") { Some(PackageFormat::Aab) }
        else if f.ends_with(".apk") { Some(PackageFormat::Apk) }
        else if f.ends_with(".AppImage") || f.ends_with(".appimage") { Some(PackageFormat::AppImage) }
        ...
        else if f.ends_with(".appbundle") || (f.ends_with(".app") && !f.ends_with("arch_pkg.pkg")) { Some(PackageFormat::AppBundle) }
        else if f.ends_with(".hap") { Some(PackageFormat::Hap) }
        else if f.ends_with(".pisi") || f.ends_with(".PiSi") { Some(PackageFormat::Pisi) }
        ...
        else if f.ends_with(".sigma") || f.ends_with(".sigpkg") { Some(PackageFormat::Sigma) }
