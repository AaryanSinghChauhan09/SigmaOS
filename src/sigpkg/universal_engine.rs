...
impl PackageFormat {
    pub fn from_filename(filename: &str) -> Option<Self> {
        ...
        else if f.ends_with(".ebuild") { Some(PackageFormat::Portage) }
        ...
        else if f.ends_with(".ports") || f.ends_with(".portage") { Some(PackageFormat::Ports) }
        else if f.ends_with(".openbsd.tgz") { Some(PackageFormat::OpenBsdPkg) }
        ...
        else if f.ends_with(".tar.gz") || f.ends_with(".tgz") || f.contains(".tar .gz") { Some(PackageFormat::TarGz) }
        else if f.ends_with(".pkg.tar.xz") || f.ends_with(".pkg.tar.zst") || f.ends_with(".arch.pkg.tar.xz") || f.ends_with(".pacman.pkg.tar.zst") { Some(PackageFormat::Pacman) }
        ...
        else if f.ends_with(".p5p") { Some(PackageFormat::SolarisIps) }
        ...
        else if f.ends_with(".xbps") { Some(PackageFormat::Xbps) }
