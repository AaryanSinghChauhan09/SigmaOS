...
impl PackageFormat {
    pub fn from_filename(filename: &str) -> Option<Self> {
        ...
        if normalized.ends_with(".openbsd.tgz") { Some(PackageFormat::OpenBsdPkg) }
        else if normalized.ends_with(".pkg.tar.zst") || normalized.ends_with(".pkg.tar.xz") || normalized.ends_with(".pkg.tar.gz") { Some(PackageFormat::Pacman) }
        ...
        else if normalized.ends_with(".portage") { Some(PackageFormat::Ports) }
        else if normalized.ends_with(".ebuild") { Some(PackageFormat::Portage) }
        else if normalized.ends_with(".deb") || normalized.ends_with(".udeb") { Some(PackageFormat::Apt) }
        ...
        else if normalized.ends_with(".p5p") { Some(PackageFormat::SolarisIps) }
