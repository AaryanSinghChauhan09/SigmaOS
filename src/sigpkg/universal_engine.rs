...
impl PackageFormat {
    pub fn from_filename(filename: &str) -> Option<Self> {
        ...
        let f_clean = f.trim();
        if f_clean.ends_with(".deb") { Some(PackageFormat::Apt) }
        else if f_clean.ends_with(".rpm") { Some(PackageFormat::Yum) }
        else if f_clean.ends_with(".openbsd.tgz") { Some(PackageFormat::OpenBsdPkg) }
        else if f_clean.ends_with(".pkg.tar.zst") || f_clean.ends_with(".pkg.tar.xz") || f_clean.ends_with(".pkg.tar.gz") { Some(PackageFormat::Pacman) }
        else if f_clean.ends_with(".ebuild") { Some(PackageFormat::Portage) }
        else if f_clean.ends_with(".portage") { Some(PackageFormat::Ports) }
        else if f_clean.ends_with(".nix") || f_clean.ends_with(".nixpkg") { Some(PackageFormat::Nix) }
        else if f_clean.ends_with(".apk") { Some(PackageFormat::Apk) }
        else if f_clean.ends_with(".xbps") { Some(PackageFormat::Xbps) }
        else if f_clean.ends_with(".air") { Some(PackageFormat::Air) }
        else if f_clean.ends_with(".bottle") { Some(PackageFormat::Bottle) }
        else if f_clean.ends_with(".ipa") { Some(PackageFormat::Ipa) }
        ...
        else if f_clean.ends_with(".hap") { Some(PackageFormat::Hap) }
        else if f_clean.ends_with(".pisi") { Some(PackageFormat::Pisi) }
        else if f_clean.ends_with(".superdeb") { Some(PackageFormat::Superdeb) }
        else if f_clean.ends_with(".lzm") { Some(PackageFormat::Lzm) }
        else if f_clean.ends_with(".pup") { Some(PackageFormat::Pup) }
        else if f_clean.ends_with(".pet") { Some(PackageFormat::Pet) }
        else if f_clean.ends_with(".flatpak") { Some(PackageFormat::Flatpak) }
        else if f_clean.ends_with(".snap") { Some(PackageFormat::Snap) }
        else if f_clean.ends_with(".txz") { Some(PackageFormat::Txz) }
        else if f_clean.ends_with(".guix") { Some(PackageFormat::Guix) }
        else if f_clean.ends_with(".eopkg") { Some(PackageFormat::Eopkg) }
        else if f_clean.ends_with(".zypper") || f_clean.ends_with(".spec") { Some(PackageFormat::Zypper) }
        else if f_clean.ends_with(".appimage") { Some(PackageFormat::AppImage) }
        else if f_clean.ends_with(".moss") { Some(PackageFormat::Moss) }
        else if f_clean.ends_with(".hpkg") { Some(PackageFormat::Hpkg) }
        else if f_clean.ends_with(".tcz") { Some(PackageFormat::Tcz) }
        else if f_clean.ends_with(".gobo") { Some(PackageFormat::Gobo) }
        else if f_clean.ends_with(".ostree") { Some(PackageFormat::Ostree) }
        else if f_clean.ends_with(".pkgsrc") { Some(PackageFormat::Pkgsrc) }
        else if f_clean.ends_with(".sfs") { Some(PackageFormat::Sfs) }
        else if f_clean.ends_with(".puk") { Some(PackageFormat::Puk) }
        else if f_clean.ends_with(".dmg") { Some(PackageFormat::Dmg) }
        else if f_clean.ends_with(".cports") { Some(PackageFormat::Cports) }
        else if f_clean.ends_with(".dports") { Some(PackageFormat::Dports) }
        else if f_clean.ends_with(".slackbuild") { Some(PackageFormat::SlackBuild) }
        else if f_clean.ends_with(".crux") { Some(PackageFormat::Crux) }
        else if f_clean.ends_with(".drpm") { Some(PackageFormat::Drpm) }
        else if f_clean.ends_with(".stratum") { Some(PackageFormat::Stratum) }
        else if f_clean.ends_with(".ipk") { Some(PackageFormat::Ipk) }
        else if f_clean.ends_with(".opkg") { Some(PackageFormat::Opkg) }
        else if f_clean.ends_with(".p5p") { Some(PackageFormat::SolarisIps) }
        else if f_clean.ends_with(".nar") { Some(PackageFormat::GuixNar) }
        else if f_clean.ends_with(".whl") { Some(PackageFormat::Wheel) }
        else if f_clean.ends_with(".crate") { Some(PackageFormat::Crate) }
        else if f_clean.ends_with(".gem") { Some(PackageFormat::Gem) }
        else if f_clean.ends_with(".nupkg") { Some(PackageFormat::Nupkg) }
        else if f_clean.ends_with(".vcpkg") { Some(PackageFormat::Vcpkg) }
        else if f_clean.ends_with(".narinfo") { Some(PackageFormat::NarInfo) }
        else if f_clean.ends_with(".spack") { Some(PackageFormat::Spack) }
        else if f_clean.ends_with(".conan") { Some(PackageFormat::Conan) }
        else if f_clean.ends_with(".sysupdate") { Some(PackageFormat::Sysupdate) }
        else if f_clean.ends_with(".sigma") || f_clean.ends_with(".sigpkg") { Some(PackageFormat::Sovereign) }
        else { None }
    }
}
