...
impl FedoraAnityaReleaseMonitoringEngine {
    ...
    pub fn check_upstream_version(
        ...
        if let Some(record) = self.projects.get_mut(project_name) {
            ...
            if is_new {
                let pkg_mapping = vec![
                    AnityaPackageMapping { distro: "Fedora".to_string(), package_name: project_name.to_string() },
                    AnityaPackageMapping { distro: "SigmaOS".to_string(), package_name: project_name.to_string() },
                ];
