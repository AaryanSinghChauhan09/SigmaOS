...
mod tests {
    ...
    fn test_uefi_secure_db_signature_validation() {
        ...
        // Calculate hash
        let mut data_hash = [0u8; 64];
        for i in 0..kernel_data.len().min(64) {
            data_hash[i] = kernel_data[i].wrapping_add(0x13);
        }

        // Enroll revoked certificate in dbx
        let revoked_cert = UefiCertificate {
            key_id: 2002,
            subject: [0; 32],
            hash_dilithium5: data_hash,
        };
        sb.enroll_certificate(revoked_cert, 3);

        // Check revoked fails
        let check_revoked = sb.verify_signature(&kernel_data);
        assert_eq!(check_revoked, Err(BootError::SignatureRevoked));

        // Enroll authorized certificate in db
        let authorized_cert = UefiCertificate {
            key_id: 2001,
            subject: [0; 32],
            hash_dilithium5: data_hash,
        };
        sb.enroll_certificate(authorized_cert, 2);

        // Check authorized succeeds
        let check_auth = sb.verify_signature(&kernel_data).unwrap();
        assert!(check_auth);
    }
}
