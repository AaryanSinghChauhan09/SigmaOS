...
impl Initramfs {
    ...
    pub fn extract_cpio(&self) -> Result<Vec<ExtractedFile>, BootError> {
        ...
        while offset + 110 <= self.data.len() {
            ...
            // Skip namesize pad to next 4-byte boundary
            ...
            // Skip filesize pad to next 4-byte boundary
...
mod tests {
    ...
    fn test_initramfs_cpio_parsing() {
        ...
        // 3-byte padding to 4-byte boundary
        cpio_bytes.extend_from_slice(b"\0\0\0");

        // 11-byte file content
        cpio_bytes.extend_from_slice(b"hello world");
        // 1-byte padding to 4-byte boundary
        ...
        cpio_bytes.extend_from_slice(b"\0"); // padding to 4-byte boundary (11 + 1 = 12)
