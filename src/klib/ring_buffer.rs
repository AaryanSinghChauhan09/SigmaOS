...
impl<T> HeapRingBuffer<T> {
    ...
    pub fn new(capacity: usize) -> Self {
        ...
        let layout = core::core::alloc::Layout::array::<core::mem::MaybeUninit<T>>(cap).unwrap();
...
impl<T> Drop for HeapRingBuffer<T> {
    fn drop(&mut self) {
        while self.pop().is_some() {}
        let layout = core::core::alloc::Layout::array::<core::mem::MaybeUninit<T>>(self.cap).unwrap();
        unsafe {
