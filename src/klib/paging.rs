...
impl PageTableEntry for SimplePageTableEntry {
    ...
    fn is_huge(&self) -> bool {
        self.accessed.load(Ordering::SeqCst) == 1 || self.huge.load(Ordering::SeqCst) == 1
    }
    fn is_giant(&self) -> bool {
...
impl SimpleVMM {
    ...
    pub fn mark_copy_on_write(&mut self, virt: VirtualAddress) -> Result<(), PageFaultError> {
        ...
        if let Some(ref mut pdpt) = self.pdpt_tables.get_mut(pml4_idx).and_then(|x| x.as_mut()) {
            if !pdpt.get_entry(pdpt_idx).is_present() {
                return Err(PageFaultError::NotPresent);
            }

            if let Some(ref mut pd) = self.pd_tables.get_mut(pdpt_idx).and_then(|x| x.as_mut()) {
                if !pd.get_entry(pd_idx).is_present() {
                    return Err(PageFaultError::NotPresent);
                }

                if let Some(ref mut pt) = self.pt_tables.get_mut(pd_idx).and_then(|x| x.as_mut()) {
...
impl VirtualMemoryManager for SimpleVMM {
    fn map_page(
        ...
        if !self.pml4.get_entry(pml4_idx).is_present() {
            ...
            self.pdpt_tables[pml4_idx] = Some(SimplePageTable::new(pdpt_phys));
            self.pml4.set_entry(pml4_idx, pdpt_entry);
        }
    ...
    fn map_huge_page(
        ...
        if let Some(ref mut pdpt) = self.pdpt_tables[pdpt_idx_in_vec] {
            ...
            let pd_phys = if !is_pdpt_present {
                ...
                let pd_table = SimplePageTable::new(phys);
                while self.pd_tables.len() <= pdpt_idx {
                    self.pd_tables.push(None);
                }
                self.pd_tables[pdpt_idx] = Some(pd_table);
                ...
                pdpt.get_entry_ref(pdpt_idx).get_physical_address()
            };
        ...
        if let Some(ref mut pd) = self.pd_tables[pdpt_idx] {
            if !pd.get_entry(pd_idx).is_present() {
                ...
                self.pt_tables[pd_idx] = Some(SimplePageTable::new(pt_phys));
                pd.set_entry(pd_idx, pt_entry);
            }
        }

        // Set PT entry -> Physical address
    ...
    fn get_physical(&self, virt: VirtualAddress) -> Option<PhysicalAddress> {
        ...
        if let Some(ref pdpt) = self.pdpt_tables.get(pml4_idx).and_then(|x| x.as_ref()) {
            ...
            if let Some(ref pd) = self.pd_tables.get(pdpt_idx).and_then(|x| x.as_ref()) {
                ...
                if pd_entry.is_huge() {
                    let huge_page_offset = virt & 0x1FFFFF; // 2MB offset
                    return Some(pd_entry.get_physical_address() | huge_page_offset);
                }

                if let Some(ref pt) = self.pt_tables.get(pd_idx).and_then(|x| x.as_ref()) {
                    let pt_entry = pt.get_entry_ref(pt_idx);
                    if pt_entry.is_present() {
                        let page_offset = virt & 0xFFF; // 4KB offset
                        ...
                        if pd.get_entry(pd_idx).is_present() {
                            if let Some(ref mut pt) = self.pt_tables.get_mut(pd_idx).and_then(|x| x.as_mut()) {
                                ...
                                if pt_entry.is_present() && pt_entry.is_cow() && is_write {
                                    ...
                                    // Only perform memory copying if physical addresses are valid
...
mod tests {
    ...
    fn test_transparent_huge_pages_and_tlb_shootdowns() {
        ...
        let virt_2mb = 0x200000;
        let phys_2mb = 0x4000000;
        let virt_1gb = 0x40000000;
        let phys_1gb = 0x80000000;

        assert!(vmm.get_physical(virt_2mb).is_none());
        assert!(vmm
            .map_large_page(virt_2mb, phys_2mb, PageSize::Huge2MB, false, true)
            .is_ok());
        assert_eq!(vmm.get_physical(virt_2mb).unwrap(), phys_2mb);
        assert_eq!(
            vmm.get_physical(virt_2mb + 0x1000).unwrap(),
            phys_2mb + 0x1000
        ); // offset check
        ...
        assert!(vmm.unmap_page(virt_2mb).is_ok());
        assert_eq!(vmm.tlb_tracker.pending_shootdowns.load(Ordering::SeqCst), 3);
    }
}
