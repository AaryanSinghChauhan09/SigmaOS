...
impl VirtualMemoryManager for SimpleVMM {
    fn map_page(
        ...
        let pd_phys = if let Some(ref pdpt) = self.pdpt_tables[pdpt_idx_in_vec] {
    ...
    fn get_physical(&self, virt: VirtualAddress) -> Option<PhysicalAddress> {
        ...
        if let Some(ref pdpt) = self.pdpt_tables.get(pml4_idx).and_then(|o| o.as_ref()) {
            ...
            if let Some(ref pd) = self.pd_tables.get(pd_idx_in_vec).and_then(|o| o.as_ref()) {
                ...
                if let Some(ref pt) = self.pt_tables.get(pt_idx_in_vec).and_then(|o| o.as_ref()) {
...
mod tests {
    ...
    fn test_huge_pages_mapping() {
        ...
        assert!(vmm
            .map_large_page(virt_2mb, phys_2mb, PageSize::Huge2MB, true, true)
            .is_ok());
