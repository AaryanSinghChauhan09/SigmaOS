...
/// Upper bound length limit for unbounded C-string scans to prevent memory overreads
pub const MAX_CSTR_LEN: usize = 1_048_576; // 1 MB guard
...
pub unsafe fn cstr_to_rust_string(ptr: *const i8) -> Result<String, &'static str> {
    ...
    if len == MAX_CSTR_LEN {
        return Err("String exceeds maximum length limit");
    }
