...
static uint32_t current_port_id = 1000;

mach_port_t sovereign_mach_port_allocate() {
    uint32_t allocated = current_port_id++;
    printf("[MACH] Allocated Port: %u\n", allocated);
...
void sovereign_mach_msg_send(mach_msg_header_t* header, void* data, mach_msg_size_t size) {
    if (header->remote_port == MACH_PORT_NULL) {
        printf("[MACH] Error: Sending to NULL port.\n");
        return;
    }
    printf("[MACH] Sending Message ID: %d to Port %u (Size: %u bytes)\n", header->id, header->remote_port, size);
...
void sovereign_mach_msg_receive(mach_port_t port, mach_msg_header_t* header, void* buffer, mach_msg_size_t buffer_size) {
    if (port == MACH_PORT_NULL) {
        printf("[MACH] Error: Receiving from NULL port.\n");
        return;
    }
    printf("[MACH] Receiving message from Port %u (Current Buffer: %u bytes)\n", port, buffer_size);
