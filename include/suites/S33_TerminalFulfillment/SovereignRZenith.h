...
CLASS_DECLARE(SovereignRVectorMath) { 
    ...
    VIRTUAL(void, ExecuteSIMDVectorOp, struct SovereignRVectorMath* self, const sigma_f64* vecA, const sigma_f64* vecB, sigma_f64* result, sigma_size_t len);
    VIRTUAL(sigma_f64, ExecuteStatisticalInference, struct SovereignRVectorMath* self, const sigma_f64* dataSet, sigma_size_t len);
};
...
CLASS_DECLARE(SovereignDataframeMatrix) { 
    ...
    VIRTUAL(void, InitializeTabularFrame, struct SovereignDataframeMatrix* self, const char* colNames[], sigma_size_t numCols);
