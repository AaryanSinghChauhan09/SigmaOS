...
| Legacy Drivers (6) | Driver_Ecosystem.md | ✅ Implemented | `src/kernel/drivers/legacy/` |
...
| CBOR IPC wire format | 🆕 | `kernel/ipc.rs` |
| IPC message deduplication | 🆕 | `kernel/ipc.rs` |
...
| 50+ POSIX syscalls | ✅ | `kernel/syscall/table.rs` |
| TCP state machine (RFC 793) | ✅ | `kernel/net/tcp_state_machine.rs` |
| CBOR IPC + deduplication | 🆕 | `kernel/ipc.rs` |
| Dataflow kernel mode | 🆕 | `kernel/sigma_dataflow.rs` |
| Blockchain audit log | 🆕 | `kernel/sigma_blockchain_audit.rs` |
...
| PQC crypto (Kyber+Dilithium) | ✅ | `src/crypto/` |
| CryptFS PBKDF2 + XTS-AES | 🆕 | `fs/sigmacas.rs` |
...
## ⚠️ Remaining Gaps (Verified Against Source Tree)

The following files claimed in earlier documentation sections have been verified against the current `src/` tree and remain missing:

| Missing File | Category | Notes |
|---|---|---|
| `kernel/sigma_dataflow.rs` | Kernel | Dataflow kernel mode |
| `kernel/sigma_blockchain_audit.rs` | Kernel | Blockchain audit log |
| `kernel/sigma_workload_modes.rs` | Kernel | Workload performance modes |
| `fs/sigma_cryptfs.rs` | Filesystem | Closest match: `fs/sigmacas.rs` |
| `kernel/ipc/sigma_ipc_cbor.rs` | Kernel | Closest match: `kernel/ipc.rs` |
| `userland/tools/sigma_compliance.nim` | Userland | Not yet implemented |
| `userland/tools/sigma_syscall_telemetry.nim` | Userland | Not yet implemented |
| `userland/tools/sigma_deterministic_replay.nim` | Userland | Not yet implemented |
| `userland/net/sigma_swarm_lb.rs` | Userland | Not yet implemented |
| `userland/update/sigma_decentralised_update.rs` | Userland | Not yet implemented |
| `userland/compat/sigma_linux_compat.nim` | Userland | Not yet implemented |
| `userland/shell/sigma_sh_features.rs` | Userland | Not yet implemented |
| `tools/sigma_shard_generator.nim` | Tools | Not yet implemented |
| `tools/sigma_posix_conformance.nim` | Tools | Not yet implemented |
| `tools/sigma_kernel_doc.nim` | Tools | Not yet implemented |
| `pkg/sigma_pkg_absorb.nim` | Packages | Not yet implemented |
| `pkg/sigma_pkg_recipe.nim` | Packages | Not yet implemented |
| `virtualization/ocirunner/sigma_oci.rs` | Virtualization | Not yet implemented |
| All `sigma_agent_*.nim` files | AI Agent | ~30 Nim modules not yet implemented |

**Recent Additions (2026-07-27):**
- ✅ `kernel/sched/sigma_mlfq.rs` — Multi-Level Feedback Queue scheduler
- ✅ `kernel/memory/sigma_buddy.rs` — Buddy allocator wrapper
- ✅ `userland/indiastack/sigma_india_stack.rs` — India Stack integration
- ✅ `kernel/sigma_kernel_autotuner.rs` — Genetic kernel autotuner
- ✅ `kernel/sched/sigma_transformer_sched.rs` — Transformer-based scheduler
- ✅ `kernel/sched/sigma_thermal_sched.rs` — Thermal-aware scheduler
