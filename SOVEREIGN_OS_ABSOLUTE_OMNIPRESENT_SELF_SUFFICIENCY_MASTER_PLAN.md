...
| **Apache OpenOffice / LibreOffice** | `S-OFFICE` | Formula dependency DAG engine executing cell calculations asynchronously with O(1) reactive sheet updates. |
| **GIMP / Krita** | `S-MEDIA` | Non-destructive vector adjustment layer system with hardware-accelerated SIMD brush and layer mixing. |
| **Audacity** | `S-MEDIA` | Dual-buffered hardware DMA direct audio mixer with microsecond-level latency and multi-track lockless ring buffers. |
...
| **Inkscape** | `S-MEDIA` | Infinite-canvas vector renderer executing bezier transformations on local GPU rasterization pipelines. |
| **7-Zip / PeaZip** | `S-OFFICE` | Native bounds-checked parallel LZMA, DEFLATE, and ZPAQ compression algorithms in safe-Rust. |
| **Virtual Magnifying Glass** | `S-MEDIA` | Desktop compositor level fragment shader magnification filter over cursor bounding coordinates. |
| **VYM / Compendium** | `S-OFFICE` | Concept semantic mind mapping displaying graph nodes with visual vector links compiled natively into UI. |
...
| **Tor / Tails** | `S-CONNECT` | Microkernel onion routing stack wrapping sockets in volatile, RAM-only temporary enclaves with auto-shred on exit. |
| **Signal** | `S-CONNECT` | Double-Ratchet PQ-authenticated end-to-end messaging client embedded directly into local shell communication protocols. |
...
| **Android / Scratch** | `S-VIRT` | Safe-Rust ART runtime simulator compiling Dex bytecode directly to microkernel capability tokens. |
| **GParted / TestDisk / FIPS** | `S-VIRT` | Dynamic GPT boundary realignment and sector-level partition mapping and metadata table recovery. |
...
| **Wireshark** | `S-SECURE` | Kernel-level raw packet interceptor parsing multi-layer network frames directly onto ring buffers. |
| **ClamAV / ClamWin** | `S-SECURE` | Real-time, memory-mapped signature scanning engine inspecting filesystem read/write sequences on-access. |
| **The Sleuth Kit / Coroner's Toolkit** | `S-SECURE` | Raw disk sector imaging and partition forensic analysers running natively outside guest address spaces. |
| **Lynis** | `S-SECURE` | Local system capability, token boundary, and syscall boundary integrity auditing engine. |
| **Leaf Project** | `S-SECURE` | Ultra-compact firewall and router configuration system acting as microkernel packet filter. |
...
| **Apache Cassandra / CouchDB** | `S-DATA` | Log-structured merge tree wide-column databases with gossiped cluster state synchronization. |
| **PostGIS** | `S-DATA` | R-Tree / Kd-Tree spatial indexing supporting O(log N) operations on coordinates and polygon geometries. |
...
| **Google JAX** | `S-ML` | Asynchronous multi-dimensional tensor sharding and transformation matrix engines written in pure Rust. |
| **DeepSpeed / Horovod** | `S-ML` | Ring-AllReduce data parallel training primitives routing gradients directly across Sovereign mesh nodes. |
| **Dlib / OpenCV** | `S-ML` | Convolutions, Hough transforms, and Canny edge detectors executing directly on SIMD hardware registers. |
| **fastai / PlaidML / ONNX / OpenVINO** | `S-ML` | High-performance graph compilers transforming neural network DAGs into hardware-optimized assembly. |
| **MindSpore / MXNet / Microsoft CNTK** | `S-ML` | Static graph compilations with optimal memory allocation algorithms for massive layer backprop. |
| **Scikit-learn / Shogun / Weka / ML.NET** | `S-ML` | Zero-dependency classical statistical machine learning primitives (SVMs, Random Forests, K-Means). |
...
| **DeepSeek (R1, V3)** | `S-AI` | Mixture-of-Experts (MoE) routing with expert load-balancing auxiliary loss metrics executing at bare-metal speed. |
| **Mistral / Falcon / Gemma / OLMo** | `S-AI` | Memory-efficient sliding window attention and RoPE (Rotary Position Embeddings) inference loops. |
| **BERT / GPT / T5 / XLNet** | `S-AI` | High-throughput encoder-decoder pipelines compiled without heavy Python/C++ dependency bloat. |
| **Cerebras / GLM / Granite / Grok-1** | `S-AI` | Massive parameter streaming architectures utilizing high-bandwidth GPU-to-disk pipelines. |
| **Kimi / Phi / Qwen / Sarvam / Step** | `S-AI` | Fine-grained tokenizers and low-bit quantized weight execution engines optimized for ARM/x86 vector arrays. |
| **vLLM / SGLang / llama.cpp / Ollama** | `S-AI` | In-memory continuous batching and PagedAttention algorithms managing KV caches at kernel level. |
| **CrewAI / AutoGPT / AgentGPT / LangChain** | `S-AI` | Autonomous visual planning workflows coordinating recursive agents via microkernel IPC channels. |
...
| **NLTK / spaCy / Spark NLP / Word2vec** | `S-AI` | Fast POS taggers, dependency parsers, and continuous-bag-of-words (CBOW) word vector lookups. |
| **Apertium / Moses / ChatScript / Gensim** | `S-AI` | Multi-language rule-based translation modules and Latent Dirichlet Allocation topic modelling. |
...
| **OpenModelica / ASCEND / Calcpad** | `S-SIM` | Dynamic physical system simulator resolving non-linear differential equations via RK4/Verlet. |
| **Calculix / OpenSees / QBlade / XFOIL** | `S-SIM` | Finite Element Analysis (FEA) and aerodynamic panel solver modeling structural load boundaries. |
| **GROMACS / LAMMPS / CP2K** | `S-SIM` | Molecular dynamics simulator computing atomistic interactions with long-range electrostatic algorithms. |
| **CHEMKIN / COCO / DWSIM / Open Babel** | `S-SIM` | Chemical kinetics, chemical structure mappings, and thermodynamic equilibrium phase solvers. |
| **JSBSim / GMAT / OpenVSP** | `S-SIM` | Orbital orbital mechanics, flight dynamics equations, and vehicle vector aircraft design models. |
| **KNIME / Orange / RapidMiner** | `S-SCIENCE` | Visual ETL data mining workflow graphs mapping tabular transformations to pipeline tasks. |
