...
- **Unit Testing**: We use **GoogleTest** for shard verification. Tests are located in `tests/cpp_host/`.
- **Modular Architecture**: Shards are organized by domain (e.g., `kernel/core/fs/`, `kernel/core/security/`). Always keep implementation decoupled from headers.
- Static analysis: `cppcheck` and `clang-tidy` run in CI.
