...
public:
    void Clean(float* data, sigma_usize& size) override {
        sigma_log("[DATA/CLEAN]: Identifying outliers in %u data points...\n", (unsigned int)size);
    ...
    void Reduce(float* data, sigma_usize& size, sigma_usize target_size) override {
        ...
        sigma_log("[DATA/REDUCE]: Reduction complete. Target size: %u.\n", (unsigned int)target_size);
    }
};

extern "C" void _start(void) {
    sigma_log("--- Î£ SIGMA OS SOVEREIGN DATA PREPROCESSOR (ZENITH) ---\n");
