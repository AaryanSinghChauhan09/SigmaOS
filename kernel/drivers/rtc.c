/*
 * =============================================================================
 * Σ SIGMAOS KERNEL: RTC DRIVER (v1.0 — PURE C11)
 * =============================================================================
 * Hardware: CMOS RTC (ports 0x70/0x71) — i8254-compatible
...
static bool_t rtc_is_updating(void) {
...
/* =========================================================================
 * rtc_read_time — reads date/time from CMOS RTC
...
const SigmaRTCTime *rtc_read_time(void) {
    ...
    u8 sec   = rtc_read(RTC_REG_SEC);
    u8 min   = rtc_read(RTC_REG_MIN);
    u8 hour  = rtc_read(RTC_REG_HOUR);
    u8 day   = rtc_read(RTC_REG_DAY);
    u8 month = rtc_read(RTC_REG_MONTH);
    u8 year  = rtc_read(RTC_REG_YEAR);

    /* Check if BCD mode or binary mode */
    u8 regb = rtc_read(RTC_REG_B);
    if (!(regb & 0x04u)) {
        /* BCD mode */
        sec   = bcd_to_bin(sec);
        min   = bcd_to_bin(min);
    ...
    g_rtc_time.year  = (u16)(2000u + year);  /* Y2K correction */
...
/* =========================================================================
 * rtc_uptime_seconds — estimated uptime using jiffy counter
 * ========================================================================= */
extern u64 pit_get_jiffies(void);

u64 rtc_uptime_seconds(void) {
