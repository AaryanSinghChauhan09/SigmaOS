...
public:
    ...
    static void init() {
        sigma_log("S [PKEY]: Initializing Hardware Memory Protection Keys (MPK)...");
        // Enable CR4.PKE bit
        this->active_keys = 0;
    ...
    int allocateKey() {
        ...
        sigma_log("S [PKEY]: Allocated Memory Protection Key: %d\n", key);
        return key;
    }

    void setProtection(int key, sigma_u32 rights) {
        // WRPKRU instruction to set rights for the key
        sigma_log("S [PKEY]: Setting Rights 0x%X for Key %d\n", rights, key);
    }
