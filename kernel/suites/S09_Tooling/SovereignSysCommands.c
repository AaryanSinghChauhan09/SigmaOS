...
void handle_ui(int argc, char** argv) {
    ...
    else if (sigma_strcmp_local(action, "tile") == 0)      sigma_printf("[UI] Activating tiling layout: %s\n", target);
    else sigma_printf("[UI] Action %s executed.\n", action);
}
...
void handle_sys(int argc, char** argv) {
    ...
    if (sigma_strcmp_local(action, "kill") == 0 && argc > 3) {
        ...
        sigma_printf("[SYS] Process %d terminated.\n", pid);
    } else if (sigma_strcmp_local(action, "info") == 0) {
        sigma_printf("[SYS] SigmaOS Zenith vROADMAP_1001 | Industrial Parity Secured.\n");
    } else {
        sigma_printf("[SYS] Action %s processed.\n", action);
    }
}
