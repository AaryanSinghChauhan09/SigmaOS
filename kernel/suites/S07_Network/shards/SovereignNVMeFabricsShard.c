...
void sigma_net_storage_connect(const char* target_ip, sigma_u16 nqn_port) {
    sigma_printf("[NVME-oF]: Establishing Fabric Connection to %s:%u...\n", target_ip, nqn_port);
    sigma_printf("[NVME-oF]: Performing RDMA/RoCE Handshake... Queue pairs established.\n");
    sigma_printf("[NVME-oF]: Remote Block Device '/dev/snvme0' mapped to VFS.\n");
}

/**
 * sigma_net_storage_io: Dispatches an I/O request over the fabric.
 */
void sigma_net_storage_io(sigma_u64 lba, sigma_size_t size, int is_write) {
    sigma_printf("[NVME-oF]: Fabric I/O: %s %llu bytes at LBA 0x%llX.\n", 
