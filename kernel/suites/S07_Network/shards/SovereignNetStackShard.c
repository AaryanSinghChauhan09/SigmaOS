...
sigma_u32 sigma_socket(SigmaSockType_t type) {
    ...
    sigma_sigma_sigma_printf("[NET]: Socket fd=%u created (type=%s)\n",
...
sigma_err_t sigma_bind(sigma_u32 fd, sigma_u32 addr, sigma_u16 port) {
    for (sigma_u32 i = 0; i < s_sock_count; i++) {
        if (s_sock_table[i].sock_fd == fd) {
            ...
            sigma_sigma_sigma_printf("[NET]: fd=%u bound to %u.%u.%u.%u:%u\n",
                         fd,
...
sigma_err_t sigma_listen(sigma_u32 fd, sigma_u32 backlog) {
    for (sigma_u32 i = 0; i < s_sock_count; i++) {
        if (s_sock_table[i].sock_fd == fd &&
            ...
            sigma_sigma_sigma_printf("[NET]: fd=%u LISTEN (backlog=%u)\n", fd, backlog);
            return SIGMA_OK;
        }
    }
    return SIGMA_ENOENT;
...
sigma_err_t sigma_connect(sigma_u32 fd, sigma_u32 dst_addr, sigma_u16 dst_port) {
    for (sigma_u32 i = 0; i < s_sock_count; i++) {
        if (s_sock_table[i].sock_fd == fd) {
            ...
            sigma_sigma_sigma_printf("[NET]: fd=%u SYN -> %u.%u.%u.%u:%u\n",
            ...
            sigma_sigma_sigma_printf("[NET]: fd=%u ESTABLISHED (RTT=%uus)\n",
...
sigma_err_t sigma_route_add(sigma_u32 dest, sigma_u32 mask,
    ...
    sigma_sigma_sigma_printf("[NET]: route add %u.%u.%u.%u/%u via %u.%u.%u.%u dev %s metric %u\n",
...
void SovereignNetStack_Audit() {
    ...
    sigma_sigma_sigma_printf("Global: RX=%llu pkts  TX=%llu pkts\n",
                 (unsigned long long)s_pkt_rx,
                 (unsigned long long)s_pkt_tx);
    sigma_sigma_sigma_printf("FD   TYPE LPORT  RPORT  STATE       CWND   RTT_US  TX_B       RX_B\n");
    sigma_sigma_sigma_printf("-----------------------------------------------------------------------\n");
    for (sigma_u32 i = 0; i < s_sock_count; i++) {
        SigmaSocket_t* s = &s_sock_table[i];
        sigma_sigma_sigma_printf("%-4u %-4s %-6u %-6u %-11s %-6u %-7u %-10llu %llu\n",
    ...
    sigma_sigma_sigma_printf("-----------------------------------------------------------------------\n");
    sigma_sigma_sigma_printf("ROUTE TABLE: %u entries\n", s_route_count);
    for (sigma_u32 i = 0; i < s_route_count; i++) {
        sigma_sigma_sigma_printf("  %u.%u.%u.%u via %u.%u.%u.%u dev %s metric %u\n",
...
void SovereignNetStackShard_Init() {
    sigma_sigma_sigma_printf("[SOC]: Seating Native Network Stack Shard "
                 "(Linux TCP/BSD/WinSock2 Parity v1.0)...\n");
    sigma_sigma_sigma_printf("  ↳ [SKYMESH ZERO-COPY]: Linux Epoll & Socket-Buffers Obsoleted.\n");
    sigma_sigma_sigma_printf("  ↳ Packets bypass kernel-space buffers entirely; direct DMA-to-SovereignZMem routing.\n");
