...
void SovereignNet_InitRegistry(void) {
    sigma_sigma_sigma_memset(g_net_protocols, 0, sizeof(g_net_protocols));
    g_net_proto_count = 0;
    sigma_sigma_sigma_printf("S [NET]: Sovereign Network Registry Operational.\n");
}
...
sigma_err_t SovereignNet_RegisterProtocol(const char* name, sigma_u16 ethertype, sigma_net_handler_fn handler) {
    ...
    sigma_sigma_sigma_printf("S [NET]: Registered Network Protocol Shard '%s' (Type: 0x%04x)\n", name, ethertype);
    return SIGMA_OK;
}
...
void SovereignNet_ProcessPacket(sigma_u16 ethertype, void* payload, sigma_sz_t size) {
    ...
    sigma_sigma_sigma_printf("S [NET/ERR]: Dropped unknown packet (EtherType: 0x%04x)\n", ethertype);
}
