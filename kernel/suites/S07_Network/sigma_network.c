...
void sigma_net_init(void) {
    ...
    printf("[NET] Sovereign Network Protocol Stack online. Linux TCP/IP containerized in Vault.\n");
}
...
void sigma_net_handle_packet(void* packet, uint32_t size) {
    ...
    if (ethertype == 0x0800) { // IPv4
        ...
        if (calc != 0) {
            printf("[NET] WARN: Dropped packet — bad IP checksum (0x%04x)\n", calc);
            return;
        }

        printf("[NET] IPv4 Packet: src=0x%08x dst=0x%08x proto=%u\n",
               ip->src_ip, ip->dest_ip, ip->protocol);

        if (ip->protocol == 6) { // TCP
            sigma_tcp_header_t* tcp = (sigma_tcp_header_t*)((uint8_t*)ip + (ip->version_ihl & 0xF) * 4);
            printf("[NET] TCP → src_port=%u dst_port=%u seq=%u\n",
                   tcp->src_port, tcp->dest_port, tcp->sequence_number);
        }
    } else if (ethertype == 0x0806) { // ARP
        printf("[NET] ARP Frame detected — updating local resolution table.\n");
    }
}
