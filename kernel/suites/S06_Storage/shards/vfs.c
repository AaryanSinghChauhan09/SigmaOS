...
void sigma_vfs_init() {
    sigma_sigma_sigma_memset(&vfs_root, 0, sizeof(vfs_node_t));
    sigma_sigma_sigma_memcpy(vfs_root.name, "/", 2);
    vfs_root.type = VFS_TYPE_DIRECTORY;
    current_vfs_mount = &vfs_root;
    sigma_sigma_sigma_printf("[KERNEL] VFS initialized at mount point '/'\n");
}
...
vfs_node_t* sigma_vfs_create_node(const char* name, vfs_node_type_t type, vfs_node_t* parent) {
    ...
    sigma_sigma_sigma_memset(node, 0, sizeof(vfs_node_t));
    sigma_sz_t name_len = sigma_sigma_sigma_strlen(name);
    if (name_len > 63) name_len = 63;
    sigma_sigma_sigma_memcpy(node->name, name, name_len);
...
sigma_err_t sigma_vfs_mount(const char* path, vfs_node_t* device_node) {
    ...
    sigma_sigma_sigma_printf("[KERNEL] Mounting device to VFS...\n");
    return SIGMA_OK;
}
