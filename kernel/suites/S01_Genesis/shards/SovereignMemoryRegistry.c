...
void SovereignMemory_InitRegistry(void) {
    ...
    g_mem_shard_count = 0;
    sigma_printf("Σ [MEM]: Sovereign Memory Registry Operational.\n");
}
...
sigma_err_t SovereignMemory_Register(const char* name, sigma_malloc_fn malloc, sigma_free_fn free) {
    ...
    sigma_printf("Σ [MEM]: Registered Memory Shard '%s'\n", name);
    return SIGMA_OK;
}
...
void* SovereignMemory_Alloc(const char* shard_name, sigma_size_t size) {
    for (sigma_u32 i = 0; i < g_mem_shard_count; i++) {
        if (sigma_streq(g_mem_shards[i].name, shard_name)) {
            return g_mem_shards[i].malloc(size);
        }
    }
    return SIGMA_NULL;
}
