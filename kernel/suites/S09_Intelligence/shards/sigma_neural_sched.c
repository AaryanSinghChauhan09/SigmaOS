...
void sigma_neural_sched_init(void) {
    ...
    sigma_sigma_sigma_memset(&s_last_pred, 0, sizeof(s_last_pred));
    s_last_pred.freq_scale_pct = 100;
    sigma_neural_sched_load_defaults();
    sigma_sigma_sigma_printf("S [NEURAL] Resource Balancer initialized (Q8 perceptron)\n");
}
...
void sigma_neural_sched_apply(void) {
    ...
    sigma_sigma_sigma_printf("S [NEURAL] APPLY: sched_boost=%d reclaim=%u%% freq=%u%%\n",
                 p.sched_boost, p.mem_reclaim_pct, p.freq_scale_pct);

    if (p.cpu_pressure >= PRESSURE_HIGH)
        sigma_sigma_sigma_printf("S [NEURAL] ⚠ CPU PRESSURE HIGH — boosting CFS quantum\n");
    if (p.mem_pressure >= PRESSURE_HIGH)
        sigma_sigma_sigma_printf("S [NEURAL] ⚠ MEM PRESSURE HIGH — triggering reclaim %u%%\n",
                     p.mem_reclaim_pct);
    if (p.thermal_advice >= THERMAL_HOT)
        sigma_sigma_sigma_printf("S [NEURAL] 🌡 THERMAL HOT — scaling freq to %u%%\n",
                     p.freq_scale_pct);
}
...
void sigma_neural_sched_stats(void) {
    ...
    sigma_sigma_sigma_printf("  sched_boost:   %+d\n",    p.sched_boost);
    sigma_sigma_sigma_printf("  mem_reclaim:   %u%%\n",   p.mem_reclaim_pct);
    sigma_sigma_sigma_printf("  freq_scale:    %u%%\n",   p.freq_scale_pct);
