...
void agent_init(void) {
    ...
    sigma_sigma_sigma_printf("S [S09]: SigmaAgent Runtime Online (Idea 436).\n");
}

void agent_execute_step(void) {
    if (g_primary_agent.current_step >= g_primary_agent.plan_steps) {
        sigma_sigma_sigma_printf("S [AGENT]: Current Mission '%s' Accomplished.\n", g_primary_agent.current_mission);
        return;
    }
    ...
    sigma_sigma_sigma_printf("S [AGENT]: Executing Step %d/%d -> %s\n", 
...
void agent_dispatch_mission(const char* mission) {
    ...
    g_primary_agent.current_step = 0;
    sigma_sigma_sigma_printf("S [AGENT]: New Primary Mission Dispatched: %s\n", mission);
}
