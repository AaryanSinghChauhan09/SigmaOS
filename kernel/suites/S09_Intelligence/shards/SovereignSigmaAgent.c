...
void agent_execute_step(void) {
    ...
    sigma_printf("S [AGENT]: Executing Step %d/%d -> %s\n", 
...
void agent_dispatch_mission(const char* mission) {
    ...
    g_primary_agent.current_step = 0;
    sigma_printf("S [AGENT]: New Primary Mission Dispatched: %s\n", mission);
}
