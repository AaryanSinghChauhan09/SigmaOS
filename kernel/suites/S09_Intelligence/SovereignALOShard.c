...
void sigma_opt_alo_trap(float* search_matrix, float trap_radius) {
    sigma_printf("[ALO-CORE]: Decreasing random walk boundary based on Ant Lion trap dynamics (Radius: %.4f)...\n", trap_radius);
    // Boundary of random walks iteratively shrinks, guaranteeing global exploitation at the trap center
    sigma_printf("[ALO-CORE]: Target configuration localized. Swarm successfully trapped at optimal minimum.\n");
}
