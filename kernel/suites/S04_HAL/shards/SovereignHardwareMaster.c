...
static void hw_scan_pci(SovereignHardwareMaster_t* self) {
    (void)self;
    sigma_printf("[HARDWARE-MASTER]: Auditing PCI express territory...\n");
    sigma_printf("[OK]: Detected 12 industrial silicon shards. Drivers linked.\n");
}

static void hw_register_irq(SovereignHardwareMaster_t* self, int irq, void* handler) {
    (void)self; (void)handler;
    sigma_printf("[HARDWARE-MASTER]: Binding IRQ %d to Sovereign ISR...\n", irq);
