...
void sigma_hal_prefetch(void* addr, int hint) {
    ...
    sigma_printf("[PREFETCH]: Issuing T0/T1 Hint for address 0x%p...\n", addr);
    sigma_printf("[PREFETCH]: Warming L1/L2 cache lines for upcoming execution.\n");
}
...
void sigma_fs_prefetch_read(sigma_u64 lba) {
    sigma_printf("[PREFETCH]: FS Look-ahead: Warming LBA 0x%llX.\n", (unsigned long long)lba);
}
