...
void sched_init(void) {
    sigma_memset(g_proc_table, 0, sizeof(g_proc_table));
    sigma_memset(g_proc_ready_bitmap, 0, sizeof(g_proc_ready_bitmap));
    sigma_printf("S [S03]: O(1) Sovereign Scheduler Active. Target Latency: <10 cycles.\n");
}
...
void sched_dispatch(void) {
    ...
    sigma_printf("S [SCHED]: Context Switch -> Task 0x%x [%d cycles drift]\n", next, 5);
