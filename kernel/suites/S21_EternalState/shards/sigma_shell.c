...
sigma_err_t sigma_shell_execute(const char* line) {
    if (!line || sigma_strlen(line) == 0) return SIGMA_OK;
    ...
    if (sigma_streq(line, "ps")) {
        ...
        sigma_printf("  1    init        ACTIVE\n");
        sigma_printf("  10   gcd-worker  SLEEP\n");
    } else if (sigma_streq(line, "help")) {
        sigma_printf("  Available: cd, ps, kill, help, lattice-info, stats\n");
    } else {
        sigma_printf("  sigma-sh: command not found: %s\n", line);
        return SIGMA_ERROR;
    }
