...
void SovereignInit_InitRegistry(void) {
    ...
    g_svc_count = 0;
    sigma_printf("S [INIT]: Sovereign Service Registry Operational.\n");
}
...
sigma_err_t SovereignInit_RegisterService(const char* name, const char* path, sigma_bool restart, sigma_svc_init_fn init) {
    ...
    sigma_printf("S [INIT]: Registered Service Shard '%s' (Path: %s)\n", name, path);
    return SIGMA_OK;
}

void SovereignInit_StartAll(void) {
    sigma_printf("S [INIT]: Orchestrating Parallel Service Activation...\n");
    for (sigma_u32 i = 0; i < g_svc_count; i++) {
        sigma_printf("S [INIT]: Activating %s -> PID %d\n", g_services[i].name, 1000+i);
