...
void sigma_security_init(void) {
    ...
    domains[0].active     = 1;
    memcpy(domains[0].name, "kernel_root", 12);

    printf("[SEC] Sovereign Security Matrix initialized. SELinux/AppArmor containerized in Vault.\n");
}
...
void sigma_security_audit_log(uint32_t domain_id, const char* action) {
    ...
    snprintf(audit_ring[audit_head % AUDIT_ENTRIES], 127,
