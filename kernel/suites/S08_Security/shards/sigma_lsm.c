...
void sigma_lsm_init(void) {
    sigma_sigma_sigma_memset(s_ctxs, 0, sizeof(s_ctxs));
    sigma_sigma_sigma_printf("S [LSM] Security framework initialized\n");
    sigma_sigma_sigma_printf("S [LSM] Active policies: SELinux | AppArmor | Pledge | MIC | TCC\n");
}

void sigma_lsm_register_hooks(sigma_lsm_hooks_t *hooks) {
    s_hooks = hooks;
    sigma_sigma_sigma_printf("S [LSM] Hook table registered\n");
}
...
sigma_i32 sigma_lsm_ctx_create(sigma_u32 pid, const char *domain) {
    ...
    sigma_sigma_sigma_printf("S [LSM] CTX: pid=%u domain=%s\n", pid, domain);
    return LSM_ALLOW;
}
...
sigma_i32 sigma_lsm_pledge(sigma_u32 pid, sigma_u32 pledge_mask) {
    ...
    sigma_sigma_sigma_printf("S [LSM] PLEDGE: pid=%u mask=0x%x\n", pid, ctx->pledge_mask);
    return LSM_ALLOW;
}
...
void sigma_lsm_unveil(sigma_u32 pid, const char *path, const char *perms) {
    ...
    sigma_sigma_sigma_printf("S [LSM] UNVEIL: pid=%u path=%s perms=%s\n", pid, path, perms);
}
...
sigma_i32 sigma_lsm_check_file_open(sigma_u32 pid, const char *path, sigma_u32 flags) {
    ...
    if ((flags & 0x01) && !(ctx->pledge_mask & PLEDGE_WPATH)) {
        ...
        sigma_sigma_sigma_printf("S [LSM] DENY: pid=%u write-open '%s' (pledge WPATH)\n", pid, path);
        return LSM_DENY;
    }
...
sigma_i32 sigma_lsm_check_net(sigma_u32 pid, sigma_u32 dst_ip, sigma_u32 port) {
    ...
    if (!(ctx->pledge_mask & (PLEDGE_INET | PLEDGE_DNS))) {
        sigma_sigma_sigma_printf("S [LSM] DENY: pid=%u net dst=0x%x:%u (pledge INET)\n",
...
sigma_i32 sigma_lsm_check_cap(sigma_u32 pid, unsigned long long needed_cap) {
    ...
    sigma_sigma_sigma_printf("S [LSM] DENY: pid=%u missing cap 0x%llx\n",
...
void sigma_lsm_audit_dump(void) {
    ...
    for (sigma_u32 i = 0; i < count; i++) {
        ...
        sigma_sigma_sigma_printf("  pid=%-5u %-6s %s\n",
                     s_audit_log[idx].pid,
