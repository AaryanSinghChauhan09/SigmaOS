...
void sigma_tool_backup_shard(const char* src_shard, const char* dest_shard) {
    sigma_sigma_sigma_printf("[BACKUP]: Initiating metadata-preserving backup of shard '%s' to '%s'...\n", src_shard, dest_shard);
    ...
    if (fd_src < 0 || fd_dest < 0) {
        sigma_sigma_sigma_printf("[ERROR]: Failed to open shards for backup.\n");
        return;
    }
    ...
    sigma_close(fd_src);
    sigma_close(fd_dest);
    sigma_sigma_sigma_printf("[OK]: Backup mission complete for shard '%s'.\n", src_shard);
}
