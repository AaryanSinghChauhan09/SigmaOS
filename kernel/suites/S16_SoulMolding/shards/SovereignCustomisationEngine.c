...
sigma_err_t sigma_config_set(const char* key, const char* value, sigma_u32 flags) {
    ...
    for (sigma_u32 i = 0; i < s_config_count; i++) {
        if (sigma_streq(s_config_store[i].key, key)) {
            if (s_config_store[i].flags & 0x01) {
                sigma_printf("[CUSTOM-ENGINE]: Key '%s' is READ-ONLY.\n", key);
                return SIGMA_EPERM;
            }
            sigma_strncpy(s_config_store[i].value, value, 128);
            sigma_printf("[CUSTOM-ENGINE]: Updated '%s' = '%s'\n", key, value);
            return SIGMA_OK;
        }
    }
    ...
    sigma_printf("[CUSTOM-ENGINE]: Set '%s' = '%s' (flags: 0x%02X)\n",
                 key, value, flags);
    return SIGMA_OK;
}
...
void SovereignCustomisation_Init(void) {
    sigma_printf("[CUSTOM-ENGINE]: Initializing System Customisation Store...\n");
    ...
    sigma_printf("[CUSTOM-ENGINE]: %u default configs loaded.\n", s_config_count);
}
...
void SovereignCustomisation_Audit(void) {
    ...
    sigma_printf("%-30s %-30s %-6s\n", "KEY", "VALUE", "FLAGS");
    sigma_printf("-------------------------------------------------------------\n");
    for (sigma_u32 i = 0; i < s_config_count; i++) {
        sigma_printf("%-30s %-30s 0x%02X\n",
                     s_config_store[i].key,
                     s_config_store[i].value,
                     s_config_store[i].flags);
    }
    sigma_printf("-------------------------------------------------------------\n");
    sigma_printf("Total customisation entries: %u\n", s_config_count);
}
