...
sigma_err_t sigma_v4l2_register_device(SigmaV4L2Device_t *dev) {
    ...
    sigma_printf("S [V4L2]: Registered Video Device /dev/video%u ('%s')\n", dev->minor, dev->name);
    return SIGMA_OK;
}
...
sigma_err_t sigma_v4l2_vidioc_s_fmt(SigmaV4L2Device_t *dev, SigmaV4L2Format_t *f) {
    ...
    sigma_printf("S [V4L2]: VIDIOC_S_FMT requested: %ux%u (Format: %c%c%c%c)\n",
...
sigma_err_t sigma_v4l2_reqbufs(SigmaV4L2Device_t *dev, sigma_u32 count, sigma_u32 memory) {
    ...
    sigma_printf("S [V4L2]: Allocated %u buffers of %u bytes each.\n", count, frame_size);
    return SIGMA_OK;
}
...
sigma_err_t sigma_v4l2_streamon(SigmaV4L2Device_t *dev) {
    ...
    dev->sequence = 0;
    sigma_printf("S [V4L2]: Stream ON -> Video capture active.\n");
    return SIGMA_OK;
}
...
static sigma_err_t mock_uvc_start(SigmaV4L2Device_t *dev) {
    ...
    sigma_printf("S [V4L2-UVC]: UVC Driver started isochronous bandwidth allocation.\n");
    return SIGMA_OK;
}
...
void SovereignMediaV4L2_Init(void) {
    sigma_printf("S [V4L2]: Initialising Sovereign Video4Linux2 Media Core...\n");
    ...
    sigma_printf("S [V4L2]: Media framing online. Optical sensor sovereignty established.\n");
}
