...
void sigma_audio_init(void) {
    sigma_memset(s_streams,  0, sizeof(s_streams));
    sigma_memset(s_controls, 0, sizeof(s_controls));
...
au_i32 sigma_au_write(au_u32 sid, const void *buf, au_u32 frames) {
    ...
    sigma_memcpy(s->buf + (s->buf_tail % cap), buf, bytes);
    s->buf_tail       += bytes;
...
au_i32 sigma_au_read(au_u32 sid, void *buf, au_u32 frames) {
    ...
    sigma_memcpy(buf, s->buf + (s->buf_head % cap), bytes);
    s->buf_head     += bytes;
...
void sigma_audio_stats(void) {
    ...
    for (au_u32 i = 0; i < s_stream_count; i++) {
        ...
        sigma_printf("  %-4u %-8s %-10s %-7u %-6s %-10llu %llu\n",
