/*
 * =========================================================================
 * S SIGMAOS kernel/suites/S18_USB/shards/sigma_usb.c
 * =========================================================================
 */
...
void sigma_usb_init(void) {
    ...
    sigma_printf("S [USB] Sovereign USB subsystem initialized\n");
    sigma_printf("S [USB] xHCI host | USB4/TB4 | Gadget mode | ADB parity\n");
}
...
usb_i32 sigma_usb_enumerate(usb_u32 port) {
    ...
    sigma_printf("S [USB] ENUMERATE port=%u: %s [%04x:%04x] addr=%u %s\n",
...
void sigma_usb_disconnect(usb_u32 dev_id) {
    for (usb_u32 i = 0; i < s_dev_count; i++) {
        if (s_devices[i].dev_id == dev_id) {
            sigma_printf("S [USB] DISCONNECT: %s (id=%u addr=%u)\n",
...
usb_i32 sigma_usb_control_msg(usb_u32 dev_id, usb_u8 request_type,
    ...
    sigma_printf("S [USB] CTRL: dev=%u type=0x%02x req=0x%02x val=%u len=%u\n",
...
usb_i32 sigma_usb_bulk_transfer(usb_u32 dev_id, usb_u8 ep,
    ...
    sigma_usb_dev_t *d = find_dev(dev_id);
    if (!d || d->suspended) return USB_ERR;
    sigma_printf("S [USB] BULK: dev=%u ep=0x%02x len=%u\n", dev_id, ep, len);
    (void)buf;
    return (usb_i32)len;
}
...
usb_i32 sigma_usb_interrupt_transfer(usb_u32 dev_id, usb_u8 ep,
    ...
    sigma_usb_dev_t *d = find_dev(dev_id);
    if (!d) return USB_ERR;
    sigma_printf("S [USB] INT: dev=%u ep=0x%02x len=%u\n", dev_id, ep, len);
    (void)buf;
    return (usb_i32)len;
}
...
usb_i32 sigma_usb_gadget_init(sigma_usb_class_t gadget_class) {
    ...
    sigma_printf("S [USB] Gadget mode: class=0x%02x (ADB/CDC parity)\n",
                 gadget_class);
    return USB_OK;
}
...
usb_i32 sigma_usb_gadget_send(const void *buf, usb_u32 len) {
    ...
    sigma_printf("S [USB] Gadget TX: %u bytes\n", len);
    (void)buf;
    return (usb_i32)len;
}
...
void sigma_usb_stats(void) {
    sigma_printf("\nS USB SUBSYSTEM\n");
    sigma_printf("  Devices: %u   Gadget: %s\n",
