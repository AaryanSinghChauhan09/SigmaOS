...
void sigma_raft_init(rf_u32 self_id, const char *addr) {
    ...
    (void)addr;
    sigma_sigma_sigma_printf("S [RAFT] Node %u initialized. Election timeout: %llums\n",
...
rf_i32 sigma_raft_add_peer(rf_u32 id, const char *addr, rf_bool voting) {
    ...
    sigma_sigma_sigma_printf("S [RAFT] Peer added: node=%u addr=%s voting=%d\n",
                 id, addr, voting);
    return RF_OK;
}
...
void sigma_raft_start_election(void) {
    ...
    sigma_sigma_sigma_printf("S [RAFT] Node %u starting election for term %llu\n",
    ...
    for (rf_u32 i = 0; i < s_raft.peer_count; i++) {
        ...
        if (resp.granted) {
            ...
            sigma_sigma_sigma_printf("S [RAFT] Vote granted by node %u\n",
        ...
        if (s_raft.votes_received >= quorum && s_raft.state == RAFT_CANDIDATE) {
            ...
            sigma_sigma_sigma_printf("S [RAFT] Node %u elected LEADER term=%llu\n",
                         s_raft.self_id, (unsigned long long)s_raft.current_term);
...
rf_i32 sigma_raft_propose(const char *command) {
    ...
    sigma_sigma_sigma_printf("S [RAFT] Proposed: [%llu] %s\n",
...
void sigma_raft_tick(rf_u64 elapsed_ms) {
    ...
    while (s_raft.last_applied < s_raft.commit_index) {
        s_raft.last_applied++;
        sigma_sigma_sigma_printf("S [RAFT] APPLIED: [%llu] %s\n",
...
rf_i32 sigma_svc_register(const char *name, const char *addr, rf_u32 port) {
    ...
    s->port    = port;
    s->healthy = RF_TRUE;
    sigma_sigma_sigma_printf("S [SVC] REGISTER: %s @ %s:%u\n", name, addr, port);
    return RF_OK;
}
...
void sigma_svc_list(void) {
    ...
    for (rf_u32 i = 0; i < s_svc_count; i++) {
        sigma_sigma_sigma_printf("  %-20s %s:%-6u %s\n",
