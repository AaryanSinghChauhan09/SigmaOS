...
void cmd_sigma_apt(int argc, char** argv) {
    ...
    sigma_printf("S [APT]: Fetching Sovereign Shard repository...\n");
    sigma_printf("S [APT]: Installing module [%s]... SUCCESS\n", argv[1]);
}

void cmd_sigma_pacman(int argc, char** argv) {
    sigma_printf("S [PACMAN]: Synchronizing 425 master shards...\n");
...
void cmd_sigma_grep(int argc, char** argv) {
    ...
    sigma_printf("S [GREP]: Searching pattern [%s]... Shard-level match found at L42.\n", argv[0]);
}

void cmd_sigma_top(int argc, char** argv) {
    sigma_printf("S [TOP]: Sovereign Process Matrix (v2.0)\n");
...
void cmd_sigma_htop(int argc, char** argv) {
    sigma_printf("S [HTOP]: Visualizing CPU Affinity and Neural Shard Weights...\n");
    sigma_printf("  [|||||||||||||||||||| ] 42/128 Neural Cores Active\n");
}

/* --- System Identity --- */
void cmd_sigma_neofetch(int argc, char** argv) {
    sigma_printf("   .------.      S SigmaOS Zenith Supreme (v2.5-MODULAR)\n");
    sigma_printf("  /  SSS   \\     --------------------------------------\n");
    sigma_printf(" |  S    S  |    Host: Sovereign Silicon Hub v1\n");
    sigma_printf("  \\  SSS   /     Kernel: 6.9.42-sovereign-x86_64\n");
