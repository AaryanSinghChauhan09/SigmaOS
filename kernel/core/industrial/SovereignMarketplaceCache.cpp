...
public:
    ...
    bool fetchFromMirror(const char* orb_name) {
...
extern "C" void marketplace_cache_init() {
    SigmaOS::Kernel::Industrial::SovereignMarketplaceCache::getInstance().refreshCache();
}

extern "C" int marketplace_fetch_fallback(const char* name) {
    return SigmaOS::Kernel::Industrial::SovereignMarketplaceCache::getInstance().fetchFromMirror(name) ? 1 : 0;
}
