...
public:
    ...
    bool load(const char* module_path) {
        ...
        sigma_log_info("[DYNMODULE] AHSL: Resolving symbols for module at '%s'...\n", module_path);
        ...
        sigma_log_info("[DYNMODULE] AHSL: Plugin %d ('%s') successfully linked to lattice.\n", 
    ...
    bool unload(sigma_u32 module_id) {
        ...
        if (mod->is_loaded) {
            sigma_log_info("[DYNMODULE] AHSL: Unloading module %d and clearing memory...\n", (int)module_id);
            mod->is_loaded = false;
            return true;
        }
        return false;
