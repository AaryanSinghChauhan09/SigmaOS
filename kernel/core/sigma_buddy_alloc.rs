...
/// VMM integration: Allocate pages for page tables (BUG-001 Fix)
/// This function is called by VMM to allocate page table pages
#[no_mangle]
pub unsafe extern "C" fn sigma_buddy_alloc_for_vmm(order: U32) -> U64 {
    let pfn = sigma_buddy_alloc(order);
    if pfn == U32::MAX {
        return 0;
    }
    sigma_buddy_pfn_to_phys(pfn)
}

/// VMM integration: Free pages used for page tables (BUG-001 Fix)
#[no_mangle]
pub unsafe extern "C" fn sigma_buddy_free_for_vmm(phys_addr: U64, order: U32) {
    let pfn = sigma_buddy_phys_to_pfn(phys_addr);
    sigma_buddy_free(pfn, order);
}
