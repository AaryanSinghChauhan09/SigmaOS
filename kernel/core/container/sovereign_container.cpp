...
void SovereignContainer::Launch() {
    sigma_printf("[CONTAINER]: Igniting Bare-Metal Shard Isolation for ID: %d\n", m_container_id);
    sigma_printf("[CONTAINER]: Image Shard: %s | Quota: %llu MB\n", m_image_shard, m_memory_limit / (1024*1024));
    m_active = SIGMA_TRUE;
}

void SovereignContainer::Terminate() {
    sigma_printf("[CONTAINER]: Extinguishing Shard ID: %d | Reclaiming Silicon Shards...\n", m_container_id);
    m_active = SIGMA_FALSE;
}

void SovereignContainer::Audit() {
    sigma_printf("| Container [%d]: %-15s | Status: %s | Memory: %llu MB\n", 
