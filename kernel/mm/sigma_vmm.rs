...
pub unsafe extern "C" fn sigma_vmm_map(vaddr: U64, phys_addr: U64, flags: U64) -> i32 {
    ...
    if !PML4[0].entries[pml4_idx].is_present() {
        ...
        // BUG-001 Fix: Use buddy allocator for page table allocation
        extern "C" {
            fn sigma_buddy_alloc_for_vmm(order: u32) -> U64;
        }
        let pdpt_phys = sigma_buddy_alloc_for_vmm(0);
    ...
    if !pdpt.entries[pdpt_idx].is_present() {
        ...
        let pd_phys = sigma_buddy_alloc_for_vmm(0);
        if pd_phys == 0 { return -2; }
    ...
    if !pd.entries[pd_idx].is_present() {
        ...
        let pt_phys = sigma_buddy_alloc_for_vmm(0);
        if pt_phys == 0 { return -3; }
