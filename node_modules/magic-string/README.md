...
The fourth argument is optional. It can have a `storeName` property â€” if `true`, the original name will be stored for later inclusion in a sourcemap's `names` array â€” and an `overwrite` property which defaults to `false` and determines whether anything that was appended/prepended to the range will be overwritten along with the original content.
