...
Returns the full domain â€” the subdomain together with the registrable domain (as
returned by `getDomain(...)`), i.e. the whole hostname _including_ any subdomain â€”
or `null` when the input has no registrable domain (IP address, single label,
bare public suffix, â€¦). The result is the normalized hostname (lower-cased,
