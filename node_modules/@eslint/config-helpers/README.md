...
- **`gitignoreResolution`** (`boolean`): Controls how ignore patterns are interpreted.
    - `false` (default) â€” patterns are resolved relative to the location of the configuration file.
    - `true` â€” patterns are resolved relative to the location of the ignore file, matching the behavior of `.gitignore` files.
