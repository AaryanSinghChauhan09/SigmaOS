...
- ðŸš€ Native ESM compatibility


- ðŸŒ Optimized for modern runtimes
  - ES2015+ browsers
...
The `*` character is a wildcard. Suppose your library has debuggers named `connect:bodyParser`, `connect:compress`, and `connect:session` â€” instead of listing each one, use `DEBUG=connect:*`. Use `DEBUG=*` to enable everything.
...
Other format specifiers supported by Node's `util.format` (`%s`, `%d`, `%j`, â€¦) and the browser console pass through to the underlying logger.
