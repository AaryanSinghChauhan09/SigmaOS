...
By default, tinyexec does not throw on nonâ€‘zero exit codes. Check `result.exitCode` or pass `{throwOnError: true}`.
...
You can pass a string to `stdin`, which is useful for whitespace-sensitive values and for secrets that shouldnâ€™t be exposed in shell history:
...
The underlying Node.js `ChildProcess`. tinyexec keeps the surface minimal and does not reâ€‘expose every child_process method/event. Use `proc.process` for advanced access (streams, events, etc.).
