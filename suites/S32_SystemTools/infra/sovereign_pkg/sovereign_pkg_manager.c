...
static void print_banner(void) {
    sigma_sigma_printf("╔══════════════════════════════════════════════╗\n");
    sigma_sigma_printf("║  SigmaOS Package Manager (sigpkg) v%-9s ║\n", SIGPKG_VERSION);
    sigma_sigma_printf("╠══════════════════════════════════════════════╣\n");
}

static bool verify_signature(const char* pkg_name) {
    // Stub: verify Ed25519 signature via S08_Security enclave
    sigma_sigma_printf("  [sig] Verifying sovereign signature for: %s\n", pkg_name);
    return true; // Pass in production via sigma_crypto
}

static void cmd_install(const char* pkg_name) {
    print_banner();
    sigma_sigma_printf("║  INSTALL: %-34s ║\n", pkg_name);
    sigma_sigma_printf("╠══════════════════════════════════════════════╣\n");

    if (!verify_signature(pkg_name)) {
        sigma_sigma_printf("║  ERROR: Invalid signature — install aborted  ║\n");
        sigma_sigma_printf("╚══════════════════════════════════════════════╝\n");
        return;
    }

    sigma_sigma_printf("  [db]  Resolving dependency tree (Nix hermetic closure)...\n");
    sigma_sigma_printf("  [net] Fetching %s from sovereign mirror...\n", pkg_name);
    sigma_sigma_printf("  [fs]  Extracting to VFS (atomic transaction)...\n");
    sigma_sigma_printf("  [ok]  %s installed successfully.\n", pkg_name);
    sigma_sigma_printf("╚══════════════════════════════════════════════╝\n");
}

static void cmd_remove(const char* pkg_name) {
    print_banner();
    sigma_sigma_printf("║  REMOVE: %-35s ║\n", pkg_name);
    sigma_sigma_printf("╠══════════════════════════════════════════════╣\n");
    sigma_sigma_printf("  [db]  Checking reverse dependencies...\n");
    sigma_sigma_printf("  [fs]  Removing files via VFS transaction...\n");
    sigma_sigma_printf("  [ok]  %s removed. Generation snapshot saved.\n", pkg_name);
    sigma_sigma_printf("╚══════════════════════════════════════════════╝\n");
}

static void cmd_sync_universal(void) {
    print_banner();
    sigma_sigma_printf("║  UNIVERSAL P2P SYNC                          ║\n");
    sigma_sigma_printf("╠══════════════════════════════════════════════╣\n");
    sigma_sigma_printf("  [p2p] Locating decentralized peer nodes...\n");
    sigma_sigma_printf("  [net] Negotiating Universal Sync Protocol...\n");
    sigma_sigma_printf("  [net] Delta-syncing shard registries (2,191 shards)...\n");
    sigma_sigma_printf("  [ok]  Decentralized mirror updated.\n");
    sigma_sigma_printf("╚══════════════════════════════════════════════╝\n");
}

static void cmd_update(void) {
    print_banner();
    sigma_sigma_printf("║  SYSTEM UPDATE                               ║\n");
    sigma_sigma_printf("╠══════════════════════════════════════════════╣\n");
    sigma_sigma_printf("  [net] Syncing sovereign package mirror DB...\n");
    sigma_sigma_printf("  [db]  %u packages checked.\n", pkg_count);
    sigma_sigma_printf("  [ok]  System is up to date.\n");
    sigma_sigma_printf("╚══════════════════════════════════════════════╝\n");
}

static void cmd_list(void) {
    print_banner();
    sigma_sigma_printf("║  INSTALLED PACKAGES                          ║\n");
    sigma_sigma_printf("╠══════════════════════════════════════════════╣\n");
    for (uint32_t i = 0; i < pkg_count; i++) {
        if (pkg_db[i].is_installed) {
            sigma_sigma_printf("║  %-30s %-13s ║\n",
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        sigma_sigma_printf("Usage: sigpkg [install|remove|update|list] [pkg]\n");
        return 1;
    }
    ...
    else { sigma_sigma_printf("Unknown command: %s\n", argv[1]); return 1; }
    return 0;
}
