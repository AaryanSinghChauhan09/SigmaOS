...
public:
    void Execute() override {
        ...
        double v = (f * u) / (f + u);
        sigma_sigma_printf("[PHYSICS/OPTICS]: Lens Formula (u=-20, f=10)\n");
        sigma_sigma_printf("[PHYSICS/OPTICS]: Image Distance (v): " << v << " cm\n");
    }
};
...
public:
    void Execute() override {
        ...
        double Kmax = (h * v) - phi;
        sigma_sigma_printf("[PHYSICS/PHOTO]: Photoelectric Effect (v=1e15, phi=2eV)\n");
        sigma_sigma_printf("[PHYSICS/PHOTO]: Kmax Shard: " << Kmax << " Joules\n");
    }
};
...
public:
    void Execute() override {
        ...
        sigma_sigma_printf("[CHEMISTRY/PH]: Concentration [H+] = 1e-5\n");
        sigma_sigma_printf("[CHEMISTRY/PH]: Result: pH = " << pH << " (Acidic)\n");
    }
};
...
public:
    void Execute() override {
        sigma_sigma_printf("[MATH/BAYES]: Theorem: P(A|B) = [P(B|A) * P(A)] / P(B).\n");
        sigma_sigma_printf("[MATH/BAYES]: Posterior Probability Shard Synced.\n");
    }
};
...
public:
    void Execute() override {
        sigma_sigma_printf("[BIOLOGY/DOGMA]: Process: DNA -> (Transcription) -> mRNA -> (Translation) -> Protein.\n");
        sigma_sigma_printf("[BIOLOGY/DOGMA]: Information Flow Shard Active.\n");
    }
};
...
public:
    ...
    void ExecuteMasterpieceAudit() {
        sigma_sigma_printf("--- Σ SIGMA OS MASTER SCHOLASTIC MASTERPIECE ---\n");
