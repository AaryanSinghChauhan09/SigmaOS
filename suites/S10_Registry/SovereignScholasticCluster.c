...
public:
    void Execute() override {
        sigma_sigma_printf("[PHYSICS/EXP]: Experiment: Photoelectric Effect (Einstein's Law).\n");
        sigma_sigma_printf("[PHYSICS/EXP]: K_max = hf - Phi. Electron emission verified for f > f_0.\n");
    }
};
...
public:
    void Execute() override {
        sigma_sigma_printf("[PHYSICS/EXP]: Experiment: Verification of AND/OR/NOT Truth Tables.\n");
        sigma_sigma_printf("[PHYSICS/EXP]: Input (1,0) -> AND (0), OR (1) Shard synchronized.\n");
    }
};
...
public:
    void Execute() override {
        sigma_sigma_printf("[BIOLOGY/EXP]: Experiment: Meselson-Stahl Semi-Conservative Replication.\n");
        sigma_sigma_printf("[BIOLOGY/EXP]: 14N/15N Density gradients identified throughout Shard cycles.\n");
    }
};
...
public:
    void Execute() override {
        sigma_sigma_printf("[BIOLOGY/EXP]: Experiment: Reflex Arc (Stimulus to Response).\n");
        sigma_sigma_printf("[BIOLOGY/EXP]: Sensory -> Relay -> Motor Shard pulse: 0.05ms latency.\n");
    }
};
...
public:
    void Execute() override {
        ...
        sigma_sigma_printf("[MATH/EXP]: Experiment: Finding Roots of a Quadratic Shard.\n");
        sigma_sigma_printf("[MATH/EXP]: x^2 - 5x + 6 = 0 -> Roots: " << x1 << ", " << x2 << " [OK].\n");
    }
};
...
public:
    ...
    void ExecuteFinalAudit() {
        sigma_sigma_printf("--- Σ SIGMA OS SOVEREIGN SCHOLASTIC CLUSTER ---\n");
