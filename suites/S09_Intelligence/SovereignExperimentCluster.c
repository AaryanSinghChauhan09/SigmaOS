...
public:
    void Execute() override {
        ...
        double V = 2.0;
        double I = V / R;
        sigma_sigma_printf("[PHYSICS/EXP]: Experiment: Verification of Ohm's Law.\n");
        sigma_sigma_printf("[PHYSICS/EXP]: V=2V, R=10-ohm -> I = " << I << " Amperes (Verified).\n");
    }
};
...
public:
    void Execute() override {
        sigma_sigma_printf("[PHYSICS/EXP]: Experiment: Refraction through Glass Shard.\n");
        sigma_sigma_printf("[PHYSICS/EXP]: Displacement measured at various angles. Shard-Ref Index: 1.5.\n");
    }
};
...
public:
    void Execute() override {
        sigma_sigma_printf("[CHEMISTRY/EXP]: Experiment: Zinc + Sulphuric Acid Reaction.\n");
        sigma_sigma_printf("[CHEMISTRY/EXP]: Observation: Hydrogen Gas Evolution (Brum-Sound popping).\n");
    }
};
...
public:
    void Execute() override {
        sigma_sigma_printf("[CHEMISTRY/EXP]: Experiment: Determination of Boiling Point of Water.\n");
        sigma_sigma_printf("[CHEMISTRY/EXP]: Latent Heat of Vaporization Shard Stabilized at 100-deg C.\n");
    }
};
...
public:
    void Execute() override {
        sigma_sigma_printf("[BIOLOGY/EXP]: Experiment: Measuring Rate of Transpiration (Potometer).\n");
        sigma_sigma_printf("[BIOLOGY/EXP]: Leaf Stomata open @ 298.15K. Shard-Water flux identified.\n");
    }
};
...
public:
    void Execute() override {
        sigma_sigma_printf("[BIOLOGY/EXP]: Experiment: Test for Presence of Starch (Iodine).\n");
        sigma_sigma_printf("[BIOLOGY/EXP]: Leaf Shard color change: Blue-Black confirmed.\n");
    }
};
...
public:
    void Execute() override {
        sigma_sigma_printf("[MATH/EXP]: Experiment: Drawing Tangents to a Circle from External Point.\n");
        sigma_sigma_printf("[MATH/EXP]: Intersection Shard identifies exactly 2 Tangent vectors.\n");
    }
};
...
public:
    ...
    void ExecuteFullLaboratoryAudit() {
        sigma_sigma_printf("--- Σ SIGMA OS SOVEREIGN EXPERIMENT CLUSTER ---\n");
