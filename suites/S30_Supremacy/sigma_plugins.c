...
sigma_bool sigma_plugin_load(const char* name, void (*on_load)(), void (*on_unload)()) {
    ...
    sigma_sigma_printf("[PLUGIN] Sharding Symbolic Link for: %s... ", name);
    sigma_sigma_memcpy(plugin_shards[plugin_count].name, name, sigma_sigma_strlen(name));
...
void sigma_plugin_list() {
    ...
    for (sigma_u32 i = 0; i < plugin_count; i++) {
        sigma_sigma_printf("[%d] %-15s v%d.%d  %s\n", 
