...
typedef struct TaskShard {
    sigma_u64         task_id;
    sigma_u32         target_node_id;
    TaskStatus  status;
    sigma_u8          payload[TASK_BUFFER_SIZE];
} TaskShard;

typedef struct NodeRegistry {
    sigma_u32         node_id;
    sigma_u32         cpu_count;
    sigma_u64         memory_free;
    bool        active;
} NodeRegistry;

static NodeRegistry g_node_table[MAX_NODES];
static sigma_u32 g_active_nodes = 0;
...
sigma_status dist_register_node(sigma_u32 remote_id, sigma_u32 cpus, sigma_u64 mem) {
...
sigma_status dist_delegate_task(sigma_u64 task_id, const void* data, sigma_u32 len) {
    /* Absorb Moltbot-USP: Autonomous offloading based on silicon debt. */
    sigma_u32 best_node = 0;
    sigma_u64 max_mem = 0;
    
    for (sigma_u32 i = 0; i < g_active_nodes; i++) {
