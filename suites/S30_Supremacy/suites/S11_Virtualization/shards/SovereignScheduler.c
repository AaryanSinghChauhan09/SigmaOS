...
void sigma_scheduler_spawn(const char* name, int priority, sigma_u32 affinity) {
    ...
    sigma_sigma_printf("  [SCHED]: Spawned task [%s] (TID: %d) on CoreMask: 0x%X\n", name, t->tid, affinity);
}

void SovereignScheduler_Init(void) {
    sigma_sigma_printf("S [SCHEDULER]: Initialising Quantum Affinity Engine...\n");
    ...
    sigma_sigma_printf("S [SCHEDULER]: Multi-queue dispatcher ACTIVE.\n");
}
