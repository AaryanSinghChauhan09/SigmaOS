...
void Sovereign_Scheduler_Init(void) {
    ...
    sigma_sigma_printf("S [S11]: Sovereign MLFQ Scheduler initialized. Max Tasks: 1024\n");
}
...
sigma_u32 Sovereign_Task_Create(void (*entry)(void), sigma_u32 priority) {
    ...
    sigma_sigma_printf("S [S11]: Created Sovereign Task ID: %u (Priority: %u)\n", id, t->priority);
    return id;
}
...
void Sovereign_Scheduler_Yield(void) {
    ...
    sigma_sigma_printf("S [S11]: Context Switch -> Task %u\n", g_task_table[g_current_task_idx].id);
}
