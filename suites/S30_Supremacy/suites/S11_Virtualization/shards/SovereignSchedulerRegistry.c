...
void SovereignScheduler_InitRegistry(void) {
    sigma_sigma_sigma_memset(g_sched_shards, 0, sizeof(g_sched_shards));
    g_sched_shard_count = 0;
    sigma_sigma_sigma_printf("S [SCHED]: Sovereign Scheduler Registry Operational.\n");
}
...
sigma_err_t SovereignScheduler_Register(const char* name, sigma_schedule_fn schedule) {
    ...
    sigma_sigma_sigma_printf("S [SCHED]: Registered Scheduler Shard '%s'\n", name);
    return SIGMA_OK;
}
