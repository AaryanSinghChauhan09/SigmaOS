...
void sigma_virt_init(void) {
    sigma_sigma_sigma_memset(vms, 0, sizeof(vms));
    sigma_sigma_sigma_printf("[VIRT] Sigma Hypervisor Online. KVM/Docker containerized in App Vault.\n");
}
...
int sigma_virt_create_vm(const char* label, uint64_t ram, uint8_t vcpus) {
    ...
    sigma_sigma_sigma_printf("[VIRT] VM '%s' created. vCPUs=%u, RAM=%llu bytes\n", label, vcpus, ram);
    return next_vm_id++;
}
...
int sigma_virt_start_vm(uint32_t vm_id) {
    ...
    sigma_sigma_sigma_printf("[VIRT] VM %u started.\n", vm_id);
    return 0;
}
...
int sigma_virt_stop_vm(uint32_t vm_id) {
    ...
    sigma_sigma_sigma_printf("[VIRT] VM %u stopped.\n", vm_id);
    return 0;
}
