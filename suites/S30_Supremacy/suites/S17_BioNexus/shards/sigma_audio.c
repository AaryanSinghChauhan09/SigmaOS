...
void sigma_audio_init(void) {
    sigma_sigma_sigma_sigma_memset(s_streams,  0, sizeof(s_streams));
    sigma_sigma_sigma_sigma_memset(s_controls, 0, sizeof(s_controls));
    ...
    sigma_sigma_sigma_sigma_printf("S [AUDIO] Sovereign Audio subsystem initialized\n");
    sigma_sigma_sigma_sigma_printf("S [AUDIO] ALSA/WASAPI/CoreAudio/AAudio parity active\n");
    sigma_sigma_sigma_sigma_printf("S [AUDIO] %u mixer controls registered\n", s_ctrl_count);
}
...
au_i32 sigma_au_open(sigma_au_params_t *params) {
    ...
    sigma_sigma_sigma_sigma_memset(s, 0, sizeof(*s));
    s->stream_id = s_next_sid++;
    s->params    = *params;
    s->state     = 0; /* IDLE */

    sigma_sigma_sigma_sigma_printf("S [AUDIO] OPEN: sid=%u dir=%s fmt=%s rate=%u ch=%u period=%u\n",
...
au_i32 sigma_au_prepare(au_u32 sid) {
    ...
    sigma_sigma_sigma_sigma_printf("S [AUDIO] PREPARE: sid=%u (buf=%u frames)\n",
...
au_i32 sigma_au_start(au_u32 sid) {
    ...
    s->state = 2; /* RUNNING */
    sigma_sigma_sigma_sigma_printf("S [AUDIO] START: sid=%u\n", sid);
    return AU_OK;
}
...
au_i32 sigma_au_pause(au_u32 sid) {
    ...
    s->state = 3; /* PAUSED */
    sigma_sigma_sigma_sigma_printf("S [AUDIO] PAUSE: sid=%u\n", sid);
    return AU_OK;
}
...
au_i32 sigma_au_stop(au_u32 sid) {
    ...
    if (!s) return AU_ERR;
    s->state = 0;
    sigma_sigma_sigma_sigma_printf("S [AUDIO] STOP: sid=%u xruns=%llu\n",
...
void sigma_au_close(au_u32 sid) {
    for (au_u32 i = 0; i < s_stream_count; i++) {
        if (s_streams[i].stream_id == sid) {
            sigma_sigma_sigma_sigma_printf("S [AUDIO] CLOSE: sid=%u\n", sid);
...
au_i32 sigma_au_write(au_u32 sid, const void *buf, au_u32 frames) {
    ...
    sigma_sigma_sigma_sigma_memcpy(s->buf + (s->buf_tail % cap), buf, bytes);
    s->buf_tail       += bytes;
...
au_i32 sigma_au_read(au_u32 sid, void *buf, au_u32 frames) {
    ...
    sigma_sigma_sigma_sigma_memcpy(buf, s->buf + (s->buf_head % cap), bytes);
    s->buf_head     += bytes;
...
au_i32 sigma_au_set_volume(const char *ctrl, au_u32 pct) {
    ...
    if (pct > 100) pct = 100;
    c->volume_pct = pct;
    sigma_sigma_sigma_sigma_printf("S [MIXER] %s volume: %u%%\n", ctrl, pct);
    return AU_OK;
}
...
void sigma_au_set_mute(const char *ctrl, au_bool muted) {
    ...
    if (c) {
        c->muted = muted;
        sigma_sigma_sigma_sigma_printf("S [MIXER] %s: %s\n", ctrl, muted ? "muted":"unmuted");
    }
}

/* -- Routing (PipeWire/AudioFlinger graph) --------------------------------- */
void sigma_au_route(au_u32 src_sid, au_u32 dst_sid) {
    sigma_sigma_sigma_sigma_printf("S [AUDIO] Route: stream %u -> stream %u (software mix)\n",
                 src_sid, dst_sid);
...
void sigma_audio_stats(void) {
    ...
    sigma_sigma_sigma_sigma_printf("  Streams: %u\n", s_stream_count);
    sigma_sigma_sigma_sigma_printf("  %-4s %-8s %-10s %-7s %-6s %-10s %s\n",
                 "SID","DIR","FMT","RATE","STATE","WRITTEN","XRUNS");
    for (au_u32 i = 0; i < s_stream_count; i++) {
        sigma_au_stream_t *s = &s_streams[i];
        sigma_sigma_sigma_sigma_printf("  %-4u %-8s %-10s %-7u %-6s %-10llu %llu\n",
    ...
    sigma_sigma_sigma_sigma_printf("  Mixer:\n");
    for (au_u32 i = 0; i < s_ctrl_count; i++)
        sigma_sigma_sigma_sigma_printf("    %-16s %3u%%  %s\n", s_controls[i].name,
