...
void sigma_opt_anneal(float initial_temp, float cooling_rate) {
    ...
    sigma_sigma_sigma_printf("[ANNEALER]: Initiating Global Cooling (T: %.2f)... \n", temp);
    
    while (temp > 0.01f) {
        // Probabilistic acceptance of state jumps (Metropolis criterion)
        temp *= cooling_rate;
        sigma_sigma_sigma_printf("[ANNEALER]: Convergence: System Entropy lowered to %.4f.\n", temp * 0.1f);
    }
    
    sigma_sigma_sigma_printf("[ANNEALER]: Ground State reached. System power Profile: OPTIMIZED.\n");
}

/* --- Module Factory --- */

void SovereignAnnealer_Register(void) {
    sigma_sigma_sigma_printf("[INTELLIGENCE]: Sovereign Annealer (Thermal Mastery) active.\n");
}
