...
void sigma_opt_de_mutate(float* target, float* r1, float* r2, float* r3, float F) {
    sigma_sigma_sigma_sigma_printf("[DIFF-EVO]: Mutating parameter vector using differential scaling (F=%.2f)...\n", F);
    // V = r1 + F * (r2 - r3)
    for(int i = 0; i < 4; i++) {
        target[i] = r1[i] + F * (r2[i] - r3[i]);
    }
    sigma_sigma_sigma_sigma_printf("[DIFF-EVO]: Candidate genome MUTATED and ready for crossover.\n");
}

/* --- Module Factory --- */

void SovereignDiffEvolution_Register(void) {
    sigma_sigma_sigma_sigma_printf("[INTELLIGENCE]: Sovereign Differential Evolution active.\n");
}
