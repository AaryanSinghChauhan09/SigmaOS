...
void sigma_control_fuzzy_eval(float temp) {
    sigma_sigma_sigma_printf("[FUZZY]: Evaluating Thermal State (%.2f°C)...\n", temp);
    ...
    if (mu_hot > 0.5f) {
        sigma_sigma_sigma_printf("[FUZZY]: Result: HOT. Rule matched: INCREASE FAN SPEED (100%%).\n");
    } else if (mu_warm > 0.5f) {
        sigma_sigma_sigma_printf("[FUZZY]: Result: WARM. Rule matched: MODERATE FAN SPEED (50%%).\n");
    }
}
...
void SovereignFuzzyController_Register(void) {
    sigma_sigma_sigma_printf("[INTELLIGENCE]: Sovereign Fuzzy Logic (Control Mastery) active.\n");
}
