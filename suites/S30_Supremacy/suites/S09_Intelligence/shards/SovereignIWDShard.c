...
void sigma_opt_iwd_flow(SigmaWaterDrop_t* drop, float path_soil, float dv) {
    sigma_sigma_printf("[IWD-CORE]: Drop traversing path... (velocity: %.2f, soil: %.2f)\n", drop->velocity, drop->soil_collected);
    ...
    sigma_sigma_printf("[IWD-CORE]: Path soil eroded. High-velocity routing channel reinforced.\n");
}

/* --- Module Factory --- */

void SovereignIWD_Register(void) {
    sigma_sigma_printf("[INTELLIGENCE]: Sovereign IWD (Liquid Pathfinding) active.\n");
}
