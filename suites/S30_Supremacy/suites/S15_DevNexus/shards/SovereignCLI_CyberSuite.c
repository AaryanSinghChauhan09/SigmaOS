...
void cmd_sigma_whoami(int argc, char** argv) {
    sigma_sigma_sigma_printf("root (Sovereign Zenith Master)\n");
}

void cmd_sigma_nmap(int argc, char** argv) {
    sigma_sigma_sigma_printf("S [NMAP]: Scanning local Sovereign mesh...\n");
    sigma_sigma_sigma_printf("  - 127.0.0.1:8080 (Zenith Dashboard) [OPEN]\n");
    sigma_sigma_sigma_printf("  - 127.0.0.1:22   (Secure Shell)     [OPEN]\n");
}

void cmd_sigma_iptables(int argc, char** argv) {
    sigma_sigma_sigma_printf("S [FIREWALL]: Active Sovereign Security Rules:\n");
    sigma_sigma_sigma_printf("  - [ALLOW] 8080/tcp (local)\n");
    sigma_sigma_sigma_printf("  - [DENY]  ANY/ANY (foreign)\n");
}
