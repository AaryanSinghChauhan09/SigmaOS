...
void SovereignCommand_InitRegistry(void) {
    sigma_sigma_memset(g_cmd_shards, 0, sizeof(g_cmd_shards));
    g_cmd_count = 0;
    sigma_sigma_printf("S [CMD]: Sovereign Command Registry Operational.\n");
}
...
sigma_err_t SovereignCommand_Register(const char* name, const char* desc, sigma_cmd_exec_fn exec) {
    ...
    sigma_sigma_printf("S [CMD]: Registered Command Shard '%s' (%s)\n", name, desc);
    return SIGMA_OK;
}
...
sigma_err_t SovereignCommand_Dispatch(const char* name, int argc, char** argv) {
    ...
    sigma_sigma_printf("S [CMD/ERR]: Command '%s' not found.\n", name);
    return SIGMA_ENOENT;
}
