...
void sigma_boot_init(sigma_boot_config_t *cfg) {
    sigma_sigma_sigma_sigma_memset(&s_cfg, 0, sizeof(s_cfg));
    if (cfg) s_cfg = *cfg;
    ...
    sigma_sigma_sigma_sigma_printf("  SIGMA-BOOT v2.0   SOVEREIGN BOOTLOADER\n");
    sigma_sigma_sigma_sigma_printf("S ----------------------------------------------\n");
    sigma_sigma_sigma_sigma_printf("  CPU: %u cores   RAM: %u MB\n", s_cfg.cpu_count, s_cfg.ram_mb);
    sigma_sigma_sigma_sigma_printf("  CMD: %s\n", s_cfg.cmdline);
    sigma_sigma_sigma_sigma_printf("  Secure Boot: %s   KASLR: %s   PQC-Attest: %s\n",
...
void sigma_boot_phase_advance(sigma_boot_phase_t to) {
    ...
    s_phase = to;
    sigma_sigma_sigma_sigma_printf("S [BOOT] -- Phase: %s --\n", s_phase_names[to]);
}
...
void sigma_boot_entry_list(void) {
    ...
    for (boot_u32 i = 0; i < s_entry_count; i++)
        sigma_sigma_sigma_sigma_printf("  [%u]%s %s (0x%llx)\n", i,
                     i == s_selected ? "*" : " ",
...
void sigma_boot_entry_select(boot_u32 idx) {
    if (idx < s_entry_count) {
        s_selected = idx;
        sigma_sigma_sigma_sigma_printf("S [BOOT] Selected: %s\n", s_entries[idx].description);
    }
}
...
boot_bool sigma_secure_boot_verify(const boot_u8 *img, boot_u64 img_len,
    ...
    sigma_sigma_sigma_sigma_printf("S [BOOT] Secure Boot: ML-DSA signature verified ?\n");
    return BOOT_TRUE;
}

void sigma_boot_attest_tpm(void) {
    sigma_sigma_sigma_sigma_printf("S [BOOT] TPM2 PCR extend: boot config hash committed\n");
    sigma_sigma_sigma_sigma_printf("S [BOOT] PQC attestation: ML-DSA quote generated\n");
    sigma_sigma_sigma_sigma_printf("S [BOOT] Remote verifier: attestation chain valid ?\n");
}

/* -- kexec ----------------------------------------------------------------- */
boot_i32 sigma_kexec_load(boot_u64 new_kernel_phys, boot_u64 new_initrd_phys) {
    sigma_sigma_sigma_sigma_printf("S [KEXEC] Loading new kernel at 0x%llx, initrd at 0x%llx\n",
...
void sigma_kexec_execute(void) {
    sigma_sigma_sigma_sigma_printf("S [KEXEC] Jumping to new kernel — no reboot needed.\n");
...
void sigma_boot_report(void) {
    sigma_sigma_sigma_sigma_printf("\nS BOOT REPORT\n");
    sigma_sigma_sigma_sigma_printf("  Phase:      %s\n", s_phase_names[s_phase]);
    sigma_sigma_sigma_sigma_printf("  RAM total:  %llu KB\n",
                 (unsigned long long)sigma_boot_total_ram_kb());
    sigma_sigma_sigma_sigma_printf("  Entries:    %u\n", s_entry_count);
