...
void memory_page_coloring_init(void) {
    sigma_printf("S05 [MEMORY]: Initiating Sovereign Page Coloring Engine...\n");
    for (int i = 0; i < CACHE_WAYS; i++) {
        system_colors[i].color_id = i;
        system_colors[i].pages_allocated = 0;
    }
    sigma_printf("  [COLORING]: %d cache-ways mapped to Sovereign Silicate.\n", CACHE_WAYS);
}
...
sigma_u64 memory_allocate_colored_page(uint32_t preferred_color) {
    ...
    sigma_printf("  [COLORING]: Allocated page 0x%llX with Color Index %u\n", address, color);
    return address;
}

void S05_Register_PageColoring(void) {
    sigma_printf("S05 [MEMORY]: Sovereign Page Coloring Shard Online.\n");
