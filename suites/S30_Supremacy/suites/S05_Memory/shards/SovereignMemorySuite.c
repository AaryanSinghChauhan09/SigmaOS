...
void pmm_init(void) {
    sigma_sigma_memset(s_frame_bitmap, 0, sizeof(s_frame_bitmap));
...
void sigma_memory_defrag(void) {
    sigma_sigma_printf("  [MEM-OPT]: Background Slab Defragmentation active.\n");
    sigma_sigma_printf("  [MEM-OPT]: Recycled 42MB of fragmented shard-pages.\n");
}

/* --- Sub-Module 3: VMM Aggregator --- */
void SovereignMemory_Init(void) {
    sigma_sigma_printf("S [MEMORY-SUITE]: Initialising Sovereign PMM and VMM...\n");
    pmm_init();
    sigma_memory_defrag();
    sigma_sigma_printf("S [MEMORY-SUITE]: Slab-based allocation matrix ONLINE.\n");
}
