...
sigma_err_t sigma_profile_create(const char* username, const char* tier,
    ...
    sigma_sigma_printf("[IDENTITY]: Created profile '%s' (tier: %s, theme: 0x%06X/0x%06X)\n",
                 username, tier, bg, fg);
    return SIGMA_OK;
}
...
sigma_err_t sigma_profile_switch(sigma_u32 profile_index) {
    ...
    sigma_sigma_printf("[IDENTITY]: Switched to profile '%s' (login #%u)\n",
                 p->username, p->login_count);
    sigma_sigma_printf("  [CHROMA]: BG=0x%06X FG=0x%06X ACCENT=0x%06X OPACITY=%u%%\n",
                 p->theme.bg_color, p->theme.fg_color,
                 p->theme.accent_color, p->theme.opacity);
    sigma_sigma_printf("  [LOCALE]: %s | Accessibility: 0x%02X\n",
...
void sigma_profile_set_theme(sigma_u32 bg, sigma_u32 fg, sigma_u32 accent) {
    ...
    sigma_sigma_printf("[CHROMA]: Theme updated for '%s' -> BG=0x%06X FG=0x%06X\n",
                 p->username, bg, fg);
}
...
void sigma_profile_set_accessibility(sigma_u32 flags) {
    ...
    sigma_sigma_printf("[IDENTITY]: Accessibility flags set to 0x%02X for '%s'\n",
                 flags, p->username);
}
...
void SovereignPersonalization_Register(void) {
    sigma_sigma_printf("[REGISTRY]: Sovereign Personalization v2.0 (Deep) active.\n");
