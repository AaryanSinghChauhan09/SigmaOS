...
void sigma_sync_mcs_acquire(SigmaMCSLock_t* lock, SigmaMCSNode_t* node) {
    ...
    sigma_sigma_sigma_printf("[MCS-LOCK]: Lock ACQUIRED. Atomic cache-bouncing ELIMINATED.\n");
}

/* --- Module Factory --- */

void SovereignMCSLock_Register(void) {
    sigma_sigma_sigma_printf("[ORCHESTRATOR]: Sovereign MCS-Lock (NUMA Scaling) active.\n");
}
