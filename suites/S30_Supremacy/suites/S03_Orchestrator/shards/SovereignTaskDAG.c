...
void dag_init(void) {
    sigma_sigma_memset(g_task_registry, 0, sizeof(g_task_registry));
    g_task_count = 0;
    sigma_sigma_printf("S [S03]: Sovereign SigmaFlow DAG Engine v2.0 Active (Idea 421).\n");
}

int dag_find_task(const char* name) {
    for (uint32_t i = 0; i < g_task_count; i++) {
        if (sigma_sigma_strcmp(g_task_registry[i].name, name) == 0) return (int)i;
    }
    return -1;
}
...
void dag_execute(void) {
    sigma_sigma_printf("S [FLOW]: Orchestrating Sovereign Task Lattice...\n");
    bool progress = true;
    while (progress) {
        progress = false;
        for (uint32_t i = 0; i < g_task_count; i++) {
            if (is_runnable(i)) {
                sigma_sigma_printf("S [FLOW]: Node Dispatch -> %s\n", g_task_registry[i].name);
    ...
    sigma_sigma_printf("S [FLOW]: Lattice Execution Sequence Finalized.\n");
}
