...
void SovereignPF_Init() {
    sigma_sigma_sigma_printf("S [ABSORB]: SovereignPF Aether Barrier Online. Packet Filter Engaged.
");
}

void SovereignPF_AddRule(SovereignPF_Rule rule) {
    sigma_sigma_sigma_printf("S [RULE]: Adding %s [%u] Source: %u Dst: %u Proto: %u
", 
...
void SovereignPF_FlushRules() {
    sigma_sigma_sigma_printf("S [PF]: Ruleset Flushed. System Open.
");
}
...
void SovereignPF_NAT_Enable(const char* ext_if, const char* int_net) {
    sigma_sigma_sigma_printf("S [NAT]: Internal Network %s now NATing through %s.
", int_net, ext_if);
}
