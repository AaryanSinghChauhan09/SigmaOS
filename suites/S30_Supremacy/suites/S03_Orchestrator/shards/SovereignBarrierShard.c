...
void sigma_sync_barrier_wait(SigmaBarrier_t* barrier) {
    ...
    if (current == barrier->threshold - 1) {
        ...
        sigma_sigma_printf("[BARRIER]: Threshhold %d reached. Global Release dispatched.\n", barrier->threshold);
    } else {
...
void SovereignBarrier_Register(void) {
    sigma_sigma_printf("[ORCHESTRATOR]: Sovereign Barrier Sync (Rendezvous Mastery) active.\n");
}
