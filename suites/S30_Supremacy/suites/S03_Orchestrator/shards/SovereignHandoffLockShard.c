...
void sigma_sync_handoff_acquire(SigmaHandoffLock_t* hl, sigma_u32 my_id) {
    ...
    sigma_sigma_printf("[HANDOFF-LOCK]: Ownership transferred to Shard %u. Resuming execution...\n", my_id);
}
...
void sigma_sync_handoff_release(SigmaHandoffLock_t* hl, sigma_u32 next_id) {
    ...
    sigma_sigma_printf("[HANDOFF-LOCK]: Shard %u releasing. Handing off to successor %u.\n", 
                 hl->next_owner_id, next_id);
}
...
void SovereignHandoffLock_Register(void) {
    sigma_sigma_printf("[ORCHESTRATOR]: Sovereign Handoff-Lock (Anti-Thrashing) active.\n");
}
