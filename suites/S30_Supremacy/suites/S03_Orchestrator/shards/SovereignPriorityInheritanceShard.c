...
void sigma_sync_pi_lock(SigmaPIMutex_t* mutex, sigma_u32 requester_id, sigma_u32 requester_pri) {
    if (__sync_lock_test_and_set(&mutex->lock, 1)) {
        if (requester_pri > mutex->owner_priority) {
            sigma_sigma_printf("[SYNC-PI]: PRIORITY INVERSION detected! Boosting Owner %u to Priority %u.\n", 
    ...
    sigma_sigma_printf("[SYNC-PI]: Lock ACQUIRED by Shard %u.\n", requester_id);
}

/* --- Module Factory --- */

void SovereignPriorityInheritance_Register(void) {
    sigma_sigma_printf("[ORCHESTRATOR]: Sovereign Priority Inheritance (RT-Safety) active.\n");
}
