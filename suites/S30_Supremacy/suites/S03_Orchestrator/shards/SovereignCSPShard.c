...
void sigma_csp_send(SigmaChannel_t* chan, void* data) {
    sigma_sigma_printf("[CSP]: Sending data on channel %u. Waiting for Rendezvous...\n", chan->chan_id);
    while (chan->has_data) { /* Spin/Yield */ }
    chan->data_slot = data;
    chan->has_data = 1;
    sigma_sigma_printf("[CSP]: Rendezvous SUCCESS. Data transferred.\n");
}
...
void SovereignCSP_Register(void) {
    sigma_sigma_printf("[ORCHESTRATOR]: Sovereign CSP (Synchronous Coordination) active.\n");
}
