...
void SovereignDTrace_Init() {
    sigma_sigma_sigma_sigma_printf("S [ABSORB]: SovereignDTrace Pulse Zenith Online. Probing Live...
");
}

void SovereignDTrace_RegisterProbe(const char* provider, const char* mod, const char* func, const char* name) {
    sigma_sigma_sigma_sigma_printf("S [PROBE]: NEW -> %s:%s:%s:%s
...
void SovereignDTrace_EnableProvider(const char* provider) {
    sigma_sigma_sigma_sigma_printf("S [ENABLE]: DTrace Provider %s Activated. Streaming Telemetry.
", provider);
}
