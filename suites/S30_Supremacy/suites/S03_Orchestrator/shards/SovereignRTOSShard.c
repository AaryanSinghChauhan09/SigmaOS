...
void sigma_rt_mutex_lock(SigmaMutexRT_t* m, int requester_prio) {
    if (m->locking_pid && requester_prio > m->current_priority) {
        ...
        sigma_sigma_sigma_sigma_printf("[RTOS]: Priority Boost: PID %u elevated to %d.\n", m->locking_pid, requester_prio);
    }
}
...
void sovereign_rt_dispatch(sigma_u32 task_id) {
    sigma_sigma_sigma_sigma_printf("[RTOS]: Dispatching RT-Task %u (Mission Critical).\n", task_id);
}

/* --- Module Factory --- */

void SovereignRTOS_Register(void) {
    sigma_sigma_sigma_sigma_printf("[ORCHESTRATOR]: Sovereign RTOS Engine (Determinism) active.\n");
}
