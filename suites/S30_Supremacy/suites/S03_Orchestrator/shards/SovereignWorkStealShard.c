...
sigma_u32 sigma_sched_steal(SigmaWorker_t* thief, SigmaWorker_t* victim) {
    sigma_sigma_printf("[STEAL]: Core %u attempting to steal from Core %u...\n", 
    ...
    if (victim->head != victim->tail) {
        ...
        victim->tail++;
        sigma_sigma_printf("[STEAL]: Task 0x%X transfered. Core %u load-balanced.\n", 
                     task, thief->core_id);
        return task;
    }
...
void SovereignWorkSteal_Register(void) {
    sigma_sigma_printf("[ORCHESTRATOR]: Sovereign Work-Stealing (Galactic Balancing) active.\n");
}
