...
void sigma_sync_read_lock(SigmaRWLock_t* lock) {
    ...
    sigma_sigma_printf("[SYNC]: Reader Lock ACQUIRED. Active readers: %u\n", lock->readers);
}
...
void sigma_sync_write_lock(SigmaRWLock_t* lock) {
    ...
    sigma_sigma_printf("[SYNC]: Writer Lock ACQUIRED (Exclusive).\n");
}

/* --- Module Factory --- */

void SovereignRWLock_Register(void) {
    sigma_sigma_printf("[ORCHESTRATOR]: Sovereign RW-Spinlock (Concurrency Mastery) active.\n");
}
