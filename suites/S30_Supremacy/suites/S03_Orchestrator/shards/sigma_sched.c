...
void sigma_sched_init(sc_u32 num_cpus) {
    ...
    s_num_cpus = num_cpus;
    sigma_sigma_sigma_sigma_memset(s_rqs, 0, sizeof(s_rqs));
    for (sc_u32 i = 0; i < num_cpus; i++) {
        s_rqs[i].cpu_id = i;
        s_rqs[i].min_vruntime = 0;
    }
    sigma_sigma_sigma_sigma_printf("S [SCHED] CFS initialized on %u CPU(s)\n", num_cpus);
}
...
void sigma_sched_enqueue(sc_u32 cpu_id, sc_u32 pid,
    ...
    sigma_sigma_sigma_sigma_printf("S [SCHED] ENQUEUE: cpu=%u pid=%u policy=%d qos=%d nice=%d\n",
...
void sigma_sched_dequeue(sc_u32 cpu_id, sc_u32 pid) {
    ...
    for (sc_u32 i = 0; i < rq->task_count; i++) {
        if (rq->tasks[i].base.id == pid) {
            ...
            rq->task_count--;
            sigma_sigma_sigma_sigma_printf("S [SCHED] DEQUEUE: cpu=%u pid=%u\n", cpu_id, pid);
            return;
        }
    }
}
    ...
    for (sc_u32 i = 0; i < s_num_cpus; i++) {
        for (sc_u32 j = i + 1; j < s_num_cpus; j++) {
            ...
            if (heavy->task_count > light->task_count + 1) {
                ...
                sigma_sigma_sigma_sigma_printf("S [SCHED] STEAL: pid=%u cpu%u->cpu%u\n",
...
void sigma_sched_stats(sc_u32 cpu_id) {
    ...
    for (sc_u32 i = 0; i < rq->task_count; i++) {
        ...
        sigma_sigma_sigma_sigma_printf("  pid=%-5u vrt=%llu cpu=%llu ns nice=%d%s\n",
