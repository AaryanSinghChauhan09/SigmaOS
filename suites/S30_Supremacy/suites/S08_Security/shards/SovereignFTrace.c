...
sigma_err_t sigma_register_kprobe(SigmaKProbe_t *kp) {
    ...
    sigma_sigma_printf("S [FTRACE]: Kprobe registered at %p (Replaced opcode: 0x%02X)\n", 
...
void SovereignFTrace_Init(void) {
    sigma_sigma_printf("S [FTRACE]: Initialising Sovereign Function Tracer & ETW...\n");
    ...
    /* Register a Kprobe */
    SigmaKProbe_t kp;
    sigma_sigma_memset(&kp, 0, sizeof(kp));
    kp.address = (void*)&SovereignFTrace_Init; /* Safe to read opcode from ourselves */
    sigma_register_kprobe(&kp);

    sigma_sigma_printf("S [FTRACE]: Execution tracing online. Introspection sovereignty achieved.\n");
}
