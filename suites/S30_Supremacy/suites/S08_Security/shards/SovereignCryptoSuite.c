...
void sigma_hmac_sha256(const sigma_u8* key, sigma_sz_t klen, const sigma_u8* msg, sigma_sz_t mlen, sigma_u8 mac[32]) {
    ...
    sigma_sigma_sigma_sigma_memset(k_ipad, 0x36, 64); sigma_sigma_sigma_sigma_memset(k_opad, 0x5C, 64);
...
void SovereignCrypto_Init(void) {
    sigma_sigma_sigma_sigma_printf("S [CRYPTO-SUITE]: Industrial SHA-256 and HMAC matrices seated.\n");
}
