...
void sigma_sec_shadow_verify(SigmaShadowStack_t* ss, sigma_u64 addr) {
    if (ss->ssp > 0) {
        ...
        if (addr != expected) {
            sigma_sigma_printf("[SECURITY-CRITICAL]: STACK CORRUPTION detected! Return address MISMATCH.\n");
...
void SovereignShadowStack_Register(void) {
    sigma_sigma_printf("[SECURITY]: Sovereign Shadow Stack (ROP Protection) active.\n");
}
