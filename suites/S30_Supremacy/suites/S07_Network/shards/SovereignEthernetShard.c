...
void sigma_ethernet_handler(void* payload, sigma_sz_t size) {
    sigma_sigma_sigma_printf("  S [ETH]: Received Ethernet Frame (%lu bytes) via hardware shunt.\n", (unsigned long)size);
    sigma_sigma_sigma_printf("  S [ETH]: Payload zero-copied to BPF network-mesh.\n");
}
