...
void sigma_raft_init(void) {
    ...
    sigma_sigma_sigma_sigma_printf("[RAFT]: Cluster initialized with %d nodes. All FOLLOWER.\n",
                 RAFT_CLUSTER_SIZE);
}
...
sigma_u32 sigma_raft_request_vote(sigma_u32 candidate_id, sigma_u64 term) {
    ...
    sigma_sigma_sigma_sigma_printf("[RAFT]: Node %u requested votes for term %llu -> %u/%u\n",
...
void sigma_raft_elect(void) {
    ...
    sigma_sigma_sigma_sigma_printf("[RAFT]: Node %u starting election for term %llu.\n",
    ...
    if (votes >= quorum) {
        ...
        sigma_sigma_sigma_sigma_printf("[RAFT]: Node %u ELECTED LEADER (term %llu, %u votes).\n",
                     s_self_id, (unsigned long long)self->current_term, votes);
    } else {
        self->state = RAFT_FOLLOWER;
        sigma_sigma_sigma_sigma_printf("[RAFT]: Node %u election failed. Reverting to FOLLOWER.\n",
                     s_self_id);
    }
}
...
void sigma_raft_heartbeat(sigma_u64 current_tick) {
    ...
    sigma_sigma_sigma_sigma_printf("[RAFT]: Leader %u heartbeat at tick %llu.\n",
...
void SovereignConsensus_Audit(void) {
    ...
    sigma_sigma_sigma_sigma_printf("%-6s %-12s %-8s %-10s\n", "NODE", "STATE", "TERM", "VOTED_FOR");
    sigma_sigma_sigma_sigma_printf("--------------------------------------\n");
    for (sigma_u32 i = 0; i < RAFT_CLUSTER_SIZE; i++) {
        RaftNode_t* n = &s_cluster[i];
        sigma_sigma_sigma_sigma_printf("%-6u %-12s %-8llu %-10u\n",
...
void SovereignConsensus_Register(void) {
    sigma_sigma_sigma_sigma_printf("[REGISTRY]: Sovereign Edge Consensus (Raft v2.0) active.\n");
    sigma_raft_init();
