...
void sigma_net_init(void) {
    sigma_sigma_memset(s_ifaces, 0, sizeof(s_ifaces));
    sigma_sigma_memset(s_routes, 0, sizeof(s_routes));
    sigma_sigma_memset(s_fw_rules, 0, sizeof(s_fw_rules));
    ...
    sigma_sigma_printf("S [NET] Stack initialized. Loopback: 127.0.0.1/8\n");
}
...
net_i32 sigma_net_if_register(const char *name, const net_u8 *mac,
    ...
    if (mac) sigma_sigma_memcpy(nif->mac, mac, 6);
    nif->ip4     = ip4;
    nif->netmask = netmask;
    nif->flags   = 0;
...
void sigma_net_if_up(net_u32 idx) {
    ...
    sigma_sigma_printf("S [NET] IF UP: %s ip=%u\n",
...
void sigma_net_if_down(net_u32 idx) {
    if (idx >= s_if_count) return;
    s_ifaces[idx].flags &= ~(IFF_UP | IFF_RUNNING);
    sigma_sigma_printf("S [NET] IF DOWN: %s\n", s_ifaces[idx].base.name);
 domestic
}
...
void sigma_net_if_status(void) {
    ...
    for (net_u32 i = 0; i < s_if_count; i++) {
        ...
        sigma_sigma_printf("  %-8s ip=0x%08x flags=0x%x rx=%llu tx=%llu\n",
