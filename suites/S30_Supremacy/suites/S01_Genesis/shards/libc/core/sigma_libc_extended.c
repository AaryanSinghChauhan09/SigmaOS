...
void* sigma_sigma_memcpy(void* dest, const void* src, uint32_t n) {
...
void* sigma_sigma_memset(void* dest, int c, uint32_t n) {
    uint8_t* d = (uint8_t*)dest;
    while (n--) *d++ = (uint8_t)c;
    return dest;
}
...
// ── String Primitives ─────────────────────────────────────────────────────────
uint32_t sigma_sigma_strlen(const char* s) {
    const char* p = s;
    while (*p) p++;
    return (uint32_t)(p - s);
}
...
int sigma_sigma_strcmp(const char* a, const char* b) {
    while (*a && (*a == *b)) { a++; b++; }
...
void sigma_sort_small(void* base, uint32_t n, uint32_t size,
    ...
    for (uint32_t i = 1; i < n; i++) {
        sigma_sigma_memcpy(tmp, arr + i * size, size);
        int32_t j = (int32_t)i - 1;
        while (j >= 0 && cmp(arr + j * size, tmp) > 0) {
            sigma_sigma_memcpy(arr + (j + 1) * size, arr + j * size, size);
            j--;
        }
        sigma_sigma_memcpy(arr + (j + 1) * size, tmp, size);
    }
}
