...
static sigma_i64 sc_handle_exit(sigma_u64 code, sigma_u64 b, sigma_u64 c,
    ...
    sigma_sigma_sigma_sigma_printf("S [SYSCALL] exit(%llu)\n", (unsigned long long)code);
    sigma_exit((int)code);
    return 0;
}
...
static sigma_i64 sc_handle_udf(sigma_u64 id, sigma_u64 args, sigma_u64 c,
    ...
    sigma_sigma_sigma_sigma_printf("S [SYSCALL] UDF call id=%llu args=0x%llx\n",
...
void sigma_syscall_table_init(void) {
    ...
    sigma_sigma_sigma_sigma_printf("S [SYSCALL] Table initialized: %u entries\n", SIGMA_SYSCALL_MAX);
}
...
sigma_i64 sigma_syscall_dispatch(sigma_u32 num,
    ...
    if (desc->sec_class == SC_SEC_CRITICAL) {
        sigma_sigma_sigma_sigma_printf("S [SYSCALL] DENIED #%u '%s' � ring-0 only\n",
                     num, desc->name);
        return -1; /* EPERM */
    }
...
void sigma_syscall_audit(void) {
    ...
    sigma_sigma_sigma_sigma_printf("%-8s %-24s %s\n", "NUM", "NAME", "CALLS");
    sigma_u32 printed = 0;
    for (sigma_u32 i = 0; i < SIGMA_SYSCALL_MAX && printed < 10; i++) {
        if (s_table[i].call_count > 0) {
            sigma_sigma_sigma_sigma_printf("  %-6u %-24s %u\n",
