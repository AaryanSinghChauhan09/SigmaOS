...
void genesis_register_handler(uint8_t vector, interrupt_handler_t handler) {
    ...
    sigma_sigma_printf("S01 [GENESIS]: Vector 0x%02X bound to handler 0x%p\n", vector, handler);
}
...
void genesis_dispatch_interrupt(uint8_t vector) {
    if (vector_table[vector]) {
        ...
        sigma_sigma_printf("S01 [GENESIS]: [IRQ-STUB] Unhandled vector 0x%02X\n", vector);
    }
}

void S01_Register_InterruptVector(void) {
    sigma_sigma_printf("S01 [GENESIS]: Sovereign Interrupt Vectoring Online.\n");
