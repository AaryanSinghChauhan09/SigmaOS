...
void sigma_lvm_snapshot(void) {
    ...
    sigma_sigma_printf("  [LVM]: Generated Atomic Snapshot (Rev: %d) of Volume [%s]\n", s_main_vol.revision, s_main_vol.volume_id);
}

void SovereignBackend_Init(void) {
    sigma_sigma_printf("S [BACKEND-SUITE]: Initialising Filesystems and LVM-Sentinel...\n");
    sigma_lvm_snapshot();
    sigma_sigma_printf("S [BACKEND-SUITE]: Logical Volume [SIGMA_ROOT_ZENITH] is now IMMUTABLE.\n");
}
