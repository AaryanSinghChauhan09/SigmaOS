...
void sigma_isr_handler(uint64_t irq_num) {
    sigma_sigma_sigma_sigma_printf("S [IDT]: Direct-to-Silicon ISR Triggered -> IRQ %d\n", irq_num);
...
void idt_init(void) {
    ...
    sigma_sigma_sigma_sigma_memset(&g_idt, 0, sizeof(g_idt));
    ...
    sigma_sigma_sigma_sigma_printf("S [S01]: Sovereign IDT Materialized. Silicon linkage complete.\n");
}
