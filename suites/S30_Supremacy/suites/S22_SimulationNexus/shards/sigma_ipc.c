...
void sigma_ipc_init(void) {
    sigma_sigma_printf("S [IPC] Sovereign IPC Subsystem initialized\n");
    sigma_sigma_printf("S [IPC] Fabric: Mach Ports | Shared Memory | Ring vBus\n");
}
...
ipc_port_t sigma_port_allocate(void) {
    ...
    s_port_count++;
    sigma_sigma_printf("S [IPC] Port allocated: 0x%08x\n", p);
    return p;
}

sigma_err_t sigma_port_destroy(ipc_port_t port) {
    sigma_sigma_printf("S [IPC] Port destroyed: 0x%08x\n", port);
...
sigma_err_t sigma_msg_send(ipc_header_t* header, const void* data) {
    sigma_sigma_printf("S [IPC] Msg SEND: (remote=0x%x, size=%u)\n", 
...
sigma_err_t sigma_msg_recv(ipc_port_t port, ipc_header_t* header, void* data, sigma_sz_t max_len) {
    sigma_sigma_printf("S [IPC] Msg RECV on port 0x%x\n", port);
...
void* sigma_shm_create(sigma_u32 id, sigma_sz_t size) {
    ...
    sigma_sigma_printf("S [SHM] Created segment %u (size=%llu) at %p\n", 
                 id, (unsigned long long)size, addr);
    return addr;
}

void sigma_shm_attach(sigma_u32 id, void* addr) {
    sigma_sigma_printf("S [SHM] Segment %u mapped to %p\n", id, addr);
}
