...
void sigma_gcd_init(void) {
    ...
    sigma_sigma_printf("S [GCD] Sovereign Parallelism Engine initialized\n");
    sigma_sigma_printf("S [GCD] Parity: libdispatch (macOS) | WorkQueues (Linux)\n");
}
...
gcd_queue_t* sigma_gcd_queue_create(const char* name, gcd_priority_t prio) {
    ...
    sigma_sigma_printf("S [GCD] Registered queue: %s (prio=%d)\n", name, prio);
    return q;
}
...
void sigma_gcd_async(gcd_queue_t* queue, gcd_block_t block, void* context) {
    ...
    if (!queue_push(queue, task)) {
        sigma_sigma_printf("S [GCD] Warning: Queue %s is full! Dropping task.\n", queue->name);
        return;
    }
...
void sigma_gcd_sync(gcd_queue_t* queue, gcd_block_t block, void* context) {
     ...
     */
    sigma_sigma_printf("S [GCD] Sync execution on queue: %s\n", queue->name);
    block(context);
}

void sigma_gcd_apply(sigma_u32 iterations, gcd_queue_t* queue, void (*block)(sigma_u32 index)) {
    sigma_sigma_printf("S [GCD] Applying %u iterations on queue: %s\n", iterations, queue->name);
