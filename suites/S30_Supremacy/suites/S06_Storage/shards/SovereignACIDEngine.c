...
sigma_u64 sigma_txn_begin(void) {
    ...
    sigma_sigma_printf("[ACID]: BEGIN transaction %llu\n",
...
sigma_err_t sigma_txn_commit(sigma_u64 txn_id) {
    ...
    sigma_sigma_printf("[ACID]: COMMIT transaction %llu (%u WAL entries flushed)\n",
...
sigma_err_t sigma_txn_rollback(sigma_u64 txn_id) {
    ...
    sigma_sigma_printf("[ACID]: ROLLBACK transaction %llu (%u entries discarded)\n",
                 (unsigned long long)txn_id, txn->wal_count);
    return SIGMA_OK;
}
...
void SovereignACID_Audit(void) {
    ...
    sigma_sigma_printf("%-8s %-12s %-8s\n", "TXN_ID", "STATE", "WAL_OPS");
    sigma_sigma_printf("----------------------------\n");
    for (sigma_u32 i = 0; i < s_txn_count; i++) {
        sigma_sigma_printf("%-8llu %-12s %-8u\n",
                     (unsigned long long)s_txns[i].txn_id,
                     state_names[s_txns[i].state],
                     s_txns[i].wal_count);
    }
    sigma_sigma_printf("WAL entries: %u | Next LSN: %llu\n",
                 s_wal_count, (unsigned long long)s_next_lsn);
    sigma_sigma_printf("----------------------------\n");
}

void SovereignACID_Register(void) {
    sigma_sigma_printf("[REGISTRY]: Sovereign ACID Database Engine active in Storage Suite.\n");
}
