...
SigmaBlockDevice_t* sigma_blk_allocate_disk(void) {
    ...
    sigma_sigma_memset(bdev, 0, sizeof(*bdev));
...
sigma_err_t sigma_blk_register_disk(SigmaBlockDevice_t *bdev) {
    ...
    sigma_sigma_printf("S [BLK]: Registered disk '%s' [Capacity: %llu MB]\n", 
...
sigma_err_t sigma_submit_bio(SigmaBlockDevice_t *bdev, SigmaBio_t *bio) {
    ...
    sigma_sigma_printf("S [BLK]: Queued %s bio on '%s' (sec: %llu, len: %u)\n",
...
static void my_bio_end_io(void *arg) {
    ...
    sigma_sigma_printf("S [BLK]: Completed %s bio on sec %llu [status=%d]\n",
                 bio->opcode == BLK_OP_READ ? "READ" : "WRITE",
...
void sigma_blk_print_stats(void) {
    ...
    for (sigma_u32 i = 0; i < s_bdev_count; i++) {
        ...
        sigma_sigma_printf("  %s: read_sectors=%llu write_sectors=%llu in_flight=%u\n",
                     s_bdevs[i].name, 
...
void SovereignBlockIO_Init(void) {
    ...
    if (nvme) {
        sigma_sigma_strcpy(nvme->name, "nvme0n1", 32);
    ...
    if (sda) {
        sigma_sigma_strcpy(sda->name, "sda", 32);
    ...
    sigma_sigma_printf("S [BLK]: Block IO layer online. Multi-queue sovereignty active.\n");
}
