...
SigmaURB_t* sigma_usb_alloc_urb(void) {
    ...
    sigma_sigma_memset(urb, 0, sizeof(SigmaURB_t));
    return urb;
}
...
sigma_err_t sigma_usb_submit_urb(SigmaURB_t *urb) {
    ...
    if (!urb->dev->bus->submit_urb) {
        sigma_sigma_printf("S [USB]: HCD missing submit_urb hook!\n");
        return SIGMA_ENOTSUP;
    }
    
    sigma_sigma_printf("S [USB]: Submitting URB [Pipe: 0x%08x] Len: %u\n", urb->pipe, urb->transfer_buffer_length);
...
static void hub_port_connect_change(SigmaUSBDevice_t *hub, int port) {
    sigma_sigma_printf("S [USB]: Port %d on Hub '%s' state changed.\n", port, hub->devpath);
    /* In reality: 
     * 1. debounce 
     * 2. reset port 
     * 3. read descriptor 
...
static void my_urb_callback(SigmaURB_t *urb) {
    sigma_sigma_printf("S [USB]: URB completed with status %d, actual_len %u\n", urb->status, urb->actual_length);
}
...
void SovereignUSBCore_Init(void) {
    sigma_sigma_printf("S [USB]: Initialising Sovereign USB Core...\n");

    if (s_usb_bus_count >= MAX_USB_BUSES) return;
    
    SigmaUSBBus_t *bus = &s_usb_buses[s_usb_bus_count++];
    bus->busnum = 1;
    sigma_sigma_strcpy(bus->bus_name, "xhci_hcd", 32);
    bus->submit_urb = mock_hcd_submit_urb;
    
    /* Create Root Hub */
    static SigmaUSBDevice_t root_hub;
    sigma_sigma_memset(&root_hub, 0, sizeof(root_hub));
    root_hub.devnum = 1;
    sigma_sigma_strcpy(root_hub.devpath, "usb1", 16);
    root_hub.bus = bus;
    ...
    sigma_sigma_printf("S [USB]: USB Core Online. Hotplug sovereignty established.\n");
}
