...
sigma_err_t sigma_spi_register_controller(SigmaSPIController_t *ctrl) {
    ...
    sigma_sigma_printf("S [SPI]: Registered SPI Controller spi%u (CS max: %u)\n", 
...
SigmaSPIDevice_t* sigma_spi_new_device(SigmaSPIController_t *ctrl, sigma_u8 cs) {
    ...
    sigma_sigma_memset(dev, 0, sizeof(*dev));
    dev->controller = ctrl;
    dev->chip_select = cs;
    dev->online = SIGMA_TRUE;
    
    sigma_sigma_printf("S [SPI]: Registered SPI Slave spi%u.%u\n", ctrl->bus_num, cs);
    return dev;
}
...
sigma_err_t sigma_spi_write_then_read(SigmaSPIDevice_t *spi, 
    ...
    sigma_sigma_memset(&msg,  0, sizeof(msg));
    sigma_sigma_memset(&x, 0, sizeof(x));
...
static sigma_err_t mock_spi_transfer(SigmaSPIDevice_t *spi, SigmaSPIMessage_t *msg) {
    ...
    while (t) {
        sigma_sigma_printf("S [SPI-HW]: Target spi%u.%u -> XFER Len: %u [TX: %p, RX: %p]\n",
                     spi->controller->bus_num, spi->chip_select, t->len, t->tx_buf, t->rx_buf);
        msg->actual_length += t->len;
        
        /* Simulated loopback for RX */
        if (t->rx_buf && t->tx_buf) {
            sigma_sigma_memcpy(t->rx_buf, t->tx_buf, t->len);
        } else if (t->rx_buf) {
            sigma_sigma_memset(t->rx_buf, 0xAA, t->len);
        }
...
void SovereignSPICore_Init(void) {
    ...
    sigma_sigma_memset(&ctrl, 0, sizeof(ctrl));
    ...
    if (flash) {
        sigma_sigma_strcpy(flash->modalias, "spidev", 32);
    ...
    if (flash) {
        ...
        sigma_sigma_printf("S [SPI]: Read JEDEC ID: %02X %02X %02X\n", rx[0], rx[1], rx[2]);
    }

    sigma_sigma_printf("S [SPI]: SPI Core online. High-speed serial sovereignty achieved.\n");
}
