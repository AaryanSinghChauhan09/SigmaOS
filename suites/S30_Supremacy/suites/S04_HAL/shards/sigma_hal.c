...
void sigma_hal_init(void) {
    sigma_sigma_sigma_sigma_memset(s_devices, 0, sizeof(s_devices));
    sigma_sigma_sigma_sigma_memset(s_irqs,    0, sizeof(s_irqs));
    s_irqs_init = HAL_TRUE;
    sigma_sigma_sigma_sigma_printf("S [HAL] Hardware Abstraction Layer initialized\n");
    sigma_sigma_sigma_sigma_printf("S [HAL] Max devices: %u   Max IRQs: %u\n",
...
hal_i32 sigma_hal_register(sigma_device_t *dev) {
    ...
    *slot = *dev;
    ...
    sigma_sigma_sigma_sigma_printf("S [HAL] REGISTER: id=%u name=%s bus=%d class=%d irq=%u\n",
    ...
    if (slot->ops && slot->ops->probe) {
        if (slot->ops->probe(slot) == HAL_OK) {
            ...
            sigma_sigma_sigma_sigma_printf("S [HAL] ONLINE: %s\n", slot->base.name);
        } else {
            sigma_sigma_sigma_sigma_printf("S [HAL] PROBE FAIL: %s\n", slot->base.name);
        }
    } else {
...
void sigma_hal_unregister(hal_u32 dev_id) {
    for (hal_u32 i = 0; i < s_dev_count; i++) {
        if (s_devices[i].base.id == dev_id) {
            ...
            sigma_sigma_sigma_sigma_printf("S [HAL] UNREGISTER: %s\n", s_devices[i].base.name);
...
void sigma_hal_enumerate_bus(sigma_bus_t bus) {
    sigma_sigma_sigma_sigma_printf("S [HAL] Enumerating bus %d...\n", (int)bus);
    for (hal_u32 i = 0; i < s_dev_count; i++)
        if (s_devices[i].bus == bus)
            sigma_sigma_sigma_sigma_printf("  [%u] %s vid=0x%04x did=0x%04x %s\n",
...
void sigma_hal_device_list(void) {
    ...
    sigma_sigma_sigma_sigma_printf("%-4s %-20s %-8s %-8s %-6s %s\n",
                 "ID", "NAME", "BUS", "CLASS", "IRQ", "STATUS");
    for (hal_u32 i = 0; i < s_dev_count; i++) {
        sigma_device_t *d = &s_devices[i];
        sigma_sigma_sigma_sigma_printf("  %-2u %-20s %-8s %-8s %-6u %s\n",
...
hal_i32 sigma_irq_request(hal_u32 irq, sigma_irq_type_t type,
    ...
    sigma_sigma_sigma_sigma_printf("S [HAL] IRQ%u registered (type=%d)\n", irq, (int)type);
    return HAL_OK;
}
...
void sigma_irq_stats(void) {
    ...
    for (hal_u32 i = 0; i < SIGMA_HAL_MAX_IRQS; i++) {
        if (s_irqs[i].count > 0)
            sigma_sigma_sigma_sigma_printf("  IRQ%-4u count=%-10llu %s\n", i,
...
hal_i32 sigma_dma_alloc(sigma_dma_buf_t *buf, hal_u64 size, hal_bool coherent) {
    ...
    sigma_sigma_sigma_sigma_printf("S [DMA] ALLOC: %llu bytes phys=0x%llx coherent=%d\n",
...
void sigma_dma_sigma_sigma_free(sigma_dma_buf_t *buf) {
    ...
    sigma_sigma_sigma_sigma_memset(buf, 0, sizeof(*buf));
}
...
hal_i32 sigma_pm_suspend_device(hal_u32 dev_id) {
    for (hal_u32 i = 0; i < s_dev_count; i++) {
        if (s_devices[i].base.id == dev_id && s_devices[i].ops && s_devices[i].ops->suspend) {
            ...
            sigma_sigma_sigma_sigma_printf("S [PM] SUSPEND: %s\n", s_devices[i].base.name);
...
hal_i32 sigma_pm_resume_device(hal_u32 dev_id) {
    for (hal_u32 i = 0; i < s_dev_count; i++) {
        if (s_devices[i].base.id == dev_id && s_devices[i].ops && s_devices[i].ops->resume) {
            ...
            sigma_sigma_sigma_sigma_printf("S [PM] RESUME: %s\n", s_devices[i].base.name);
