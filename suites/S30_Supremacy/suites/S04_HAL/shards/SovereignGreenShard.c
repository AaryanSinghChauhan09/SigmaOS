...
sigma_err_t sigma_green_transition(PowerState_t target) {
    ...
    sigma_sigma_sigma_sigma_printf("[GREEN]: DVFS transition %s -> %s\n",
                 pstate_names[s_power.state], pstate_names[target]);
    sigma_sigma_sigma_sigma_printf("  [FREQ]:    %u MHz -> %u MHz\n", s_power.freq_mhz, entry->freq_mhz);
    sigma_sigma_sigma_sigma_printf("  [VOLTAGE]: %u mV -> %u mV\n",   s_power.voltage_mv, entry->voltage_mv);
    sigma_sigma_sigma_sigma_printf("  [POWER]:   %u mW -> %u mW (delta: %d mW)\n",
...
void sigma_green_thermal_check(sigma_u32 current_temp_c) {
    ...
    if (current_temp_c >= s_power.thermal_limit) {
        sigma_sigma_sigma_sigma_printf("[GREEN]: THERMAL ALERT! %u C >= %u C limit. Throttling!\n",
                     current_temp_c, s_power.thermal_limit);
        sigma_green_transition(PSTATE_ECO);
    } else if (current_temp_c <= 50 && s_power.state == PSTATE_ECO) {
        sigma_sigma_sigma_sigma_printf("[GREEN]: Temperature nominal (%u C). Restoring NOMINAL.\n",
                     current_temp_c);
...
void SovereignGreen_Audit(void) {
    ...
    sigma_sigma_sigma_sigma_printf("  State:       %s\n",   pstate_names[s_power.state]);
    sigma_sigma_sigma_sigma_printf("  Frequency:   %u MHz\n", s_power.freq_mhz);
    sigma_sigma_sigma_sigma_printf("  Voltage:     %u mV\n",  s_power.voltage_mv);
    sigma_sigma_sigma_sigma_printf("  Power Draw:  %u mW\n",  s_power.power_mw);
    sigma_sigma_sigma_sigma_printf("  Temperature: %u C / %u C limit\n",
...
void SovereignSustainability_Register(void) {
    sigma_sigma_sigma_sigma_printf("[REGISTRY]: Sovereign Green Sustainability v2.0 (DVFS) active.\n");
