...
sigma_err_t sigma_acpi_transition(SigmaACPIState_t target) {
    sigma_sigma_sigma_sigma_printf("S [ACPI]: %s ? %s\n",
                 acpi_state_name(s_system_state),
                 acpi_state_name(target));
    if (target == ACPI_S3)
        sigma_sigma_sigma_sigma_printf("S [ACPI]: Saving CPU state, suspending devices...\n");
    else if (target == ACPI_S4)
        sigma_sigma_sigma_sigma_printf("S [ACPI]: Writing hibernation image to swap...\n");
    else if (target == ACPI_S5)
        sigma_sigma_sigma_sigma_printf("S [ACPI]: Powering off (soft-off)...\n");
    else if (target == ACPI_S0 && s_system_state != ACPI_S0)
        sigma_sigma_sigma_sigma_printf("S [ACPI]: Resuming — restoring CPU state...\n");
...
sigma_err_t sigma_cpufreq_set_governor(sigma_u32 cpu_id, SigmaCpufreqGov_t gov) {
    for (sigma_u32 i = 0; i < s_ncpus; i++) {
        if (s_cpus[i].cpu_id == cpu_id) {
            ...
            sigma_sigma_sigma_sigma_printf("S [CPUFREQ]: CPU%u governor=%s freq=%uMHz\n",
...
sigma_err_t sigma_cpufreq_set_freq(sigma_u32 cpu_id, sigma_u32 freq_khz) {
    for (sigma_u32 i = 0; i < s_ncpus; i++) {
        if (s_cpus[i].cpu_id == cpu_id) {
            ...
            sigma_sigma_sigma_sigma_printf("S [CPUFREQ]: CPU%u freq set to %uMHz (userspace)\n",
...
void sigma_turbo_set(sigma_u32 cpu_id, sigma_bool enable) {
    for (sigma_u32 i = 0; i < s_ncpus; i++) {
        if (s_cpus[i].cpu_id == cpu_id) {
            ...
            sigma_sigma_sigma_sigma_printf("S [CPUFREQ]: CPU%u Turbo Boost %s\n",
    ...
    for (sigma_u32 i = 0; i < s_thermal_count; i++) {
        if (sigma_streq(s_thermal[i].name, name)) {
            ...
            if (temp_mc >= s_thermal[i].critical_milli_c) {
                sigma_sigma_sigma_sigma_printf("S [THERMAL]: CRITICAL! %s = %d°C — EMERGENCY THROTTLE\n",
                             name, temp_mc / 1000);
                ...
                sigma_sigma_sigma_sigma_printf("S [THERMAL]: PASSIVE trip: %s = %d°C — throttling\n",
                             name, temp_mc / 1000);
                s_thermal[i].throttled = SIGMA_TRUE;
            } else {
...
void sigma_battery_status(void) {
    sigma_sigma_sigma_sigma_printf("S [BATTERY]: %u%% | %s | %umV | %dmA | Threshold: %u%%\n",
...
void sigma_battery_set_threshold(sigma_u32 pct) {
    ...
    sigma_sigma_sigma_sigma_printf("S [BATTERY]: Charge threshold set to %u%%\n", pct);
}
    ...
    for (sigma_u32 i = 0; i < s_pm_count; i++) {
        if (sigma_streq(s_pm_devs[i].name, name)) {
            ...
            if (s_pm_devs[i].usage_count == 0) {
                sigma_sigma_sigma_sigma_printf("S [PM]: Device '%s' idle — autosuspend in %ums\n",
...
void SovereignPowerManagement_Init(void) {
    sigma_sigma_sigma_sigma_printf("S [PM]: Initialising Sovereign Power Management Subsystem...\n");
    ...
    sigma_sigma_sigma_sigma_printf("S [PM]: Power management sovereignty achieved.\n");
}
