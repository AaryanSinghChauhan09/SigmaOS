...
sigma_u64 sigma_hal_rdtsc_measure(void) {
    sigma_sigma_sigma_printf("[RDTSC-CLOCK]: Serializing instruction pipeline for invariant clock read...\n");
    // x86_64: LFENCE -> RDTSC -> LFENCE to prevent out-of-order execution spoofing
    sigma_sigma_sigma_printf("[RDTSC-CLOCK]: Precise silicon cycle count harvested defensively.\n");
...
void SovereignRDTSC_Register(void) {
    sigma_sigma_sigma_printf("[HAL]: Sovereign RDTSC (Invariant Clock Mastery) active.\n");
}
