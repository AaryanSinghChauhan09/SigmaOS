...
static sigma_err_t hci_send_cmd(SigmaHCIController_t *ctrl, sigma_u16 ogf, sigma_u16 ocf, const sigma_u8 *param, sigma_u8 plen) {
    ...
    if (plen > 0 && param) {
        sigma_sigma_memcpy(&buf[3], param, plen);
    }

    sigma_sigma_printf("S [BLUETOOTH]: HCI CMD TX -> OGF: 0x%02x OCF: 0x%04x Len: %d\n", ogf, ocf, plen);
...
void sigma_bt_inquiry_start(SigmaHCIController_t *ctrl) {
    ...
    sigma_sigma_printf("S [BLUETOOTH]: Started Device Inquiry...\n");
}

void sigma_bt_reset(SigmaHCIController_t *ctrl) {
    hci_send_cmd(ctrl, OGF_HOST_CTL, 0x0003, SIGMA_NULL, 0);
    sigma_sigma_printf("S [BLUETOOTH]: Controller Reset Issued\n");
}
...
void sigma_hci_rx_event(SigmaHCIController_t *ctrl, const sigma_u8 *data, sigma_sz_t len) {
    ...
    switch (event_code) {
        case HCI_EV_CMD_COMPLETE: {
            ...
            sigma_sigma_printf("S [BLUETOOTH]: CMD Complete -> Opcode 0x%04x Status: 0x%02x\n", opcode, status);
            break;
        }
        case HCI_EV_INQUIRY_RESULT: {
            ...
            for (int i = 0; i < num_responses; i++) {
                ...
                sigma_sigma_printf("S [BLUETOOTH]: Found Device -> %02X:%02X:%02X:%02X:%02X:%02X\n",
        ...
        case HCI_EV_CONN_COMPLETE: {
            ...
            if (status == 0) {
                sigma_sigma_printf("S [BLUETOOTH]: Connected successfully. Handle: 0x%04x\n", handle);
            } else {
                sigma_sigma_printf("S [BLUETOOTH]: Connection failed. Status: 0x%02x\n", status);
            }
            break;
        }
        default:
            sigma_sigma_printf("S [BLUETOOTH]: Unhandled HCI Event Code: 0x%02x\n", event_code);
            break;
    }
}
...
sigma_err_t sigma_l2cap_send(SigmaHCIController_t *ctrl, sigma_u16 handle, sigma_u16 cid, const sigma_u8 *data, sigma_sz_t len) {
    ...
    sigma_sigma_memcpy(&acl_buf[8], data, len);
    
    sigma_sigma_printf("S [BLUETOOTH]: L2CAP TX -> Handle: 0x%04x CID: 0x%04x Len: %lu\n", handle, cid, (unsigned long)len);
    if (ctrl->tx_packet) {
...
static sigma_err_t mock_usb_tx(sigma_u8 type, const sigma_u8 *data, sigma_sz_t len) {
    ...
    sigma_sigma_printf("S [BLUETOOTH-HW]: (Mock USB TX) Type 0x%02x length %lu dispatched.\n", type, (unsigned long)len);
    return SIGMA_OK;
}

void SovereignBluetooth_Init(void) {
    sigma_sigma_printf("S [BLUETOOTH]: Initialising Sovereign Bluetooth Stack (BlueZ parity)...\n");

    if (s_hci_count >= BT_MAX_DEVICES) return;
    
    SigmaHCIController_t *hci0 = &s_hci_ctrl[s_hci_count++];
    sigma_sigma_memset(hci0, 0, sizeof(*hci0));
    hci0->tx_packet = mock_usb_tx;
    hci0->online = SIGMA_TRUE;
    sigma_sigma_strcpy(hci0->name, "hci0", 5);
    ...
    sigma_sigma_printf("S [BLUETOOTH]: Bluetooth stack online. Wireless peripheral sovereignty active.\n");
}
