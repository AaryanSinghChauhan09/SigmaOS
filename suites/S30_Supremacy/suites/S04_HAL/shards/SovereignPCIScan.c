...
void sigma_pcie_scan(void) {
    sigma_sigma_sigma_sigma_printf("S [PCI]: Enumerating PCIe configuration space...\n");
    ...
    for (sigma_u32 i = 0; i < 4 && s_pci_count < MAX_PCI_DEVICES; i++) {
        ...
        sigma_sigma_sigma_sigma_printf("S [PCI]: 00:%02x.0 [%04x:%04x] — %s\n", i, p->vendor_id, p->device_id, simulated[i].name);
    }
    sigma_sigma_sigma_sigma_printf("S [PCI]: PCIe scan complete — %u devices found.\n", s_pci_count);
}
