...
sigma_err_t sigma_i2c_add_adapter(SigmaI2CAdapter_t *adap) {
    ...
    sigma_sigma_sigma_sigma_printf("S [I2C]: Registered adapter i2c-%u: '%s'\n", adap->nr, adap->name);
    return SIGMA_OK;
}
...
static sigma_i32 mock_i2c_xfer(SigmaI2CAdapter_t *adap, SigmaI2CMsg_t *msgs, sigma_i32 num) {
    ...
    for (int i = 0; i < num; i++) {
        ...
        sigma_sigma_sigma_sigma_printf("S [I2C-HW]: %s Addr: 0x%02X Len: %u\n", 
...
void SovereignI2CCore_Init(void) {
    sigma_sigma_sigma_sigma_printf("S [I2C]: Initialising Sovereign I2C/SMBus Core...\n");

    /* Register DesignWare I2C Controller (Common on Intel/AMD) */
    SigmaI2CAdapter_t dw_adap;
    sigma_sigma_sigma_sigma_memset(&dw_adap, 0, sizeof(dw_adap));
    sigma_sigma_sigma_strcpy(dw_adap.name, "Synopsys DesignWare I2C adapter", sizeof(dw_adap.name));
    ...
    sigma_sigma_sigma_sigma_memset(&eeprom, 0, sizeof(eeprom));
    sigma_sigma_sigma_strcpy(eeprom.name, "24c02", 32);
    eeprom.addr = 0x50;
    ...
    sigma_sigma_sigma_sigma_printf("S [I2C]: Read from client 0x50 -> 0x%02X\n", val);

    sigma_sigma_sigma_sigma_printf("S [I2C]: I2C engine online. Sensor bus parameter sovereignty established.\n");
}
