...
sigma_err_t sigma_vtable_register(const char* name,
    ...
    sigma_sigma_printf("[VTABLE]: Registered polymorphic object '%s' (type: %s)\n",
...
void SovereignVTable_Audit(void) {
    ...
    sigma_sigma_printf("%-20s %-20s %-10s\n", "OBJECT", "TYPE", "REFS");
    sigma_sigma_printf("----------------------------------------------\n");
    for (sigma_u32 i = 0; i < s_object_count; i++) {
        sigma_sigma_printf("%-20s %-20s %-10u\n",
                     s_object_pool[i].name,
                     s_object_pool[i].vtable->type_name,
                     s_object_pool[i].ref_count);
    }
    sigma_sigma_printf("----------------------------------------------\n");
    sigma_sigma_printf("Total polymorphic objects: %u\n", s_object_count);
}

/* --- Module Factory --- */

void SovereignVTableRegistry_Register(void) {
    sigma_sigma_printf("[REGISTRY]: Sovereign VTable OOP Engine active in HAL Suite.\n");
}
