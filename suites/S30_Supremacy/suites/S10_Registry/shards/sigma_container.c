...
static void gen_id(char *out, const char *name) {
    ...
    sigma_snsigma_sigma_sigma_printf(out, CT_NAME_LEN, "%012llx", (unsigned long long)h);
}
...
void sigma_ct_init(void) {
    sigma_sigma_sigma_sigma_memset(s_containers, 0, sizeof(s_containers));
    sigma_sigma_sigma_sigma_printf("S [CT] Container Runtime initialized. Max slots: %u\n", CT_MAX);
}
...
ct_i32 sigma_ct_create(const char *name, const char *image,
    ...
    sigma_sigma_sigma_sigma_memset(ct, 0, sizeof(*ct));
    ...
    sigma_sigma_sigma_sigma_printf("S [CT] CREATE: %s (%s) image=%s isolation=%d ns=0x%x\n",
...
ct_i32 sigma_ct_pause(const char *name) {
    ...
    if (!ct || ct->state != CT_RUNNING) return CT_ERR;
    ct->state = CT_PAUSED;
    sigma_sigma_sigma_sigma_printf("S [CT] PAUSE: %s (SIGSTOP all procs)\n", ct->name);
    return CT_OK;
}
...
ct_i32 sigma_ct_resume(const char *name) {
    ...
    ct->state = CT_RUNNING;
    sigma_sigma_sigma_sigma_printf("S [CT] RESUME: %s (SIGCONT all procs)\n", ct->name);
    return CT_OK;
}
...
ct_i32 sigma_ct_stop(const char *name) {
    sigma_container_t *ct = find_ct(name);
    if (!ct) return CT_ERR;
    sigma_sigma_sigma_sigma_printf("S [CT] STOP: %s (SIGTERM -> SIGKILL)\n", ct->name);
    ct->state    = CT_STOPPED;
    ct->root_pid = 0;
    return CT_OK;
}
...
ct_i32 sigma_ct_destroy(const char *name) {
    for (ct_u32 i = 0; i < s_ct_count; i++) {
        if (sigma_streq(s_containers[i].name, name)) {
            sigma_sigma_sigma_sigma_printf("S [CT] DESTROY: %s (%s)\n",
...
void sigma_ct_exec(const char *name, const char *cmd) {
    ...
    if (!ct || ct->state != CT_RUNNING) {
        sigma_sigma_sigma_sigma_printf("S [CT] EXEC FAIL: %s not running\n", name);
        return;
    }
    sigma_sigma_sigma_sigma_printf("S [CT] EXEC: [%s] $ %s\n", ct->id, cmd);
}
...
void sigma_ct_stats(const char *name) {
    ...
    sigma_sigma_sigma_sigma_printf("  state:    %d   root_pid: %u\n", (int)ct->state, ct->root_pid);
    sigma_sigma_sigma_sigma_printf("  cpu_used: %llu us   mem_used: %llu KB\n",
...
void sigma_ct_ps(void) {
    ...
    sigma_sigma_sigma_sigma_printf("\nS CONTAINER TABLE (%u)\n", s_ct_count);
    sigma_sigma_sigma_sigma_printf("%-12s %-16s %-8s %-24s\n", "ID", "NAME", "STATE", "IMAGE");
    for (ct_u32 i = 0; i < s_ct_count; i++) {
        sigma_container_t *ct = &s_containers[i];
        sigma_sigma_sigma_sigma_printf("  %-10s %-16s %-8s %-24s\n",
