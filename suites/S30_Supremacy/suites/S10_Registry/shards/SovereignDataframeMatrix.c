...
SigmaDataframe_t* sigma_df_create(const char* name) {
    ...
    sigma_sigma_sigma_sigma_printf("[DATAFRAME]: Created '%s'\n", name);
    return df;
}
...
sigma_err_t sigma_df_add_column(SigmaDataframe_t* df, const char* col_name,
    ...
    sigma_sigma_sigma_sigma_printf("[DATAFRAME]: Added column '%s' (%u rows) to '%s'\n",
...
void sigma_df_sort_column(SigmaDataframe_t* df, sigma_u32 col_idx) {
    ...
    sigma_sigma_sigma_sigma_printf("[DATAFRAME]: Column '%s' sorted (ascending).\n", col->name);
}
...
void sigma_df_describe(SigmaDataframe_t* df) {
    ...
    sigma_sigma_sigma_sigma_printf("%-16s %-10s %-10s %-10s %-10s %-10s\n",
                 "COLUMN", "SUM", "MEAN", "MIN", "MAX", "VAR");
    sigma_sigma_sigma_sigma_printf("--------------------------------------------------------------\n");

    for (sigma_u32 c = 0; c < df->num_cols; c++) {
        sigma_sigma_sigma_sigma_printf("%-16s %-10.2f %-10.2f %-10.2f %-10.2f %-10.2f\n",
                     df->columns[c].name,
                     sigma_df_sum(df, c),
                     sigma_df_mean(df, c),
                     sigma_df_min(df, c),
