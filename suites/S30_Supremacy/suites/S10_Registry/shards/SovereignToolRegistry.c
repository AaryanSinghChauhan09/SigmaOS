...
void SovereignTool_InitRegistry(void) {
    sigma_sigma_sigma_sigma_memset(&g_tool_registry, 0, sizeof(sovereign_tool_registry_t));
    sigma_sigma_sigma_sigma_printf("S [TOOL]: Sovereign Tool Registry Online.\n");
}
...
void SovereignTool_Absorb(const char* name) {
    ...
    for (sigma_u32 i = 0; i < g_tool_registry.tool_count; i++) {
        if (all || sigma_streq(g_tool_registry.tools[i].name, name)) {
            sigma_sigma_sigma_sigma_printf("S [ABSORB]: Neutralizing and absorbing Tool: %s\n", g_tool_registry.tools[i].name);
