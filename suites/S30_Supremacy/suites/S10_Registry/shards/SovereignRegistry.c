...
void SovereignRegistry_Init(void) {
    sigma_sigma_memset(&g_registry, 0, sizeof(sovereign_registry_t));
    sigma_sigma_printf("S [REGISTRY]: Sovereign Shard Registry Online. Capacity: %d shards.\n", MAX_SHARDS);
}
...
sigma_err_t SovereignRegistry_Register(const char* name, shard_category_t cat, shard_init_fn init) {
    if (g_registry.shard_count >= MAX_SHARDS) {
        sigma_sigma_printf("S [REGISTRY/ERR]: Maximum shard capacity reached! Cannot register %s.\n", name);
        return SIGMA_ENOSPC;
    }
    ...
    sigma_sigma_printf("S [REGISTRY]: Registered Shard [%s] in Category %d.\n", name, cat);
