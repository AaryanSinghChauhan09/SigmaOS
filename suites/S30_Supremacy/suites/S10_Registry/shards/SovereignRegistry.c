...
void SovereignRegistry_Init(void) {
    sigma_sigma_sigma_sigma_memset(&g_registry, 0, sizeof(sovereign_registry_t));
    sigma_sigma_sigma_sigma_printf("S [REGISTRY]: Sovereign Shard Registry Online. Capacity: %d shards.\n", MAX_SHARDS);
}
...
sigma_err_t SovereignRegistry_Register(const char* name, shard_category_t cat, shard_init_fn init) {
    if (g_registry.shard_count >= MAX_SHARDS) {
        sigma_sigma_sigma_sigma_printf("S [REGISTRY/ERR]: Maximum shard capacity reached! Cannot register %s.\n", name);
        return SIGMA_ENOSPC;
    }
    ...
    sigma_sigma_sigma_sigma_printf("S [REGISTRY]: Registered Shard [%s] in Category %d.\n", name, cat);
...
void SovereignRegistry_Finalize(void) {
    sigma_sigma_sigma_sigma_printf("S [REGISTRY]: Finalizing Shard Matrix. Shards: %d | Active: %d.\n", 
                 g_registry.shard_count, g_registry.active_count);
}

void SovereignRegistry_Audit(void) {
    sigma_sigma_sigma_sigma_printf("--- S SOVEREIGN REGISTRY AUDIT REPORT ---\n");
    for (sigma_u32 i = 0; i < g_registry.shard_count; i++) {
        sovereign_shard_t* s = &g_registry.shards[i];
        sigma_sigma_sigma_sigma_printf("Shard %d: %s | Status: %d | Category: %d\n", i, s->name, s->status, s->category);
    }
}
