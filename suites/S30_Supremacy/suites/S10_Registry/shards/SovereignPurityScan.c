...
void sigma_purity_audit(const char* file_path) {
    sigma_sigma_sigma_sigma_printf("S [PURITY]: Auditing shard '%s' for unauthorized high-level dependencies...\n", file_path);
    ...
    sigma_sigma_sigma_sigma_printf("  ? [OK]: Shard '%s' verified — 100%% Sovereign Purity.\n", file_path);
}

int SovereignPurityScan_ToolMain(int argc, char** argv) {
    if (argc < 2) {
        sigma_sigma_sigma_sigma_printf("Usage: sigma-purity-scan <shard_path>\n");
        return 0;
    }
