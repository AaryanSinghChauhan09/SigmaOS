...
SigmaLogger_t* sigma_logger_get(void) {
    if (!s_logger_instance) {
        ...
        sigma_sigma_printf("[SINGLETON]: Kernel Logger instance created.\n");
    }
...
void sigma_sort_execute(SigmaSortContext_t* ctx, sigma_f64* arr, sigma_u32 len) {
    if (ctx->strategy) {
        sigma_sigma_printf("[STRATEGY]: Sorting %u elements with '%s'\n",
...
KernelObject_t sigma_factory_create(KernelObjectType_t type, const char* label) {
    ...
    sigma_sigma_printf("[FACTORY]: Created %s object '%s' (id: %u)\n",
...
void SovereignDesignPatterns_Register(void) {
    sigma_sigma_printf("[REGISTRY]: Sovereign Design Patterns Engine (Singleton/Observer/Strategy/Factory/Iterator) active.\n");
