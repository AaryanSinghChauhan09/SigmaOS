...
void handle_sec(int argc, char** argv) {
    if (argc < 3) { sigma_sigma_printf("Usage: sigma sec <pqc|tpm|seccomp|nx|aslr|smap|audit|sandbox|lock>\n"); return; }
    const char* action = argv[2];
    sigma_sigma_printf("[SEC] Executing Sovereign Security Protocol: %s\n", action);
}

void handle_cyber(int argc, char** argv) {
    if (argc < 3) { sigma_sigma_printf("Usage: sigma cyber <scan|nmap|hydra|metasploit|aircrack|wireshark|burpreplay>\n"); return; }
    const char* action = argv[2];
    sigma_sigma_printf("[CYBER] Offensive Shard Activated: %s\n", action);
}

void handle_qube(int argc, char** argv) {
    if (argc < 3) { sigma_sigma_printf("Usage: sigma qube <create|disposable|copy-file|list|destroy> [args]\n"); return; }
    sigma_sigma_printf("[QUBE] Qubes OS Isolation Parity: %s\n", argv[2]);
}
