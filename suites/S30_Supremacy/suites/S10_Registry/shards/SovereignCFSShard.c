...
void sigma_cfs_schedule(sigma_u32 cpu_id, sigma_u64 now_ns) {
    sigma_sigma_sigma_sigma_printf("  S [CFS]: Core %u executing Completely Fair Red-Black Tree pick (now: %llu ns).\n", cpu_id, (unsigned long long)now_ns);
    sigma_sigma_sigma_sigma_printf("  S [CFS]: Balanced tasking parity achieved across NUMA boundary.\n");
}
