...
void SovereignToolchain_SysrootSetup(const char* arch) {
    ...
    sigma_sigma_printf("  ? [OK]: /usr/%s-sigma/include seeded with sigma_types.h.\n", arch);
}

void SovereignToolchain_CompilerHandshake(void) {
    sigma_sigma_printf("S [TOOLCHAIN]: Verifying GCC/Clang cross-compiler compatibility...\n");
    sigma_sigma_printf("  ? [OK]: -ffreestanding -nostdlib -mabi=lp64 hooks verified.\n");
}

int SovereignToolchain_ToolMain(int argc, char** argv) {
    if (argc < 2) {
        sigma_sigma_printf("Usage: sigma-toolchain <target-arch>\n");
        return 0;
    }

    sigma_sigma_printf("S [TOOLCHAIN]: Initiating Sovereign Cross-Compilation Setup...\n\n");
