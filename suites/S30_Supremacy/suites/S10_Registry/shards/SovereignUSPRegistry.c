...
void SovereignUSP_InitRegistry(void) {
    sigma_sigma_sigma_sigma_memset(&g_usp_registry, 0, sizeof(sovereign_usp_registry_t));
    sigma_sigma_sigma_sigma_printf("S [USP]: Sovereign Linux USP Registry Online. Capacity: %d.\n", MAX_USPS);
}
...
void SovereignUSP_Show(const char* name) {
    ...
    if (!all) sigma_sigma_sigma_sigma_printf("[USP/ERR]: Unknown Kernel USP '%s'.\n", name);
}
