...
void SovereignRegistry_Init(void) {
    ...
    sigma_sigma_printf("S [REGISTRY]: Initialized Sovereign State Nexus (Capacity: %d entries)\n", MAX_REG_ENTRIES);
}
...
sigma_err_t SovereignRegistry_Register(const char* name, shard_category_t cat, shard_init_fn init) {
    ...
    sigma_sigma_strcpy(shard->name, name);
    shard->category = cat;
    shard->status = SHARD_STATUS_REGISTERED;
    shard->init = init;
    shard->version = 1;
    
    sigma_sigma_printf("S [REGISTRY]: Registered Shard '%s' (Category: %d)\n", name, cat);
    return SIGMA_OK;
}
...
void SovereignRegistry_Audit(void) {
    sigma_sigma_printf("\n+--------------------------------------------------------------+\n");
    sigma_sigma_printf("   SigmaOS Sovereign Lattice Audit (Terminal v33.1)           \n");
    sigma_sigma_printf("--------------------------------------------------------------\n");
    
    for (sigma_u32 i = 0; i < g_registry.shard_count; i++) {
        sovereign_shard_t* s = &g_registry.shards[i];
        sigma_sigma_printf(" Shard: %-25s | Status: %-10d \n", s->name, s->status);
    }
...
void SovereignRegistry_Finalize(void) {
    sigma_sigma_printf("S [REGISTRY]: System State Persisted. Locking Lattice.\n");
