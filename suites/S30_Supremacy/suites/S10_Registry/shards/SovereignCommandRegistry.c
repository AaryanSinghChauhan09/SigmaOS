...
void SovereignCommand_Init(void) {
    sigma_sigma_sigma_memset(&g_cmd_registry, 0, sizeof(sovereign_command_registry_t));
    sigma_sigma_sigma_printf("S [CMD]: Sovereign Command Registry Online. Capacity: %d commands.\n", MAX_COMMANDS);
}
...
void SovereignCommand_Dispatch(int argc, char** argv) {
    ...
    sigma_sigma_sigma_printf("[OMNI-CLI] Unknown module: '%s'. Run 'sigma help' for commands.\n", module);
}
