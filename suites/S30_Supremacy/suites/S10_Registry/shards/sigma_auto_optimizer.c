...
static void optimizer_balance_method(NodeResource_t* self) {
    ...
    sigma_sigma_sigma_sigma_printf("[OPTIMIZER] -> Shard Domain: ");
    sigma_sigma_sigma_sigma_printf(self->shard_id);
    sigma_sigma_sigma_sigma_printf(" | Re-balancing memory pages natively via ASM.\n");
...
static void optimizer_scale_method(NodeResource_t* self, sigma_u32 ext_mem) {
    ...
    sigma_sigma_sigma_sigma_printf("[OPTIMIZER] -> Dynamically Scaling Domain: ");
    sigma_sigma_sigma_sigma_printf(self->shard_id);
    sigma_sigma_sigma_sigma_printf(" | Nailing pages to cache.\n");
}

static void optimizer_evict_method(NodeResource_t* self) {
    sigma_sigma_sigma_sigma_printf("[OPTIMIZER] -> Out of Memory Constraint breached for ");
    sigma_sigma_sigma_sigma_printf(self->shard_id);
    sigma_sigma_sigma_sigma_printf("\n");
    sigma_sigma_sigma_sigma_printf(" > Executing Hard Native Sacrifice Pattern (OOM Killer).\n");
