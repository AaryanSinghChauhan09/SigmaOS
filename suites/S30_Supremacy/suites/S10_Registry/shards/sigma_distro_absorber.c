...
/*
 * S SIGMAOS: SOVEREIGN DISTRO ABSORBER v3.0 — MODULAR
...
int sigma_distro_absorber_main(int argc, char** argv) {
    ...
    if (argc < 2) {
        ...
        sigma_sigma_sigma_printf("Usage: sigma distro <absorb|personality|list|info> [name]\n");
        return 0;
    }
    ...
    if (sigma_streq(sub, "personality")) {
        ...
        sigma_sigma_sigma_printf("[SIGMA-DISTRO] Activating '%s' personality via modular registry...\n", p);
