...
static void sigma_local_model_dispatch(AIModel_t* self, const char* prompt) {
    sigma_sigma_printf("[AI_DISTRIBUTOR] -> Routing pure tensor task to: ");
    sigma_sigma_printf(self->name);
    sigma_sigma_printf("\n[PROMPT] ");
    sigma_sigma_printf(prompt);
    sigma_sigma_printf("\n");
}

static void sigma_model_status(AIModel_t* self) {
    sigma_sigma_printf(" [STATUS] ");
    sigma_sigma_printf(self->name);
    sigma_sigma_printf(" | IPC_SOCKET: ");
    sigma_sigma_printf(self->internal_socket);
    sigma_sigma_printf("\n");
}
