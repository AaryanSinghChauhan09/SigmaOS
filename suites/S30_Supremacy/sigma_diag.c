...
/* Read a Model-Specific Register â€ Ring-0 required */
...
/* CPUID â€ reads processor capabilities */
...
static void diag_probe_cpu(SovereignDiagnosticsZenith* d) {
    ...
    sigma_log("[RAW FREQ=%llu MHz | TSC=%llu]\n", d->cpu_freq_mhz, d->last_tsc);
}
...
static void diag_probe_thermal(SovereignDiagnosticsZenith* d) {
    ...
    sigma_log("[THERMAL JUNCTION STABLE | TJ_offset=%u]\n", offset);
}
...
static void diag_extract_kernel_ring(SovereignDiagnosticsZenith* d) {
    ...
    d->hardware_probes++;
    sigma_log("[RING SLICED O(1) | First64=%lu]\n", val);
}
...
static void diag_audit_all(SovereignDiagnosticsZenith* d) {
    ...
    sigma_log("| Total Probes   : %u\n", d->hardware_probes);
    sigma_log("| CPU Freq       : %llu MHz\n", d->cpu_freq_mhz);
    sigma_log("| Competitors    : htop/lm-sensors/dmesg neutralized.\n");
    sigma_print("------------------------------------------------------\n");
}
