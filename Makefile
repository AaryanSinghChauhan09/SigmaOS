...
ZENITH_OBJ_SENT  := build/SovereignSentinel.o
...
$(ZENITH_OBJ_PULSE): SovereignSiliconPulse.asm
	@mkdir -p build
	@echo "[NASM] SovereignSiliconPulse.asm"
	@$(NASM) $(ASMFLAGS) $< -o $@

$(ZENITH_OBJ_ENTRY): SovereignEntry.asm
	@mkdir -p build
	@echo "[NASM] SovereignEntry.asm"
	@$(NASM) $(ASMFLAGS) $< -o $@

$(ZENITH_OBJ_MATH): SovereignMath.asm
	@mkdir -p build
...
$(ZENITH_OBJ_SENT): SovereignSentinel.asm
	@mkdir -p build
	@echo "[NASM] SovereignSentinel.asm"
	@$(NASM) $(ASMFLAGS) $< -o $@
...
build/SovereignAetherAbsorption_c.o: SovereignAetherAbsorption.c SovereignLibC.h
	@$(CC) $(CFLAGS) -c $< -o $@
