...
public:
    ...
    bool loadFedoraDriver(const char* rpm_firmware_path) {
        ...
        sigma_log_info(rpm_firmware_path);
...
extern "C" void fedora_driver_load(const char* path) {
    SigmaOS::Kernel::Drivers::FedoraDriverCompat::getInstance().loadFedoraDriver(path);
}
