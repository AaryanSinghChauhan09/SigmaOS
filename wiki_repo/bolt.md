...
## 2026-08-31 - Caching Stored Byte Lengths for SimpleApplication Launcher Structs
**Learning:** In application launcher and desktop search hot paths (`SimpleApplication` in `src/desktop/launcher.rs`), calling `app.name()`, `app.executable()`, or `app.icon()` repeatedly executed $O(N)$ linear byte scans (`.position(|&b| b == 0)`) over 64-byte name and 256-byte executable/icon byte arrays on every app inspection, launcher search, or dock render query. Storing `name_len: u8`, `exec_len: u8`, and `icon_len: u8` during `SimpleApplication::new()` initialization turns slice retrieval into instantaneous $O(1)$ constant-time lookups `&self.name[..self.name_len as usize]`, eliminating linear scanning overhead during high-frequency application launcher searches and UI queries.
**Action:** Always store explicit byte lengths alongside fixed-size byte array fields in desktop launcher and application manager structures to guarantee $O(1)$ constant-time slice lookups.
