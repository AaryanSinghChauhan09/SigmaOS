...
> Maps the Advanced Python syllabus (NumPy, Pandas, Matplotlib, Scikit-Learn) to SigmaOS analytics components.

---

## Unit I: Data Science Essentials & NumPy
...
```bash
# SigmaOS ships with Anaconda-compatible Python environment
...
# Reshaping
df.set_index('timestamp').stack()  # wide to long
df_pivot = df.pivot(index='date', columns='process', values='cpu_pct')
df_melt  = df.melt(id_vars=['pid'], value_vars=['cpu_pct','mem_mb'])

# Transformations
df.drop_duplicates(subset=['pid'])
df['cpu_pct'].replace({-1: 0, None: 0})
df = df.rename(columns={'cpu_pct': 'cpu_percent'})
df['cpu_pct'].clip(lower=0, upper=100)   # Remove outliers
df = pd.get_dummies(df, columns=['process'])  # One-hot encoding

# String operations
df['process'].str.upper()
df['process'].str.contains('sigma')
df['process'].str.extract(r'sigma-(\w+)')
...
axes[0,1].bar(df['process'], df['cpu_pct'], color='#4CAF50')
...
axes[1,1].scatter(df['cpu_pct'], df['mem_mb'], c='#00BCD4', s=100, alpha=0.7)
