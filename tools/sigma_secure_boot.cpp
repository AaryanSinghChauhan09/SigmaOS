/*
 * =========================================================================
 * Î£ SIGMAOS: SIGMA SECURE BOOT MANAGER (sigma_secure_boot) v1.0
 * =========================================================================
...
public:
    ...
    bool verify_image(const char* image_name, const sigma_u8* signature, sigma_u32 sig_len) {
        ...
        if (m_keys_loaded == 0) {
            sigma_log_infoor("[SECBOOT] Verification failed for '%s': No keys loaded.", image_name);
            return false;
        }

        if (!signature || sig_len == 0) {
            sigma_log_infoor("[SECBOOT] Verification failed for '%s': Missing signature.", image_name);
            return false;
        }
