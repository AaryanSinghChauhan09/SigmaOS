...
/// Formats RedHat/Vixie-style mail headers for cron job output logging
#[no_mangle]
pub unsafe extern "C" fn cron_format_mail_output(job_index: usize, output_buf: *mut u8, buf_len: usize) -> isize {
    if job_index >= CRON_JOB_COUNT as usize || output_buf.is_null() || buf_len < 32 {
        return -1;
    }

    let job = &CRON_JOBS[job_index];
    let mailto_len = job.mailto.iter().position(|&b| b == 0).unwrap_or(64);

    // Construct header "To: " + mailto
    let prefix = b"To: ";
    let mut written = 0;

    for &b in prefix {
        if written < buf_len - 1 {
            *output_buf.add(written) = b;
            written += 1;
        }
    }

    for i in 0..mailto_len {
        if written < buf_len - 1 {
            *output_buf.add(written) = job.mailto[i];
            written += 1;
        }
    }

    *output_buf.add(written) = 0; // Null terminator
    written as isize
}

/// Calculates Arch/systemd-timer-style thundering herd mitigation jitter delay
#[no_mangle]
pub unsafe extern "C" fn cron_calculate_jitter_delay(job_index: usize) -> u32 {
    if job_index >= CRON_JOB_COUNT as usize {
        return 0;
    }

    let job = &CRON_JOBS[job_index];
    if job.randomized_delay_sec == 0 {
        return 0;
    }

    // Pseudo-random deterministic delay formula based on job index and randomized_delay_sec
    let jitter = ((job_index as u32 + 1).wrapping_mul(1103515245).wrapping_add(12345)) % job.randomized_delay_sec;
    jitter
}

/// Enforces Alpine/Busybox-style user sandbox privilege validation
#[no_mangle]
pub unsafe extern "C" fn cron_validate_user_privileges(job_index: usize, executing_uid: u32) -> isize {
    if job_index >= CRON_JOB_COUNT as usize {
        return -1;
    }
    ...
    // Root user (uid 0) can execute any job
    if executing_uid == 0 {
        return 0;
    }

    // Non-root user must match the job's assigned run_as_user
    if executing_uid == job.run_as_user {
        0
    } else {
        -1 // Permission denied
    }
...
mod tests {
    ...
    fn test_all_cron_sequential() {
        unsafe {
            ...
            test_mail_jitter_and_privileges();
        }
    }
    ...
    unsafe fn test_mail_jitter_and_privileges() {
        cron_init();
        cron_reset_logs();

        let res = cron_add_job_linux(
            b"mail_jitter_job\0".as_ptr(),
            b"echo mail_test\0".as_ptr(),
            b"*\0".as_ptr(),
            b"*\0".as_ptr(),
            b"*\0".as_ptr(),
            b"*\0".as_ptr(),
            b"*\0".as_ptr(),
            CronCategory::Custom as u8,
            1001, // run_as_user = 1001
            120,  // randomized_delay_sec = 120
            1,
            0,
            true,
            core::ptr::null(),
        ...
        // 1. Test Mail Formatting
        let mut mail_buf = [0u8; 64];
        let written = cron_format_mail_output(0, mail_buf.as_mut_ptr(), 64);
        assert!(written > 0);
        let mail_str = core::str::from_utf8(&mail_buf[..written as usize]).unwrap();
        assert_eq!(mail_str, "To: admin@sigmaos.org");

        // 2. Test Jitter Calculation
        let jitter = cron_calculate_jitter_delay(0);
        assert!(jitter < 120);

        // 3. Test Privilege Validation
