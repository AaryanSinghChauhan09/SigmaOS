/*
 * =========================================================================
 * Î£ SIGMAOS: SIGMA QUANTUM SIMULATOR (sigma_quantum_simulator) v1.0
 * =========================================================================
...
public:
    ...
    void load_circuit(sigma_u32 qubits) {
        if (qubits > m_qubit_capacity) {
            sigma_log_infoor("[QUANTUM] Circuit exceeds simulator capacity (%u > %u).", qubits, m_qubit_capacity);
            return;
        }
    ...
    void execute_circuit() {
        if (!m_active_circuit) {
            sigma_log_infoor("[QUANTUM] No circuit loaded.");
            return;
        }
