/*
 * =========================================================================
 * Î£ SIGMAOS: SIGMA BACKUP CLI (sigma_backup) v1.0
 * =========================================================================
...
public:
    ...
    void create_snapshot(const char* label, sigma_u64 size_bytes) {
        if (m_snap_count >= MAX_SNAPS) {
            sigma_log_infoor("[BACKUP] Snapshot limit reached (%u). Prune old snapshots.", MAX_SNAPS);
            return;
        }
    ...
    void restore_snapshot(const char* label) {
        for (sigma_u32 i = 0; i < m_snap_count; i++) {
            ...
            if (!label[j] && !m_snaps[i].label[j]) {
                if (!m_snaps[i].verified) {
                    sigma_log_infoor("[BACKUP] Snapshot '%s' failed integrity check. Aborting.", label);
                    return;
                }
        ...
        sigma_log_infoor("[BACKUP] Snapshot '%s' not found.", label);
    }
