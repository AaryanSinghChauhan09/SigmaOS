# AGENTS.md — AI Agent Guidelines, Versioning, Access Control Objects, Thread Sync, HTML Dependency Reduction, Compile-Time Defenses, Clock, Circular Buffers, Cache Memory, Process, Hardware Fitting, Backend, Loading, Processor & Network Management for SigmaOS

This document provides instructions, rules, and procedures for AI agents working in the SigmaOS repository, specifically regarding **Version Handling**, **Release Channels**, **Control Objects, ACLs & Zero Trust Gate**, **Thread Synchronization & Atomic Operations**, **HTML Dependency Reduction & Text-First Architecture**, **Compile-Time Defenses & Build Hardening**, **Clock & Timer Management**, **Circular Buffer & Ring Buffer Management**, **Cache Memory Management**, **Process Lifecycle & Signal ABI Management**, **Hardware Fitting & Driver Auto-Binding**, **Network Stack & eBPF Management**, **Processor Subsystem Management**, **Backend Management**, **Bootloader & Driver Loading**, **Multi-Distro Packaging Parity**, and **Core Subsystem Changes**.
...
## 3. Control Objects, ACLs & Zero Trust Gate Rules for AI Agents

When implementing or evaluating access control objects in `src/access/control.rs` or `src/security/capability.rs`:

1. **DAC & POSIX Octal Mode Bits (`DacPermission`):**
   Evaluate owner UID, group GID, and mode octal bits (`0o755`, `0o4755`). Preserve SUID, SGID, and Sticky bit deletion restrictions.
2. **Capability Token Bitmask Verification:**
   `CapabilityToken::has_permission` MUST use bit-shifting (`1 << permission`) so capability variants map cleanly to bitmasks.
3. **AppArmor Path MAC (`UbuntuAppArmorProfile`):**
   Enforce path pattern matching rules and audit non-conforming access attempts in `Enforcing` or `Complain` mode.

---

## 4. Thread Synchronization & Lock-Free Atomic Rules for AI Agents
...
## 5. HTML Dependency Reduction & Text-Based Interface Rules for AI Agents
...
## 6. Compile-Time Defenses & Build Hardening Rules for AI Agents
...
## 7. Clock Algorithm & Timer Management Rules for AI Agents
...
## 8. Circular Buffer & Lock-Free Ring Buffer Rules for AI Agents
...
## 9. Cache Memory Architecture, LRU Eviction & Package Cache Rules for AI Agents
...
## 10. Process Lifecycle, Signal ABI Translation & Supervision Rules for AI Agents
...
## 11. Hardware Fitting, Driver Auto-Binding & Device Adaptation Rules for AI Agents
...
## 12. Network Stack, eBPF/XDP & PQC Security Rules for AI Agents
...
## 13. Processor Topology, CPU Scheduling & Multi-Core Rules for AI Agents
...
1. **Update Manifests & Documentation** when bumping versions, adding drivers, or modifying access control logic.
...
4. **Follow Conventional Commits:**
   `docs(agents): add control objects management guide` or `fix(access): enforce SUID/SGID evaluation`.
...
* [`docs/AGENTS_CONTROL_OBJECTS_MANAGEMENT.md`](docs/AGENTS_CONTROL_OBJECTS_MANAGEMENT.md)
