# AGENTS.md — AI Agent Guidelines, Versioning, Backend, Loading, Processor & Network Management for SigmaOS

This document provides instructions, rules, and procedures for AI agents working in the SigmaOS repository, specifically regarding **Version Handling**, **Release Channels**, **Network Stack & eBPF Management**, **Processor Subsystem Management**, **Backend Management**, **Bootloader & Driver Loading**, **Multi-Distro Packaging Parity**, and **Core Subsystem Changes**.
...
## 3. Network Stack, eBPF/XDP & PQC Security Rules for AI Agents
...
5. **Bluetooth Stack Multi-OS Parity:**
   Bluetooth drivers (`src/bluetooth/`) MUST support BlueZ CLI commands (`bluetoothctl`) and BSD HCI controller configuration (`hciconfig`).

---

## 4. Processor Topology, CPU Scheduling & Multi-Core Rules for AI Agents
...
3. **NUMA-Aware Work Stealing:**
   Ensure SMP worker threads attempt work-stealing from local NUMA node cores before stealing across socket interconnects.
4. **Task Descriptor $O(1)$ Name Lookup:**
...
## 5. Kernel, Bootloader & Driver Loading Rules for AI Agents
...
3. **Dynamic Driver Discovery:**
   Drivers registered in `src/drivers/` MUST check device bus signatures before initializing and register cleanly in `src/drivers/mod.rs` and `src/lib.rs`.

---

## 6. Backend Subsystem & Server Engine Rules for AI Agents
...
## 7. Kernel ABI (KABI) Versioning & Stability
...
1. **Update Manifests & Documentation** when bumping versions, adding drivers, or modifying networking logic.
...
4. **Follow Conventional Commits:**
   `feat(net): add eBPF XDP zero-copy packet filter` or `docs(agents): add network management guide`.

---

## 9. Detailed Documentation References
