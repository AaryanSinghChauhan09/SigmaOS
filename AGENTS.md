...
## ⚡ Process Interaction Operation Principles

1. **Unmaskable Signals (`src/ipc/signals.rs`)**
   - `SIGKILL` cannot be masked (`sigprocmask`) or ignored (`sigaction`). Always preserve `mask & !sigkill_bit`.

2. **Capability-Gated Signal Delivery (`IPCCapability`)**
   - Enforce `can_send` and `can_receive` capability checks prior to queuing inter-process signals.

3. **Real-time Signal Payloads**
   - Support optional binary payload queues (`PendingSignal.payload`) for real-time userland signal dispatch (`SigUsr1`/`SigUsr2`).

---

## 📄 Fill-On-Demand & Memory Fault Management Principles

1. **VMA On-Demand Paging (`src/memory/paging.rs`)**
   - Register virtual memory areas (`VirtualMemoryArea`) and lazily allocate physical page frames on demand during page fault resolution (`attempt_demand_paging`).

2. **Copy-on-Write (CoW) Page Splitting**
   - Intercept write intents on KSM read-only pages to allocate unique physical page frames and update KSM registry references.

3. **zram Compressed Swap Restore**
   - Decompress and restore page frames on demand when accessing pages evicted into `zram_pool`.
