...
## 15. AI Agent Process Interaction Management Directives
- **Lock-Free Ring Buffer IPC**: High-throughput IPC must utilize lock-free atomic ring buffers (`src/klib/ringbuf.rs`, `src/klib/ring_buffer.rs`) to prevent microkernel lock contention.
- **Capability-Gated Signals & Inter-Process Access**: Process signaling and state inspection require explicit `CapabilityGate` grants (`src/security/capability.rs`, `src/security/sigma_unveil.rs`).
- **Zero-Copy Page Loans for Shared Memory**: Shared memory exchanges must leverage zero-copy virtual memory page loans (`uvm_page_loans` in `src/klib/uvm.rs`).

## 16. AI Agent Thread Synchronization Operation Management Directives
- **RCU Read-Path Supremacy**: Lock-free RCU read paths (`rcu_read_lock` / `rcu_read_unlock`) must be used for read-heavy kernel structures.
- **Sequential Consistency Fences**: Peterson's algorithm and software lock-free operations must issue `core::sync::atomic::fence(Ordering::SeqCst)` memory barriers (`src/klib/sync.rs`).
- **Priority Inheritance Mutexes**: Blocking mutexes in real-time scheduler contexts must enforce Priority Inheritance Protocol (PIP) to prevent priority inversion.

For detailed specifications, see `docs/AGENTS_MEMORY_MANAGEMENT.md`, `docs/AGENTS_TESTING_GUIDELINES.md`, `docs/AGENTS_EFFICIENCY_GUIDELINES.md`, `docs/AGENTS_KERNEL_MANAGEMENT.md`, `docs/AGENTS_FILESYSTEM_MANAGEMENT.md`, `docs/AGENTS_BLOCK_DEVICE_DRIVERS_MANAGEMENT.md`, `docs/AGENTS_BOTTOM_HALF_THREADS.md`, `docs/AGENTS_MAIN_MEMORY_MANAGEMENT.md`, `docs/AGENTS_CACHE_SIZE_MANAGEMENT.md`, `docs/AGENTS_CLOUD_CARRIER_OPERATION.md`, `docs/AGENTS_CACHE_OPERATION_MANAGEMENT.md`, `docs/AGENTS_CAD_MANAGEMENT.md`, `docs/AGENTS_PROCESS_INTERACTION_MANAGEMENT.md`, `docs/AGENTS_THREAD_SYNC_OPERATION_MANAGEMENT.md`, and `docs/memory-management.md`.
