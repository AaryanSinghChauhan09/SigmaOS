...
*   **Address ReDoS Risks in Desktop Node Tooling:**
    Upgrade dependency `brace-expansion` inside `package.json` to `^2.0.1` and `nanoid` to `^3.3.18` and run lockfile synchronization to eliminate GHSA-mh99-v99m-4gvg and loop risks.
...
*   Provide a default flat configuration for ESLint to ensure JS/TS components are validated seamlessly inside CI environments.
