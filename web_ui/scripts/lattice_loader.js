...
function loadLatticeModules(modules, callback) {
    ...
    function loadNext() {
        ...
        // Use document.createElement and appendChild for faster sequential loading without freezing rendering
        let script = document.createElement('script');
        script.src = modules[loadedCount];
        script.onload = () => {
            loadedCount++;
            loadNext();
        };
        script.onerror = () => {
            console.error('Failed to load module:', modules[loadedCount]);
            loadedCount++;
            loadNext();
        };
...
window.startLatticeBootSequence = function(callback) {
    loadLatticeModules(LATTICE_MODULES, callback);
};
