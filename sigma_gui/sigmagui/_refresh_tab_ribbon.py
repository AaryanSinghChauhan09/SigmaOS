"""
Auto-split from sigma_gui.py — SigmaGUI._refresh_tab_ribbon
"""
...
class SigmaGUI:
    def _refresh_tab_ribbon(self):
        ...
        if not hasattr(self, '_tab_ribbon') or not self._tab_ribbon.winfo_exists():
            return
        for w in self._tab_ribbon.winfo_children():
            w.destroy()
        cur = self._active_tab.get()
        for key in self._active_tabs:
            is_active = key == cur
            t_fr = tk.Frame(self._tab_ribbon, bg=PAL['bg3'] if is_active else PAL['bg2'], padx=10, highlightthickness=1, highlightbackground=PAL['accent'] if is_active else PAL['bg4'])
            t_fr.pack(side='left', padx=1, fill='y')
            icon_map = {'dashboard': '🏠', 'browser': '🌐', 'explorer': '📁', 'brain': '🧠', 'projects': '📊'}
            icon = icon_map.get(key, '💠')
            lbl = tk.Label(t_fr, text=f'{icon} {key.title()}', font=('Inter Bold' if is_active else 'Inter', 8), fg='white' if is_active else PAL['dim'], bg=PAL['bg3'] if is_active else PAL['bg2'])
