"""
Auto-split from sigma_gui.py — SigmaGUI._show_splash
"""
    ...
    def _show_splash(self):
        ...
        def _anim(step):
            ...
            if step <= 100:
                ...
                if step == 20:
                    msg_var.set('Engaging Hyper-Boot Sequence...')
                if step == 80:
                    msg_var.set('Sovereign Core Initialized.')
                splash.after(1, lambda: _anim(step + 40))
            else:
                splash.destroy()
                self.deiconify()
        if self._ultra_perf.get():
