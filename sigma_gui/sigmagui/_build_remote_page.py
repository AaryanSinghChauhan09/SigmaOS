"""
Auto-split from sigma_gui.py — SigmaGUI._build_remote_page
"""
...
class SigmaGUI:
    def _build_remote_page(self):
        ...
        tk.Label(p, text='📱  Aura Remote Hub: Universal Control', font=FONT_LOGO, fg=PAL['cyan'], bg=PAL['bg']).pack(anchor='w', pady=(0, 8))
        ...
        devices = [('Samsung Smart TV', 'TV', 'Samsung Smart'), ('Mi Projector Pro', 'Projector', 'Mi Projector'), ('Daikin Inverter AC', 'AC', 'Daikin'), ('Sony Atmos Audio', 'Audio', 'Sony Soundbar'), ('Xiaomi Mi TV Hub', 'TV', 'Xiaomi Mi TV')]
        for label, d_type, brand in devices:
            ttk.Button(dev_card, text=f'Mirror {label}', command=lambda t=d_type, b=brand: self._remote_mirror(t, b)).pack(fill='x', pady=2)
