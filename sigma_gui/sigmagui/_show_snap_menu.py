"""
Auto-split from sigma_gui.py — SigmaGUI._show_snap_menu
"""
...
class SigmaGUI:
    def _show_snap_menu(self):
        ...
        layouts = [('Tiling (50/50)', 'TILING'), ('Quarters (2x2)', 'QUARTERS'), ('Pillar (1/3rd)', 'PILLAR'), ('Sidebar (70/30)', 'SIDEBAR'), ('Floating', 'FLOATING')]
        for name, lid in layouts:
            btn = tk.Button(fr, text=name, font=FONT_SMALL, bg=PAL['bg3'], fg=PAL['text'], activebackground=PAL['accent'], relief='flat', anchor='w', command=lambda l=lid: self._apply_snap_layout(l))
