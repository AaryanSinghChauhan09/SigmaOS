"""
Auto-split from sigma_gui.py — SigmaGUI._start_live_stats
"""
                ...
                try:
                    ...
                    def _gui_upd(c=cpu, r=ram):
                        if hasattr(self, '_cpu_pb') and self._cpu_pb.winfo_exists():
                            self._cpu_pb.config(width=c * 2)
                        if hasattr(self, '_ram_pb') and self._ram_pb.winfo_exists():
                            self._ram_pb.config(width=r * 2)
                        if 'cpu' in self._stat_widgets:
                            self._stat_widgets['cpu'].set(f'{c}%')
                        if 'ram' in self._stat_widgets:
                            self._stat_widgets['ram'].set(f'{r}%')
