"""
Auto-split from sigma_gui\sigmagui\_build_apex_page.py — SigmaGUI._build_apex_page
"""
...
class SigmaGUI:
    def _build_apex_page(self):
        ...
        self._pages['apex'] = p
        tk.Label(p, text='🏔️  Sovereign Apex: Multi-OS Fusion Hub', font=FONT_LOGO, fg=PAL['cyan'], bg=PAL['bg']).pack(anchor='w', pady=(0, 8))
        ...
        s_ent = ttk.Entry(spot_c)
        s_ent.pack(fill='x', pady=5)
        s_ent.insert(0, 'Search files, AI, or system...')
        ...
        for k, v in stats['Toggles'].items():
            btn = tk.Button(ctrl_c, text=f"{k}: {('ON' if v else 'OFF')}", bg=PAL['bg'] if v else PAL['bg2'], fg=PAL['text'], command=lambda k=k: self._log_voice(self.kernel.controls.toggle_state(k)))
