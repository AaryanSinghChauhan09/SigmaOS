"""
Auto-split from sigma_gui.py — SigmaGUI._handle_ms_shortcut
"""
...
class SigmaGUI:
    def _handle_ms_shortcut(self, key):
        ...
        mapping = {'d': lambda: self._show_page('dashboard'), 'e': lambda: self._set_modular_page('explorer', SiloPage), 'i': lambda: self._show_page('config_hub'), 's': lambda: self._show_page('search'), 'r': lambda: self._show_spotlight(), 'l': lambda: self._lock_screen(), 'a': lambda: self._show_page('automation_hub'), 'g': lambda: self._show_page('gaming_hub'), 'p': lambda: self._show_page('reports'), 'v': lambda: self._notify('CLIPBOARD', 'Sovereign Clipboard History: Encrypted.', 'INFO'), 'w': lambda: self._show_page('intelligence_hub'), 'c': lambda: self._show_page('zenith'), 'u': lambda: self._show_page('ag_guide'), 'q': lambda: self._show_page('search'), 'x': lambda: self._show_quick_link_menu(), 'k': lambda: self._show_page('aether'), 'z': lambda: self._notify('SNAP', 'Snap Layouts: AI Optimized for Focus.', 'OK')}
        if key in mapping:
            mapping[key]()
