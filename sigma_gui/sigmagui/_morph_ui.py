"""
Auto-split from sigma_gui.py — SigmaGUI._morph_ui
"""
...
class SigmaGUI:
    def _morph_ui(self, event=None):
        ...
        if ld:
            res = ''
            if mode == 'MOBILE':
                res = ld.detect_and_adapt(360, 800, True)
            if mode == 'TABLET':
                res = ld.detect_and_adapt(800, 1200, True)
            if mode == 'DESKTOP':
                res = ld.detect_and_adapt(1920, 1080, False)
