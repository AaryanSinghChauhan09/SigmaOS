"""
Auto-split from sigma_gui.py — SigmaGUI._show_omni_suggest
"""
...
class SigmaGUI:
    def _show_omni_suggest(self, event=None):
        ...
        if self._suggest_pop and self._suggest_pop.winfo_exists():
            for widget in self._suggest_pop.winfo_children():
                widget.destroy()
        for s in matches:
            btn = tk.Button(self._suggest_pop, text=s, font=FONT_SMALL, bg=PAL['bg3'], fg=PAL['text'], activebackground=PAL['accent'], relief='flat', anchor='w', padx=10, command=lambda val=s: [self._intent_var.set(val), self._suggest_pop.destroy()])
            btn.pack(fill='x')
