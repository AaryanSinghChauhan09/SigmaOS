"""
Auto-split from sigma_gui.py — SigmaGUI._build_shield_page
"""
...
class SigmaGUI:
    def _build_shield_page(self):
        ...
        tk.Label(p, text='🛡️ Speed & Shield: Performance & Privacy Engine', font=FONT_LOGO, fg=PAL['cyan'], bg=PAL['bg']).pack(anchor='w', pady=(0, 8))
        ...
        levels = [('Basic', 'Allow essential 3rd-party cookies.'), ('Balanced', 'Block most trackers, keep logins.'), ('Strict', 'Shred all non-essential identity markers.')]
        for name, desc in levels:
            btn = ttk.Button(c_priv, text=f'Enable {name} Mode', command=lambda n=name: self._set_browser_priv(n))
        ...
        opts = [('Demand Paging', 'Lazy-load inodes to save RAM.'), ('Connection Pooling', 'Zero-latency socket reuse.'), ('SSD Alignment', 'Native NAND page boundary writes.'), ('Async I/O', 'Non-blocking sovereign file operations.')]
        for opt, d in opts:
