"""
Auto-split from sigma_gui.py — SigmaGUI._build_fabric_page
"""
...
class SigmaGUI:
    def _build_fabric_page(self):
        ...
        tk.Label(p, text='🧠  Neural Fabric: Predictive Distributed Compute', font=FONT_LOGO, fg=PAL['cyan'], bg=PAL['bg']).pack(anchor='w', pady=(0, 8))
        body = tk.Frame(p, bg=PAL['bg'])
        body.pack(fill='both', expand=True)
        ctrl = self._card(body, '🔮  Intent Prediction & Prefetch')
        ctrl.master.pack(side='left', fill='y', padx=(0, 6))
        ttk.Button(ctrl, text='Pre-fetch Work Environment', command=lambda: self._fabric_exec('Work')).pack(fill='x', pady=4)
        ttk.Button(ctrl, text='Neural-Warm Creative Suite', command=lambda: self._fabric_exec('Creative')).pack(fill='x', pady=4)
        ttk.Button(ctrl, text='Optimize Performance Profile', command=lambda: self._fabric_exec('Performance')).pack(fill='x', pady=4)
