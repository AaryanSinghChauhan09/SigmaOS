"""
Auto-split from sigma_gui.py — SigmaGUI._forge_op
"""
...
class SigmaGUI:
    def _forge_op(self, mode):
        ...
        if cf:
            res = ''
            if mode == 'Audit':
                res = cf.process_document('local_doc.pdf', mode)
            if mode == 'OCR':
                res = cf.capture_visual_region('Primary_Screen', mode)
            if mode == 'Convert':
                res = cf.convert_file('asset.docx', 'pdf')
