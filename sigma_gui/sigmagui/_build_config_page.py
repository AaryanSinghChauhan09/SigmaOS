"""
Auto-split from sigma_gui.py — SigmaGUI._build_config_page
"""
...
class SigmaGUI:
    def _build_config_page(self):
        ...
        def _show_cfg(cat):
            for w in c_fr.winfo_children():
                w.destroy()
            if cat == 'System':
                self._cfg_system(c_fr)
            if cat == 'Display':
                self._cfg_display(c_fr)
            if cat == 'Network':
                self._cfg_network(c_fr)
            if cat == 'Security':
                self._cfg_security(c_fr)
            if cat == 'Sovereignty':
                self._cfg_sovereignty(c_fr)
            if cat == 'About':
                self._cfg_about(c_fr)
        for cat in ['System', 'Display', 'Network', 'Security', 'Sovereignty', 'About']:
            tk.Button(s_fr, text=cat, font=FONT_MED, bg=PAL['bg2'], fg=PAL['text'], relief='flat', anchor='w', padx=15, command=lambda c=cat: _show_cfg(c)).pack(fill='x', pady=2)
        _show_cfg('About')
