"""
Auto-split from sigma_gui.py — SigmaGUI._build_sidebar
"""
...
class SigmaGUI:
    def _build_sidebar(self, parent):
        fr = tk.Frame(parent, bg=PAL['bg2'], width=70)
        fr.pack(side='left', fill='y')
        fr.pack_propagate(False)
        for key, icon, name in self._get_nav_items():
            btn = tk.Button(fr, text=f'{icon}\n{name.split()[-1]}', font=('Inter', 9), bg=PAL['bg2'], fg=PAL['dim'], bd=0, activebackground=PAL['accent'], command=lambda k=key: self._show_page(k))
