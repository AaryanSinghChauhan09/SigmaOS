"""
Auto-split from sigma_gui.py — SigmaGUI._build_manual_page
"""
...
class SigmaGUI:
    def _build_manual_page(self):
        ...
        tk.Label(p, text='📖  Universal Manual: Sovereign v3.0 Apex', font=FONT_LOGO, fg=PAL['cyan'], bg=PAL['bg']).pack(anchor='w', pady=(0, 8))
        body = tk.Frame(p, bg=PAL['bg'])
        body.pack(fill='both', expand=True)
        left = self._card(body, '📂  Manual Sections')
        left.master.pack(side='left', fill='y', padx=(0, 6))
        self._manual_list = tk.Listbox(left, bg=PAL['bg2'], fg=PAL['text'], font=FONT_MED, borderwidth=0, highlightthickness=1, highlightbackground=PAL['border'], width=30)
