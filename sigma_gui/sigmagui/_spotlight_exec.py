"""
Auto-split from sigma_gui.py — SigmaGUI._spotlight_exec
"""
...
class SigmaGUI:
    def _spotlight_exec(self, event=None):
        ...
        if 'speed' in query or 'fast' in query or 'perf' in query or ('brain' in query):
            self._show_page('fabric')
        elif 'fix' in query or 'health' in query:
            self._do_health()
        elif 'clean' in query or 'file' in query or 'forge' in query:
            self._show_page('forge')
        elif 'rout' in query or 'task' in query or 'automat' in query:
            self._show_page('automator')
        elif 'mesh' in query or 'net' in query or 'cloud' in query:
            self._show_page('mesh')
        elif 'sec' in query or 'lock' in query:
            self._show_page('security')
        elif 'term' in query or 'code' in query:
            self._show_page('terminal')
        elif 'man' in query or 'guide' in query or 'help' in query:
