"""
Auto-split from sigma_gui.py — SigmaGUI._log_voice
"""
...
class SigmaGUI:
    def _log_voice(self, message):
        ...
        if hasattr(self, '_dash_log'):
            target_console = self._dash_log
        active = self._active_tab.get() if hasattr(self, '_active_tab') else 'dashboard'
        if active == 'brain' and hasattr(self, '_brain_log'):
            target_console = self._brain_log
        elif active == 'terminal' and hasattr(self, '_term_log'):
            target_console = self._term_log
        elif active == 'law' and hasattr(self, '_legal_pro_log'):
            target_console = self._legal_pro_log
        if target_console:
