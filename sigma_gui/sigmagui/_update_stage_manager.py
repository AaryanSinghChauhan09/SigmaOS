"""
Auto-split from sigma_gui.py — SigmaGUI._update_stage_manager
"""
...
class SigmaGUI:
    def _update_stage_manager(self, active_key):
        ...
        for w in self._stage_manager.winfo_children():
            w.destroy()
        ...
        for key in self._stage_stack:
            ...
            btn = tk.Button(self._stage_manager, text=icon, font=('Segoe UI Symbol', 16), bg=PAL['bg3'] if is_active else PAL['bg'], fg=PAL['cyan'] if is_active else PAL['dim'], relief='flat', bd=0, width=4, height=2, command=lambda k=key: self._show_page(k))
