"""
Auto-split from sigma_gui.py — SigmaGUI._build_keeping_board
"""
...
class SigmaGUI:
    def _build_keeping_board(self, parent):
        ...
        stickies = [('Shopping List', ['Milk', 'Eggs', 'Caffeine'], PAL['gold']), ('Project Idea', ['Modular Kernel', 'Sovereign UI'], PAL['cyan']), ('Meeting Notes', ['Discuss PQC', 'Finalize ISO'], PAL['green'])]
        for i, (title, items, color) in enumerate(stickies):
            ...
            for item in items:
                ...
                tk.Checkbutton(f, text=item, bg=color, fg=PAL['bg'], selectcolor=PAL['bg'], activebackground=color).pack(side='left')
