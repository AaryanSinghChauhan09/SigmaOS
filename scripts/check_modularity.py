import os
import sys
import re

def check_modularity(directory):
    errors = 0
    # Pattern to detect raw parent-directory includes or legacy rooted includes
    unsafe_patterns = [
        re.compile(r'#include\s+"\.\./.*"'),
        re.compile(r'#include\s+<include/.*>')
    ]
    
    for root, dirs, files in os.walk(directory):
        if ".git" in root or "obj" in root: continue
        for file in files:
            if file.endswith(('.c', '.h', '.cpp', '.hpp')):
                path = os.path.join(root, file)
                with open(path, 'r', errors='ignore') as f:
                    for i, line in enumerate(f, 1):
                        for pattern in unsafe_patterns:
                            if pattern.search(line):
                                print(f"[ERROR]: Legacy/Unsafe include in {path}:{i} -> {line.strip()}")
                                errors += 1
    return errors

if __name__ == "__main__":
    base_dir = "."
    print(f"[SENTINEL]: Auditing shard boundaries in {os.path.abspath(base_dir)}...")
    error_count = check_modularity(base_dir)
    if error_count > 0:
        print(f"[FAIL]: Found {error_count} modularity violations.")
        sys.exit(1)
    print("[PASS]: Shard boundaries verified.")
    sys.exit(0)
