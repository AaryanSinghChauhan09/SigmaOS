...
## 2026-09-13 - Chunked Modulo Deferral for Rolling Checksums & Direct Character Case Folding
**Learning:** Computing Adler-32 rolling checksums with per-byte `% 65521` modulo forces hardware division instructions (`div`/`rem`) on every single byte. Deferring modulo division to chunk boundaries (`NMAX = 3800` bytes) guarantees no integer overflow and reduces modulo operations by ~99.9%. Additionally, calling `to_lowercase()` on strings during fuzzy search passes allocates temporary heap `String` buffers per candidate; performing inline case conversion with `c.to_ascii_lowercase()` on character iterators eliminates all intermediate heap allocations.
**Action:** In checksum and hashing algorithms, defer modulo division until accumulators reach maximum safe bounds (`NMAX`). In string matching loops, map characters inline with `to_ascii_lowercase()` rather than calling `to_lowercase()` on full strings.
