...
## 2026-08-22 - Caching Sliced Byte Lengths for SimpleSyscallEntry in Kernel Syscall Dispatch
**Learning:** In syscall dispatch tables (`SimpleSyscallEntry`), calling `entry.name()` repeatedly executed an $O(N)$ linear byte scan (`.position(|&b| b == 0)`) over 64-byte name arrays on every single syscall registration, lookup, and audit log dispatch. Storing an explicit `name_len: u8` field during `SimpleSyscallEntry::new()` initialization turns slice retrieval into an instantaneous $O(1)$ constant-time lookup `&self.name[..self.name_len as usize]`, eliminating linear zero-byte scans during syscall dispatch and auditing hot paths.
**Action:** Always cache byte lengths when initializing fixed-size byte array records in syscall tables and kernel dispatch filters to guarantee $O(1)$ constant-time slice lookups.
