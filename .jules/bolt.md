...
## 2026-08-27 - Caching Array Byte Lengths for SimpleTerminal Title & Working Directory Buffers
**Learning:** In desktop terminal emulator management (`SimpleTerminal`), calling `title()` or `working_directory()` repeatedly executed $O(N)$ linear byte scans (`.position(|&b| b == 0)`) over 128-byte title and 256-byte working directory arrays on every title query, PTY working directory update, UI draw, and tab rendering. Storing explicit `title_len: usize` and `working_directory_len: usize` fields during initialization and updating `working_directory_len` on `set_working_directory()` replaces linear scans with instant $O(1)$ slice lookups `&self.title[..self.title_len]`, eliminating linear zero-byte scanning overhead in desktop terminal hot paths and avoiding `u8` integer overflow truncation bugs for buffers sized up to 256 bytes.
**Action:** Store `title_len` and `working_directory_len` using `usize` (or `u16`) when managing fixed byte array buffers to ensure $O(1)$ slice access and prevent integer truncation wrapping.
