...
**Learning:** In thermal management and sensor querying routines (`SimpleThermalSensor` in `src/thermal/manager.rs`), calling `ThermalSensor::name(&self)` executed an $O(N)$ linear zero-byte scan (`.position(|&b| b == 0)`) over 64-byte name array buffers on every sensor access or state check. Storing `name_len: u8` during `SimpleThermalSensor::new()` construction turns slice retrieval into an instantaneous $O(1)$ constant-time lookup `&self.name[..self.name_len as usize]`, eliminating linear scanning overhead in thermal management loops.
**Action:** Store explicit byte length fields alongside fixed-size buffer fields in thermal and hardware monitoring structures to guarantee $O(1)$ constant-time slice retrieval.
