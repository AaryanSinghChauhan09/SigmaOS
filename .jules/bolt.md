...
## 2026-09-07 - Pre-reserving Capacity and Direct Ownership Transfer in Dual-Stack Ring Buffers
**Learning:** In dual-vector deques (`VecDeque`), transferring elements between `front` and `back` vectors by cloning each element in an index loop (`for i in 0..len { push(back[...].clone()) }`) incurs $O(N)$ redundant heap/clone operations, forces multiple intermediate vector reallocations, and drops old vector allocations. Calling `reserve(len)` up front and transferring items directly via `while let Some(item) = src.pop() { dst.push(item); }` moves ownership directly, avoids all intermediate reallocations, preserves target buffer capacity, and removes restrictive `T: Clone` bounds.
**Action:** In collection transfers or deque queue refills, always call `reserve()` before transferring items and move owned values via `pop()` rather than cloning.
