...
## 2026-08-27 - Caching Stored Byte Lengths for SimpleContainer Name Buffers
**Learning:** In container runtime inspection hot paths (`SimpleContainer`), calling `container.name()` repeatedly executed an $O(N)$ linear byte scan (`.position(|&b| b == 0)`) over 64-byte container name arrays during container listing, status checks, and process lookup operations. Storing `name_len: u8` during `SimpleContainer::new()` initialization turns slice retrieval into an instantaneous $O(1)$ constant-time lookup `&self.name[..self.name_len as usize]`, eliminating linear zero-byte scanning overhead during high-frequency container runtime management routines.
**Action:** Always store explicit byte length fields (`name_len: u8`) alongside fixed-size buffer fields in container structures to guarantee $O(1)$ constant-time slice lookups.
