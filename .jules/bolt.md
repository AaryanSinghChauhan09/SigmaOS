...
## 2026-08-30 - Caching Explicit Byte Length for SimpleThermalSensor in Thermal Subsystem
**Learning:** In thermal monitoring and thermal throttling management routines (`SimpleThermalSensor` in `src/thermal/manager.rs`), calling `ThermalSensor::name(&self)` repeatedly executed an $O(N)$ linear zero-byte scan (`.position(|&b| b == 0)`) over 64-byte name buffer arrays on every thermal state evaluation, temperature check, and sensor query. Storing `name_len: u8` during `SimpleThermalSensor::new()` initialization turns slice retrieval into an instantaneous $O(1)$ constant-time lookup `&self.name[..self.name_len as usize]`, eliminating linear scanning overhead in thermal management hot paths.
**Action:** Always store explicit byte lengths (`name_len: u8`) when initializing fixed byte array fields in thermal and hardware sensor structures to guarantee $O(1)$ constant-time slice lookups.
