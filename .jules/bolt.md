...
## 2026-08-26 - Caching Byte Lengths in Thermal Sensor Structures
**Learning:** In kernel thermal monitoring loops (`SimpleThermalSensor`), calling `sensor.name()` repeatedly executed an $O(N)$ linear zero-byte scan (`.position(|&b| b == 0)`) over a 64-byte `[u8; 64]` array on every temperature query or status report. Storing `name_len: u8` during `SimpleThermalSensor::new()` initialization converts `name()` retrieval into an $O(1)$ constant-time slice `&self.name[..self.name_len as usize]`, eliminating linear byte scanning during high-frequency thermal monitoring cycles.
**Action:** Cache explicit byte length fields alongside fixed-size byte array fields in hardware and thermal monitoring structures to guarantee $O(1)$ constant-time slice lookups.
