...
## 2026-08-25 - Single-Pass Distance Mapping in Spatial Navigation Candidate Selection
**Learning:** In spatial navigation focus engines (`SpatialNavigationEngine`), calculating target element center coordinates and squared spatial distances inside a `.min_by(|a, b| ...)` comparison closure causes distance calculations to be re-executed for every pair comparison (up to $2(N-1)$ times). Pre-calculating active element center coordinates outside the loop and mapping candidate elements in a single pass via `.map(|node| (dist_sq, node.id)).min_by(...)` guarantees that center coordinates and squared distances are computed exactly once per candidate node ($N$ times total).
**Action:** Always map elements to `(key, value)` pairs in a single pass before invoking `.min_by()` or `.max_by()` when key calculations involve non-trivial floating-point or spatial math.
