...
## 2026-09-14 - Zero-Allocation Borrowed String Pattern Matching in Iterators
**Learning:** In string split iterators (`Split`), calling `SigmaString::from_str(self.haystack)` inside `next()` creates a temporary owned string and heap allocation on every single iteration step to match patterns against `SigmaString`. Extending the `Pattern` trait with direct slice matching methods (`find_in_str` and `find_in_str_from`) with default fallback implementations allows string split iterators to operate directly on borrowed `&str` slices, completely eliminating $O(N)$ intermediate heap allocations during string splitting while preserving 100% trait compatibility.
**Action:** For string search and split iterators, always provide pattern matching methods that operate on borrowed `&str` slices directly instead of wrapping haystack slices in owned custom string structures.
