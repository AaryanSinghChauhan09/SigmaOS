...
## 2026-08-31 - Caching Explicit Byte Length for SimpleWindow Title Buffer
**Learning:** In window management routines (`SimpleWindow` in `src/ui/window.rs`), calling `Window::title(&self)` executed an $O(N)$ linear zero-byte scan (`.position(|&b| b == 0)`) over 128-byte title array buffers on every title access, window list query, or render pass. Storing `title_len: u8` during `SimpleWindow::new()` initialization turns slice retrieval into an instantaneous $O(1)$ constant-time lookup `&self.title[..self.title_len as usize]`, eliminating linear zero-byte scanning overhead in window manager hot paths.
**Action:** Store explicit byte lengths (`title_len: u8`) alongside fixed-size buffer fields in window management structures to guarantee $O(1)$ constant-time slice lookups.
