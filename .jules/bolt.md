...
## 2026-09-07 - Hoisting Outer Dependency Slices in Package Resolution
**Learning:** In package dependency resolution (`resolve_dependencies`), calculating the dependency name slice `dep.name.iter().position(|&b| b == 0)` inside the candidate package loop performs $O(D \cdot P)$ redundant zero-byte searches. Hoisting the dependency slice calculation outside the inner loop reduces zero-byte searches to $O(D)$. Furthermore, when comparing against candidate package names via trait references (`&dyn Package`), candidate package names must still be trimmed of trailing zero bytes to safely handle arbitrary `Package` trait implementations returning fixed zero-padded byte arrays.
**Action:** When searching or matching dependencies against registered items, always calculate dependency boundary slices once before entering candidate loops while preserving element-level boundary trimming for trait-abstracted items.
