## 2026-09-21 - Slice-Based Pattern Traversal in String Splitting
**Learning:** In string iterators (such as `Split::next` in `klib::string`), defining search pattern traits to accept owned/heap abstraction types (`&SigmaString`) forces the split iterator to allocate a full temporary heap `SigmaString` copy (`SigmaString::from_str(self.haystack)`) on *every* iteration step. Updating `Pattern` traits to accept borrowed string slices (`&str`) allows pattern search methods to operate directly on the iterator's borrowed slice (`self.haystack`), completely eliminating $O(N)$ temporary heap allocations during `split()`, `contains()`, and `replace()` operations.
**Action:** Always accept borrowed string slices (`&str`) in search and pattern-matching traits rather than owned struct abstractions to prevent temporary heap allocations during iteration.
