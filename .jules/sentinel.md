...
**Prevention:** Track path segment boundaries across directory separators (`/`, `\`, `:`). Inspect segment lengths and dot-only composition to reject any segment consisting solely of dot characters with length >= 2.

## 2026-09-23 - Path Prefix Confusion Bypass in Developer Execution Exception Rules
**Vulnerability:** `ZorinExecGuardPolicyEngine::evaluate_execution` in `src/security/exec_guard.rs` checked developer path exception rules using naive `binary_path.starts_with(&rule.path_prefix)` without validating component boundary separators (`/` or `\`). When a developer rule allowed unsigned executions under a path prefix without a trailing separator (e.g., `/home/developer/app`), an attacker could execute arbitrary untrusted binaries located in sibling directories (e.g., `/home/developer/app_evil/payload`) and bypass default-deny capability enforcement.
**Learning:** Matching file paths against permitted prefix rules using string `starts_with` without checking path component boundaries (`/` or `\`) allows path prefix confusion bypasses where sibling paths sharing the prefix inherit elevated execution permissions or sandboxing exemptions.
**Prevention:** Always verify path component boundaries when evaluating path prefixes: ensure `binary_path == prefix`, `prefix == "/"`, `prefix.ends_with('/') || prefix.ends_with('\\')`, or that the character immediately following the prefix in `binary_path` is a directory separator (`/` or `\`).
