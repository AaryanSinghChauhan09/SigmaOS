## 2026-09-09 - Environment Variable Key Injection and POSIX Name Format Bypass Vulnerability
**Vulnerability:** `validate_env_key` only checked for `=` and NUL bytes, permitting environment variable names starting with digits (e.g. `123KEY`), or containing hyphens, spaces, and CRLF newlines (`\r`, `\n`), leading to multi-line environment injection and POSIX shell parameter corruption when passed into subprocess execution or C library environments.
**Learning:** Checking only specific blacklisted characters (`b == 0 || b == b'='`) is insufficient for environment variable key validation because non-standard character sets and leading digits violate POSIX IEEE Std 1003.1 variable naming requirements and allow multi-line variable injection into process execution environments.
**Prevention:** In environment variable key validation, enforce strict POSIX variable naming rules (`[a-zA-Z_][a-zA-Z0-9_]*`) by requiring the first byte to be an ASCII letter or underscore (`first.is_ascii_alphabetic() || first == b'_'`) and subsequent bytes to be ASCII alphanumeric or underscore.
