...
3. **Memory Safety & Process Isolation:**
   - Enforce guard page allocations (`alloc_with_guard_page`), stack clash protection (`stack_guard_pages`), and non-executable stack/heap (NX/DEP) policies.
   - Refer to `AGENTS_BUFFER_OVERFLOW.md` and `docs/AGENTS_BUFFER_OVERFLOW.md` for AI agent memory safety protocols.
