...
SigmaOS is strictly modularized to prevent component entanglement:

```text
SigmaOS/
├── kernel/       # Core microkernel (memory, scheduler, syscalls, bootloader)
├── drivers/      # Zero-dependency hardware shards (storage, network, input, display)
├── fs/           # Filesystem drivers (fat32, ext2)
├── tools/        # Sovereign userland (shell, utilities like ls/echo, package manager)
├── init/         # S01_Genesis deterministic init system
├── tests/        # Automated UI/unit tests
└── docs/         # Roadmap, contribution guidelines, architecture
```
