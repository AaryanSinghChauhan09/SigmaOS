...
class DashboardPage(SigmaPage):
    ...
    def build(self):
        ...
        for lbl, cmd in [("📖 Guide Explorer", lambda: self.gui._show_page("nexus_ai")), 
                         ("🛡️ Loophole Audit", lambda: [self.gui._show_page("nexus_ai"), self.gui._notify("Audit", "Scanning System Loopholes...", "INFO")]),
                         ("🤖 Talk to Nexus", lambda: self.gui._launch_app("sigma.ai.nexus_ai"))]:
