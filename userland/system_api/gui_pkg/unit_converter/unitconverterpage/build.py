# Generated method: UnitConverterPage.build
...
class UnitConverterPage:
    def build(self):
        ...
        self.units = {'Length': ['Meters', 'Kilometers', 'Centimeters', 'Millimeters', 'Miles', 'Yards', 'Feet', 'Inches'], 'Mass': ['Kilograms', 'Grams', 'Milligrams', 'Metric Tons', 'Pounds', 'Ounces'], 'Temperature': ['Celsius', 'Fahrenheit', 'Kelvin'], 'Speed': ['Meters per second', 'Kilometers per hour', 'Miles per hour', 'Knots'], 'Time': ['Seconds', 'Minutes', 'Hours', 'Days', 'Weeks', 'Years'], 'Data Storage': ['Bytes', 'Kilobytes', 'Megabytes', 'Gigabytes', 'Terabytes']}
        ...
        def _convert(*args):
            ...
            if cat == 'Length':
                ...
                factors = {'Kilograms': 1, 'Grams': 0.001, 'Milligrams': 1e-06, 'Metric Tons': 1000, 'Pounds': 0.453592, 'Ounces': 0.0283495}
                v_in_kg = v * factors.get(f_u, 1)
                res = v_in_kg / factors.get(t_u, 1)
            elif cat == 'Temperature':
                if f_u == 'Celsius' and t_u == 'Fahrenheit':
                    res = v * 9 / 5 + 32
                elif f_u == 'Fahrenheit' and t_u == 'Celsius':
                    res = (v - 32) * 5 / 9
                elif f_u == 'Celsius' and t_u == 'Kelvin':
                    res = v + 273.15
                elif f_u == 'Kelvin' and t_u == 'Celsius':
                    res = v - 273.15
                elif f_u == 'Fahrenheit' and t_u == 'Kelvin':
                    res = (v - 32) * 5 / 9 + 273.15
                elif f_u == 'Kelvin' and t_u == 'Fahrenheit':
                    res = (v - 273.15) * 9 / 5 + 32
            elif cat == 'Storage':
