...
class SovereignWellnessPage(SigmaPage):
    ...
    def _build_ui(self):
        ...
        tk.Label(focus_fr, text="Keep Scary Things Away" if is_child else "Purge Brain-Rot & Doomscrolling", 
                 font=FONT_SMALL, bg=PAL["card"], fg=PAL["dim"]).pack(anchor="w")
        
        if is_child:
            features = [
                ("Block Sad Videos", True),
                ("Magic Safe Mode", True),
                ("Bright Lights Off", False),
                ("New Toys Daily", True)
            ]
        else:
            features = [
        ...
        if is_child:
            metrics = [
                ("Stand Up Tall", "Looking Good!", "ACTIVE"),
                ("Drink Your Water", "Glug Glug!", "LOG"),
                ("Eye Rest", "Close your eyes", "WAIT")
            ]
        else:
            metrics = [
        ...
        for title, desc, stat in metrics:
            ...
            if stat == "LOG":
                tk.Button(m_fr, text="+ Bottle" if is_child else "+250ml", bg=PAL["accent"], fg="white", font=("Inter Bold", 7), relief="flat", padx=10,
                          command=lambda: self.gui._log_voice("Yummy water!")).pack(side="right")
            else:
        ...
        self.quote_disp = tk.Label(wisdom_fr, text=quote_text, 
                                   font=("Inter Italic", 10), bg=PAL["bg2"], fg=PAL["text"], wraplength=300, pady=15, padx=10)
        self.quote_disp.pack(fill="x", pady=5)
        
        if is_child:
            btns = [
                ("📖 Read a Story", PAL["cyan"]),
                ("🎨 Color a Picture", PAL["gold"]),
                ("🎈 Play a Game", PAL["teal"])
            ]
        else:
            btns = [
        ...
        tk.Label(zen_fr, text="Happy Sounds" if is_child else "Binaural Retuner (432Hz/528Hz)", 
                 font=FONT_SMALL, bg=PAL["card"], fg=PAL["dim"]).pack(anchor="w")
        ...
        for freq in sound_list:
            tk.Button(zen_fr, text=f"▶ {freq}", font=FONT_SMALL, bg=PAL["bg2"], fg=PAL["text"], relief="flat", pady=4, anchor="w",
                      command=lambda f=freq: self.gui._log_voice(f"Playing {f}")).pack(fill="x", pady=1)
        ...
        if is_child:
            tools = [
                ("🧘 Deep Breathing", self._breathing_overlay),
                ("🖼️ Picture Book", lambda: self.gui._log_voice("Opening picture book...")),
                ("🧸 Toy Box", lambda: self.gui._log_voice("Looking for toys...")),
                ("🎭 Happy Filter", self._toggle_grayscale)
            ]
        else:
            tools = [
                ("🧘 Guided 4-7-8 Breathing", self._breathing_overlay),
                ("🖼️ Vision Board Mosaic", lambda: self.gui._log_voice("Gen: Synthesizing vision board...")),
                ("🌡️ AIC/Blood Sugar Log", lambda: self.gui._log_voice("Health: Telemetry log ready.")),
        ...
        for calc in calc_list:
            tk.Button(growth_fr, text=f"🧮 {calc}", font=FONT_SMALL, bg=PAL["bg2"], fg=PAL["cyan"], relief="flat", anchor="w",
                      command=lambda c=calc: self.gui._log_voice(f"Checking {c}")).pack(fill="x", pady=1)
        ...
        tk.Label(ai_fr, text="Robot Friend says Hello!" if is_child else "Real-time Cognitive Load Analysis", 
                 font=FONT_SMALL, bg=PAL["card"], fg=PAL["dim"]).pack(anchor="w")
        ...
        tk.Button(btn_fr, text=btn1_text, font=FONT_BOLD, bg=PAL["accent"], fg="white", relief="flat", padx=20, pady=8,
                  command=lambda: self.gui._log_voice("Shhh... Nap time.")).pack(side="left")
        btn2_text = "🩺 Check Boo-Boo" if is_child else "🩺 AI Symptom Check"
        tk.Button(btn_fr, text=btn2_text, font=FONT_SMALL, bg=PAL["bg3"], fg=PAL["text"], relief="flat", padx=20, pady=8,
                  command=lambda: self.gui._log_voice("Checking boo-boo...")).pack(side="left", padx=10)
