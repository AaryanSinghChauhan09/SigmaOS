# Generated method: CustomizerPage.build
...
class CustomizerPage:
    def build(self):
        ...
        for m in ['Focus', 'Creative', 'Night', 'Neon']:
            tk.Radiobutton(ai_c, text=m, variable=m_var, value=m, bg=PAL['card'], fg=PAL['text'], command=lambda m=m: self.gui._log_voice(self.gui.kernel.registry.get('customizer').generate_ai_theme(m)['message'])).pack(side='left', padx=5)
        ...
        for i in range(0, len(auras), 4):
            ...
            for a in auras[i:i + 4]:
                tk.Radiobutton(row, text=a.capitalize(), variable=a_var, value=a, bg=PAL['card'], fg=PAL['text'], command=lambda a=a: self.gui._log_voice(self.gui.kernel.registry.get('customizer').apply_branding_aura(a)['msg'])).pack(side='left', padx=2)
        ...
        for s in ['Left', 'Right', 'Floating']:
            tk.Radiobutton(lc_c, text=s, variable=s_var, value=s, bg=PAL['card'], fg=PAL['text'], command=lambda s=s: self.gui._log_voice(self.gui.kernel.registry.get('customizer').switch_layout(s, 'Comfortable'))).pack(side='left')
        ...
        for p in ['Sovereign_3D', 'Fluent', 'Retro_8Bit']:
            tk.Radiobutton(lc_c, text=p, variable=p_var, value=p, bg=PAL['card'], fg=PAL['text'], command=lambda p=p: self.gui._log_voice(self.gui.kernel.registry.get('customizer').swap_icon_pack(p))).pack(side='left', padx=5)
        ...
        for v in ['Calm', 'Mechanical', 'Cyber']:
            tk.Radiobutton(sp_c, text=v, variable=v_var, value=v, bg=PAL['card'], fg=PAL['text'], command=lambda v=v: self.gui._log_voice(self.gui.kernel.registry.get('customizer').apply_soundscape(v))).pack(side='left', padx=5)
        tk.Label(sp_c, text='\nAnimation Curve:', bg=PAL['card'], fg=PAL['dim']).pack(anchor='w')
        c_var = tk.StringVar(value='Quartic')
        for c in ['Quartic', 'Bouncy', 'Elastic']:
            tk.Radiobutton(sp_c, text=c, variable=c_var, value=c, bg=PAL['card'], fg=PAL['text'], command=lambda c=c: self.gui._log_voice(self.gui.kernel.registry.get('customizer').adjust_animation_studio(c, 300))).pack(side='left', padx=5)
