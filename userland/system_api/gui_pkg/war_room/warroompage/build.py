# Generated method: WarRoomPage.build
...
class WarRoomPage:
    def build(self):
        ...
        for i, comp in enumerate(comps):
            ...
            if 'n8n' in comp:
                crush_msg = '2s Auto-Plan vs 120s Manual Node Sync.'
            if 'OpenClaw' in comp:
                crush_msg = 'Ring-0 Token Guard vs Plaintext Key Memory.'
            if 'Windows' in comp:
                crush_msg = '290MB Idle vs 4.2GB Blob.'
            tk.Label(card, text=crush_msg, font=FONT_SMALL, fg=PAL['text'], bg=PAL['card']).pack(anchor='w')
        grid.grid_columnconfigure(0, weight=1)
        grid.grid_columnconfigure(1, weight=1)

        def _refresh():
            self.controller._show_page('war_room')
