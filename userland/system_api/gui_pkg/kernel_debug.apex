...
class KernelDebugPage(SigmaPage):
    ...
    def build(self):
        ...
        for c in ("Selector", "Base", "Limit", "Access"): self.gdt_tree.heading(c, text=c)
        ...
        for c in ("Vector", "Mnemonic", "Handler"): self.irq_tree.heading(c, text=c)
        ...
        for c in ("PID", "Name", "State", "Quota"): self.sched_tree.heading(c, text=c)
        self.sched_tree.pack(fill="both")

        def _refresh():
            if not self.winfo_exists(): return
            for i in self.gdt_tree.get_children(): self.gdt_tree.delete(i)
            for i in self.irq_tree.get_children(): self.irq_tree.delete(i)
