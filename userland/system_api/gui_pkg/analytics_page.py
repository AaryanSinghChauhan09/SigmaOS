    ...
    def _build_ui(self):
        ...
        def draw_velocity(event=None):
            ...
            for i in range(len(pts) - 1):
                self.v_canv.create_line(pts[i], pts[i + 1],
                                   fill=PAL["cyan"], width=2.5, smooth=True)
            for i, (xp, yp) in enumerate(pts):
                self.v_canv.create_oval(xp - 3, yp - 3, xp + 3, yp + 3,
                                   fill=PAL["cyan"], outline="")
                self.v_canv.create_text(xp, H - 12, text=f"S{41 + i}",
                                   fill=PAL["dim"], font=("Segoe UI", 7))
                self.v_canv.create_text(xp, yp - 10, text=str(sprints[i]),
        ...
        ttk.Button(bot, text="Export CSV", command=lambda:
                   self.gui._log_voice("Reports exported to /workspace/reports/")).pack(side="right")
