...
class DashboardPage(SigmaPage):
    def __init__(self, parent, gui):
        super().__init__(parent, gui, "Sovereign Dashboard", "Real-Time Kernel Telemetry & Control")
        self.build()
    ...
    def build(self):
        ...
        if is_child:
            stat_defs = [
                ("ram", "MEMORY HAPPY", "12%", PAL["cyan"]),
                ("cpu", "BRAIN SPEED", "4%", PAL["teal"]),
                ("cap", "FUN METER", "MAX", PAL["gold"]),
                ("events", "HAPPY EVENTS", "0", PAL["accent"]),
                ("sec", "SAFETY SCORE", "100", PAL["green"]),
            ]
        else:
            stat_defs = [
        ...
        # Right: Competitor Blame / Friendly Robins
