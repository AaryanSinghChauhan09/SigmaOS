...
impl LlmBackend for AutoBackend {
    ...
    fn generate_stream(&self, messages: &[Message], params: &GenParams, on_token: &mut dyn FnMut(&str)) -> Result<(), LlmError> { self.inner.generate_stream(messages, params, on_token) }
}
