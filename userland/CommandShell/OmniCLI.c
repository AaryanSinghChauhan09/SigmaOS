...
void print_omni_prompt() {
    sigma_printf("\nS SigmaOS Omni-Agent [SOVEREIGN ZENITH]\n");
...
int main(int argc, char** argv) {
    ...
    if (argc > 1) {
        sigma_printf("S [INIT]: Autonomous Routine Execution: %s\n", argv[1]);
    ...
    while (1) {
        ...
        if (sigma_streq(input_buffer, "exit") || sigma_streq(input_buffer, "quit")) {
            sigma_printf("S [EXIT]: Omni-Agent disengaged.\n");
            break;
        }
        ...
        if (sigma_streq(input_buffer, "help")) {
            ...
            sigma_printf("S [AUDIT]: Performing Hardware-Level Integrity Check...\n");
        } else if (sigma_streq(input_buffer, "sigma-status")) {
            sigma_printf("S [STATUS]: SILICON HEALTH: 100% | SHARDS: 350+ | PARITY: SUPREME\n");
            ...
            sigma_printf("S [OMNI-AGENT]: Semantic routing active for: %s\n", input_buffer);
        }
    }
