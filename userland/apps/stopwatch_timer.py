...
class SigmaStopwatch(tk.Tk):
    def __init__(self, kernel=None):
        ...
        self._cd_running=False; self._cd_remaining=0; self._cd_total=0
    ...
    def _build(self):
        ...
        tk.Label(hdr,text="⏱  STOPWATCH & TIMER",fg=PAL["accent"],bg=PAL["panel"],
    ...
    def _build_stopwatch(self,nb):
        tab=tk.Frame(nb,bg=PAL["bg"]); nb.add(tab,text="  ⏱ Stopwatch  ")
        ...
        self._sw_start_btn=tk.Button(btn_fr,text="START",bg=PAL["success"],fg="white",
        ...
        tk.Button(btn_fr,text="RESET",bg=PAL["danger"],fg="white",
                  font=("Segoe UI Bold",11),relief="flat",padx=22,pady=10,
    ...
    def _build_countdown(self,nb):
        tab=tk.Frame(nb,bg=PAL["bg"]); nb.add(tab,text="  ⏳ Countdown  ")
        ...
        self._cd_btn=tk.Button(btn_fr,text="START",bg=PAL["warn"],fg="white",
                                font=("Segoe UI Bold",11),relief="flat",padx=22,pady=10,
    ...
    def _build_pomodoro(self,nb):
        ...
        self._pomo_btn=tk.Button(btn_fr,text="START",bg=PAL["danger"],fg="white",
                                  font=("Segoe UI Bold",11),relief="flat",padx=22,pady=10,
                                  command=self._pomo_toggle)
        self._pomo_btn.pack(side="left",padx=6)
        tk.Button(btn_fr,text="SKIP",bg=PAL["card"],fg=PAL["text"],
                  font=("Segoe UI Bold",11),relief="flat",padx=22,pady=10,
        ...
        tk.Label(cfg_fr,text="Settings",fg=PAL["accent"],bg=PAL["card"],
                 font=("Segoe UI Bold",10)).pack(anchor="w")
        self._work_min=self._mini_spin(cfg_fr,"Work (min)",25)
    ...
    def _sw_toggle(self):
        if self._sw_running:
            ...
            self._sw_start_btn.config(text="START",bg=PAL["success"])
        else:
    ...
    def _cd_toggle(self):
        if self._cd_running:
            ...
            if not self._cd_running and self._cd_remaining==0:
                self._cd_remaining=self._hh.get()*3600+self._mm.get()*60+self._ss.get()
    ...
    def _cd_tick(self):
        ...
        h=self._cd_remaining//3600; m=(self._cd_remaining%3600)//60; s=self._cd_remaining%60
        self._cd_disp.config(text=f"{h:02}:{m:02}:{s:02}")
        pct=self._cd_remaining/self._cd_total if self._cd_total else 1
    ...
    def _cd_reset(self):
        self._cd_running=False; self._cd_remaining=0
        self._cd_disp.config(text="00:00:00",fg=PAL["warn"])
        self._cd_btn.config(text="START",bg=PAL["warn"])
    ...
    def _pomo_toggle(self):
        if self._pomo_running:
            ...
            if not hasattr(self,"_pomo_remaining"):
                self._pomo_remaining=self._work_min.get()*60
            self._pomo_tick()
    ...
    def _pomo_next_phase(self):
        if self._pomo_state=="work":
            ...
            if self._pomo_count%4==0:
                self._pomo_state="long"; self._pomo_remaining=self._long_break.get()*60
                self._pomo_state_lbl.config(text="🌿 LONG BREAK",fg=PAL["success"])
            else:
                self._pomo_state="break"; self._pomo_remaining=self._break_min.get()*60
                self._pomo_state_lbl.config(text="☕ SHORT BREAK",fg=PAL["success"])
        else:
            self._pomo_state="work"; self._pomo_remaining=self._work_min.get()*60
    ...
    def _pomo_tick(self):
        ...
        m=self._pomo_remaining//60; s=self._pomo_remaining%60
