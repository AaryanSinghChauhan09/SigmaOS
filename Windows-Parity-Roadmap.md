...
| 2 | Only FAT32 / Ext2 — no journaling | fs/sigmafs early stage | Implement Ext4 journal (JBD2 rewrite done: fs/ext4\_journal.c) + SigmaFS native | Ph 1 |
| 3 | Memory allocator fragmentation under load | No compaction pass yet | O(1) CAS slab compaction (klib/sigma\_slab\_lockfree.cpp) | Ph 1 |
| 4 | sigma-sh minimal — no scripting, no env vars | Early prototype | Extend with env, scripting, history, completions (userland/shell/sigma\_shell.cpp) | Ph A |
...
| 8 | IPv6 / QUIC missing from network stack | TCP only, IPv4 only | Add UDP, IPv6 (SLAAC/DHCPv6), QUIC (kernel/net/sigma\_net\_ipv6.cpp) | Ph 1–2 |
| 9 | No enterprise identity management | No domain controller | sigma-trustd DID + .sigma-policy group policy + sigma-fleet MDM | Ph 7 |
| 10 | CryptFS uses 32 zero bytes (Issue #44) | Stub derive\_key() | Real Argon2id (time=3, mem=65536) + TPM2 PCR seal | Ph 1 |
...
| 14 | No CI QEMU boot test | Test not wired to Actions | Wire test\_boot\_sequence.sh to sigma\_ci.yml | Ph 0 |
