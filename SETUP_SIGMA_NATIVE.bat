...
if "%COMPILER%"=="g++" (
    echo [BUILD]: Compiling Sovereign Kernel (PID-0)...
    g++ -O3 -std=c++23 -o SigmaKernel.exe sigma_core/kernel/SovereignKernel.cpp >build_log.txt 2>&1
    echo [BUILD]: Compiling Sovereign Advocate (Legal Zenith)...
    g++ -O3 -std=c++23 -luser32 -lgdi32 -o SovereignAdvocate.exe userland/apps/SovereignAdvocate.cpp >>build_log.txt 2>&1
) else (
    echo [BUILD]: Using MSVC toolchain...
    cl /O2 /std:c17 /Fe:SigmaKernel.exe sigma_core/kernel/SovereignKernel.cpp >build_log.txt 2>&1
)
